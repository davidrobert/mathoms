...
- **Lanes abertas agora (2026-04-22 — sync pós-A6g.6b merge):** **☐ livres:** A6e.4 slices 9+ (fase 4a faltam 7 routers: pipeline, workspaces, reports, transactions, llm, invitations, ws); A6e.4 fase 4b restante (documents, tasks — config ✅ em slice 8); A6e.events-migration (~14 audit inline → eventos, destravada pela A6e.events ✅ — **sem prompt**); A6g.3 backend sweep (🚧 ativo em `friendly-elion-088f27`); A6-human smoke (gate humano). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
