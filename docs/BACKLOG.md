...
- **Restante:** A6e (**.events-followup** ⏸ — ativar flag notif em prod + remover cron, aguarda janela F7) · **A6g.2b T3** scripts com goldens (destravado por A6c.3 ✅ 2026-04-24) · A6g.3 🚧 parcial rodada 3 (P1 em `content_classifier`/`pipeline_service`/`models/task.py`/repositories) · F7 (7A-7F + LGPD).
...
Resíduos da F12 (a11y + axe-core + Lighthouse + snapshots) ficam na
lane [`report-a11y-finalize`](#lanes-abertas-agora--pickup-table);
resíduos da F13 (smoke humano + CHANGELOG v1 + ARCHITECTURE/RUNBOOK)
ficam em [`report-v1-polish`](#lanes-abertas-agora--pickup-table).
...
| batch2.14 | **Checklist WCAG 2.1 AA operacional** — por seção do relatório: contraste (`<MonetaryValue>` vermelho em `--color-compare-neg`), teclado em Kanban, ordem de tab em seções. **Absorvido como output da lane [`report-a11y-finalize`](#lanes-abertas-agora--pickup-table)** (axe-core + Lighthouse + tab-order); checklist complementa. | P2 | 2h | ☐ | — |
