...
>
> **Última atualização:** 2026-04-22 (5 lanes órfãs ganham linhas dedicadas na tabela "Lanes abertas agora" + §A6e/§A6g sub-fases: A6e.3c · A6e.events-migration · A6e.events-followup · A6g.6b · A6g.2c — antes só em prose, ADRs ou follow-up notes de rows ✅ fechadas)
...
**Status global (2026-04-22 — pós-merge A6e.3b + A6e.events + A6e.4 slices 1-7 + A6g.6):**
- **Entregues ✅:** A5a-A5f · A6a · A6b · A6b.5 · A6c · A6d (2026-04-20) · **A6f.1** (pipeline-as-service HTTP boundary, ADR-112, 2026-04-21) · A6f.2/.3/.4/.5a/.6 · **A6g.1** (audit baseline 2026-04-21) · **A6g.2 1ª rodada** (T1.a/b/c + T2.a/b, 2026-04-21/22 — T3 scripts com goldens fica como A6g.2b pós-A6c.3) · **A6g.5** (tests sweep, 2026-04-21) · **A6g.7** (Go prep + ADR-113, 2026-04-22) · **A6e.3** (3 slices: FamilyMember + Category + Goal use cases, 2026-04-21) · **A6e.3b** (3 slices: ConfigBlob + Task + Document, 25 use cases + 61 testes, 2026-04-22) · **A6e.5** (`/api/v1/` + aliases + ADR-108, 2026-04-22) · **A6e.events** (domain events infra + 2 agregados migrados + ADR-115, 32 testes, 2026-04-22) · **A6g.4** (rodadas 1+2+3, T2 `ts_long_files` frontend zerado, 2026-04-22) · **A6g.6** (enforcement automatizado — Ruff + ESLint + pre-commit hooks + AST tests + audit baseline decrescente, ADR-114, 2026-04-22).
...
- **Lanes abertas agora (2026-04-22 — sync pós-A6g.6 merge):** **🚧 ativos:** A6e.4 slices 4+ (worktree `dreamy-taussig-8cb686`). **☐ livres:** A6e.4 fase 4b (config/documents/tasks destravados por A6e.3b ✅); A6e.events-migration (~14 audit inline → eventos, destravada pela A6e.events ✅ — **sem prompt**); A6g.6b sweep pós-enforcement (ruff --fix I001/F541 tocando 356 arquivos + ruff format nos 422 pendentes + promove max-lines warn→error — **sem prompt**; destravada por A6g.6 ✅); A6-human smoke (gate humano). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
...
| **A6g.2c** rename `pipeline/llm/service.py` (filename genérico) | `a6g2c-rename-llm-service` | Filename `service.py` sozinho viola `forbidden-names` (CLAUDE.md §Code style); hoje em ALLOWLIST de `dev/check_forbidden_names.py`. Renomear para `pipeline/llm/llm_service.py` (ou específico); atualizar imports. **Sem prompt** — escopo trivial <0.2 sessão | A6g.6 ✅ (hook ativo) | 3 | ☐ aberta |
...
| A6e.3c | Sweep `dict[str, Any]` → tipado em DTOs não-OPAQUE (follow-up ADR-114) | 4 arquivos em `schemas/dto/{family_member/*, category/mapper.py}`; promove `LEGACY_FILES` → `CLEAN_FILES` em `test_no_any_in_boundary.py` | 0.5 sessão | ☐ destravada por A6g.6 ✅ |
...
| A6g.2c | Follow-up A6g.6: rename `pipeline/llm/service.py` (filename genérico, hoje em ALLOWLIST `check_forbidden_names.py`) | Renomear para `pipeline/llm/llm_service.py` + atualizar imports; remove entry da ALLOWLIST | 0.2 sessão | ☐ destravada por A6g.6 ✅ |
