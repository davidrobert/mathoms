...
- **Lanes abertas agora (2026-04-22 — sync pós-A6g.7 merge):** Onda 1: A6g.2 🚧 ocupada; A6g.4 ☐ aberta para rodada 3 (A6g.4c, `plano/*`); Onda 2: A6e.3 🚧; **livres com prompt pronto:** A6e.3b (destravada por A6f.1 ✅), A6e.4 (fase 4a), A6-human (gate manual). A6e.6 ⏸ preferencial pós-A6e.3. A6g.7 Go prep ✅ 2026-04-22 (ADR-113). Ver tabela e diagrama abaixo. **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
