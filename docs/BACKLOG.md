...
**Status global (2026-04-27):** 🚧 em andamento — **A7.0 ✅** + **A7.1 ✅** + **A7.2a ✅** + **A7.2b ✅** + **A7.4 ✅** (Onda 2 fechada: 5 lanes mergeadas no mesmo dia). **A7.6 aberta** (rules-as-code: dissolver `docs/methodology/` que A7.4 introduziu como solução incompleta — dados cliente PII + regras universais misturadas violam CLAUDE.md §Regras críticas; gate G1 ✅ (CTO 2026-04-27) — 4 ADRs draft + CTO sign-off antes de codar). A7.3 destravada por A7.1 (Onda 3 livre).
...
| **A7.3** Catalog + Override resolver (categorization + institutions) | `a7-3-catalog-override` | [track_a7_3_catalog_override.md](agent_prompts/track_a7_3_catalog_override.md) | A7.1 ✅ | 3 | — | 🚧 em andamento — branch `agent/a7-3-catalog-override/20260427-1301` |
...
| **A7.6** Rules-as-code (dissolver `docs/methodology/`) | `a7-6-rules-as-code` | [track_a7_6_rules_as_code.md](agent_prompts/track_a7_6_rules_as_code.md) | A7.4 ✅ + ADR-143/145/146/147 (G1) | 2.5 | A7.2a, A7.3 | 🚧 em andamento — branch `agent/a7-6-rules-as-code/20260427-1311`. Gate G1 ✅ (CTO 2026-04-27, ADRs 143/145/146/147 Decididos). **Substitui A7.4 retroativamente:** A7.4 moveu arquivos cliente-PII para `docs/methodology/` violando CLAUDE.md §Regras críticas; A7.6 dissolve o diretório (regras universais → docstrings + ADRs · dados cliente → DB ou `storage/<ws>/notes/`). |
...
| **v2.D.1.1** product-designer revisa copy templates `narratives.py` antes de v2.8 flipar YAML | débito v2.D.1 (templates determinísticos sem revisão de design) | `report-v2-narratives-copy-review` | inline (review only — sem branch) | v2.D.1 ✅ + product-designer disponível | D | S (≤2h) | P2 | ☐ aberta — 6 templates em `pipeline/domain/services/snapshot_changelog/narratives.py::TEMPLATES` (`up`/`down`/`stable`/`from_zero`/`to_zero`/`both_zero`). Genéricos por design (paridade `EXEMPLO_DE_RELATORIO.html` indireta). Antes de v2.8 ativar, [product-designer](.claude/agents/product-designer.md) revisa cópia/tom (atual: "{section_label} cresceu {pct_str} desde o relatório anterior" etc.) — pode evoluir para narrativa LLM em lane v3 separada (ADR-148 §Coordenação). |
...
| **v2.9.1** product-designer revisa copy de prompts em `config/prompts/section_summaries.yaml` antes de habilitar LLM em prod | débito v2.9 (templates editoriais placeholder, sem revisão de design) | `report-v2-section-prompts-copy-review` | inline (review only — sem branch) | v2.9 ✅ + product-designer disponível | C | S (≤2h) | P2 | ☐ aberta — 13 user_prompts em `config/prompts/section_summaries.yaml::sections` (S1, S2, S3, S4, S7, S8, S9, S10, T2, T3, T5, U1, U2) + system_prompt compartilhado. Genéricos por design (placeholder Fase 2 — paridade `EXEMPLO_DE_RELATORIO.html` indireta). [product-designer](.claude/agents/product-designer.md) revisa tom/voz/few-shot examples antes de flipar `MATHOMS_LLM_SECTION_SUMMARIES=1` em staging/prod. Sem mudança de schema (`SectionSummaryOutput`); só copy. |
...
- **Onda D 1/2 entregue:** ~~v2.D.1~~ ✅ **2026-04-27** (SnapshotChangelogBuilder + ADR-148; renumerada de 143 por colisão A7.6).
  Resta v2.8 (ativar comparisons/changelog YAML + render). v2.D.1.1 aberta como débito de copy review.
- **Onda C 3/3 entregue 2026-04-27:** ~~v2.7~~ ✅ · ~~v2.9~~ ✅ (Fase 2; v2.9.1 aberta para revisão de copy) · ~~v2.10~~ ✅.
