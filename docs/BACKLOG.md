...
**Status global (2026-04-27):** 🚧 em andamento — **A7.0 ✅** + **A7.1 ✅** + **A7.4 ✅** (Onda 2 progredindo: cutover `materialize_config` + reorganização editorial `docs/methodology/` mergeados; configs A7.1 fluem via `WorkspaceContext.config_overrides` populado por `build_config_overrides_from_db` no worker boundary; 4 docs metodológicos vivem em `docs/methodology/`). Onda 2 ainda livre para A7.2a, A7.2b (2 lanes paralelas). A7.3 destravada por A7.1.
