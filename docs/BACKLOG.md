...
| 7F.17 | **Exclusão de documento individual (UI + serviço):** complementa purge bulk (7F.12) permitindo deletar **um** upload específico por `document_id`; UI lista documentos por `user_id`/`workspace_id` com metadados (nome, data, tamanho, tipo) e ação "excluir" por linha; `internal_ops.delete_document(document_id)` remove registro + blob em storage ([storage.py](backend/app/services/storage.py)); confirmação simples (não dupla — escopo menor que purge); audit inclui hash/nome do arquivo removido | P0 | 4h | ☐ |
