...
>
> **Última atualização:** 2026-04-25 (Report Premium UI **v2 roadmap aberto** — auditoria pós-v1 catalogou 3 inconsistências, 3 débitos declarados e 3 lacunas em 11 sub-lanes paralelizáveis em 4 ondas; meta-prompt em [track_report_v2.md](agent_prompts/track_report_v2.md), tabela detalhada em [§Report Premium UI v2](#report-premium-ui--paridade-com-exemplo_de_relatoriohtml). Onda A pronta para pickup paralelo (v2.1, v2.2, v2.3). · lane `report-v1-polish` ✅ 6/6 fechada — Report Premium UI v1 oficialmente entregue: milestone consolidado no CHANGELOG, ARCHITECTURE §10 com tree decomposto, RUNBOOK §8 debug da rota, SMOKE §5.2 checklist humano. · lane `adr-129-e6-kill` ✅ 6/6 fechada — renderer HTML server-side erradicado; React é único renderer; PDF Playwright único export server-side. · Report Premium UI — 10/10 fases úteis v1 entregues em main: F0 discovery → F10 apêndices. **Fases 11/12/13 canceladas via [ADR-129](DECISIONS.md#adr-129--descontinuação-completa-do-renderer-html-server-side)**. · F7F-Local MVP fechado · A6e.4 ✅ fase 4a completa 14/14.)
...
| **Report Premium UI v2** (guarda-chuva — 11 sub-lanes paralelizáveis em 4 ondas) | `report-v2-*` (sub-slug por lane) | Meta-prompt + paralelização: [track_report_v2.md](agent_prompts/track_report_v2.md). **Pickup hoje (Onda A inteira + 2 da B):** v2.1 `report-v2-yaml-placeholders` (P0, ≤2h, comparisons/changelog placeholders no YAML — cumprir promessa BACKLOG §3.1 da auditoria) · v2.2 `report-v2-visual-baselines` (P0, ≤4h, trigger workflow_dispatch + commit baselines Linux) · v2.3 `report-v2-s5-s6-clarification` (P1, ≤4h, decisão sobre S5/S6 ausentes) · v2.5 `report-v2-score-dto` (P2, ≤4h, score top-level no DTO) · v2.6 `report-v2-cards-cleanup` (P2, 1d, decidir cards/ legacy). **Bloqueadas hoje:** v2.4 (idealmente após v2.5; [prompt dedicado](agent_prompts/track_report_v2_t2_aportes.md)) · v2.D.1 + v2.8 ([prompt dedicado fundido](agent_prompts/track_report_v2_changelog_engine.md), aguarda v2.1) · v2.7/v2.9/v2.10 (Onda C, paraleláveis após Onda B). Tabela detalhada + ondas + pickup protocol em [§Report Premium UI v2](#report-premium-ui--paridade-com-exemplo_de_relatoriohtml). | v1 ✅ (10 fases + 3 lanes residuais) | P0/P1/P2 (mistura por lane) | ☐ aberta — Onda A pronta para pickup paralelo (3 agentes simultâneos OK) |
...
|------|--------|-------------|--------|------------|------|---------|------|--------|
| **v2.1** comparisons/changelog placeholders no YAML | §3.1 auditoria | `report-v2-yaml-placeholders` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | A | S (≤2h) | P0 | ☐ aberta |
| **v2.2** trigger baselines visuais Linux + commit | §3.5 auditoria | `report-v2-visual-baselines` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | `report-a11y-finalize` ✅ | A | S (≤4h) | P0 | ☐ aberta |
| **v2.3** S5/S6 esclarecimento (auditoria + decisão) | §4.1 auditoria | `report-v2-s5-s6-clarification` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | A | S (≤4h) | P1 | ☐ aberta |
...
**Pickup hoje (Onda A inteira + 2 da B):**
v2.1, v2.2, v2.3, v2.5, v2.6 — todas independentes, prontas para pickup
paralelo por agentes distintos. Conferir colisão com `git worktree list`
+ `git for-each-ref refs/remotes/origin/agent/report-v2-*`.
