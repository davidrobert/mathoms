...
> aggregate Decision/ADR-136 entregue na Sprint A7.5 sem UI; copy
> PT-BR de leigo escondendo vocabulário event-sourced; rationale
> obrigatório ≥10 chars). **Onda 3** (`cf14af6`): Modo Tático removido
> do relatório ([ADR-151](DECISIONS.md#adr-151--remoção-do-modo-tático-do-relatório-direção-e-do-redesign-de-interfaces),
> supersede parcial ADR-117/ADR-123); `tatico:` removido do YAML;
> `plano_de_acao` movido para `estrategico.sections`; `TaticoSections.tsx`
> 494 LOC + `aportesAdapter.ts` deletados; ReportMode reduzido a
> `'estrategico' | 'usa'`. **Onda 4** (`775df4b`): `/plano` virou
> executive summary com KPIs row (Patrimônio · IF % · Aporte alvo) +
> SuggestionsBanner (oculto até Onda 5 ligar fonte de dados real;
> stub `useSuggestionsCount` retorna 0). **Onda 6** (`a2db28f`): rota
> `/acao` com 4 tabs ([ADR-152](DECISIONS.md#adr-152--plano-de-acao-renomeada-para-acao-com-tabs-direção-e--onda-6))
> — Inbox · Tarefas · Timeline · Notas; ActionStatusBar agrega
...
| **A8.2** IRPF full schema (E1.6 — pipeline + analyzer + E5 wire) | `irpf-full-schema` | [track_irpf_full_schema.md](agent_prompts/track_irpf_full_schema.md) — destrava 6 KPIs do relatório premium (renda anual líquida, alíquota dual, capacidade PGBL, split trabalho×capital). [ADR-157](DECISIONS.md#adr-157--schema-irpf-completo-stage-extract_irpf_full). | A7 ✅ | 🚧 pipeline + tests ✅ entregues 2026-04-30 (ADR-157 + schema + prompt + validator + stage + analyzer + E5 wire + 22 tests); **goldens** ✅ entregue 2026-04-30 ([track](agent_prompts/track_irpf_full_schema_goldens.md): 3 fixtures sintéticas completo/simplificado/edge_cases + 28 tests stage runner com `FakeStructuredLLMClient` em [tests/fakes/llm.py](../tests/fakes/llm.py)); **UI** (S_IRPF_RENDA + S_IRPF_OTIMIZACAO no relatório premium) ✅ entregue 2026-04-30 via [track_irpf_full_schema_ui.md](agent_prompts/track_irpf_full_schema_ui.md) — YAML+codegen, 2 sections, 5 cards, 2 charts (Chart.js), narrow guard `isIrpfKpis`, hook `useIrpfKpis`, 16 testes, degrada gracioso quando workspace sem IRPF. Pendente: G0/G4 sign-off em PR comment + visual baselines + Playwright `@critical` |
| **A8.3** TRS real — Carteira de renda + Taxa de Retirada Sustentável efetiva (S7) | `a8-trs-real` | [track_a8_trs_real.md](agent_prompts/track_a8_trs_real.md) — destrava `rule_trs_desalinhada` dormente; sai do placeholder `"N/D"` em `ratios_calculator.py`; 4 KPIs novos no S7 (Renda passiva R$/mês + Patrimônio investido + TRS efetiva + Em acumuladores) + tooltip `Info` icon WCAG-compliant + caption permanente em acumulação + 2 banners + 2 empty states. **G0 (financial-planner) ✅** + **G4 (product-designer) ✅** já feitos no track. Orquestrado em 2 fases: Fase 1 paralela (PR-A Calculator+ratios, PR-B fix bucket aluguéis); Fase 2 sequencial (PR-C wire+UI+ADR). | A7 ✅ + A8.2 ✅ | ✅ entregue 2026-05-05 — PR-B [#42](https://github.com/davidrobert/mathoms/pull/42) (aluguel→capital) + PR-A [#43](https://github.com/davidrobert/mathoms/pull/43) (Calculator+ratios, 31+21 tests) + PR-C [#44](https://github.com/davidrobert/mathoms/pull/44) (wire+UI+ADR-164 + 33 Vitest na matriz 18 cenários). 3 fases × 3 agentes paralelos/sequenciais. Tempo total ~5h wall-clock, ~95min trabalho ativo do agente C. ADR-164 "Carteira de renda e TRS efetiva" Decidido. |
| **A8.4** Cenários de Estresse — remoção de prototipagem família-específica + APP_C universal | `cenarios-estresse` (sub-slugs `-docs`, `-pr1`..`-pr5`) | [CENARIOS_ESTRESSE_PLAN.md](CENARIOS_ESTRESSE_PLAN.md) — remove **Modo USA inteiro** (U1-U4: Mudança EUA F1/F2, Green Card EB2-NIW, NCLEX Roadmap, Simulação Mariana) + 2 cenários hardcoded ("Com NCLEX", "Com NCLEX + Green Card") do analyzer; preserva como capability genérica o cenário "Sem renda do cônjuge" com **gate de elegibilidade no domain service** (`should_render_conjuge_scenarios`); rename de chave universal `cenarios_mariana` → `cenarios_conjuge` no payload E5 (5 sites do producer); APP_C "Cenários de Estresse" com hide-when-empty + numeração A/B/C/D/E estável + visualização comparativa lado-a-lado base vs estresse. **4 especialistas consultados** (financial-planner + product-designer + senior-cto + data-engineer). **6 PRs sequenciais:** PR0 docs · PR1 schema rename · PR2 gate + analyzer · PR3 frontend + APP_C · PR4 delete USA · PR5 limpeza. **3 ADRs:** ADR-165 (remoção Modo USA, supersede parcial ADR-117/123, conclui ADR-151) · ADR-166 (schema estável `cenarios_conjuge`) · ADR-167 (eligibility gate). | A7 ✅ + A8.2 ✅ | 🚧 PR0 docs em revisão · branch `agent/cenarios-estresse-docs/20260506-1430` |
...
**Status global (2026-05-06):** 🚧 Onda 0 em curso (A10.0) — ADRs 177..181 em `Proposto`; Onda 1 destrava após merge.
...
> **Pickup protocol** idêntico ao Sprint A6/A7: `git worktree list` + `git for-each-ref refs/remotes/origin/agent/` antes de escolher. Tabela é dica.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **A10.0** ADRs Proposto batch (ADR-177..181) | `a10-0-adrs` | [GOALS_JSON_CUTOVER_PLAN.md §4](GOALS_JSON_CUTOVER_PLAN.md) | — | 0 (bloqueante) | — | 0.5d | 🚧 PR aberto |
| **A10.1** Dead-data deletion + ADR-168 narrativas órfãs | `a10-1-dead-data` | [§2.3 + §5.1](GOALS_JSON_CUTOVER_PLAN.md) | A10.0 ✅ | 1 | A10.2 | 1d | ☐ proposta |
| **A10.2** Rules-as-code consolidation (ADR-177) | `a10-2-rules-as-code` | [§2.2 chaves U/M/O + ADR-177](GOALS_JSON_CUTOVER_PLAN.md) | A10.0 ✅ | 1 | A10.1 | 1d | 🚧 PR aberto |
| **A10.3** Decision schema extension (ADR-179) | `a10-3-decision-extension` | [§3.3 + ADR-179](GOALS_JSON_CUTOVER_PLAN.md) | A10.0 ✅ | 2 | A10.4, A10.7 | 1.5d | 🚧 PR aberto |
| **A10.4** `Risk` aggregate (ADR-178) | `a10-4-risk-aggregate` | [§3.4 + ADR-178](GOALS_JSON_CUTOVER_PLAN.md) | A10.0 ✅ | 2 | A10.3, A10.7 | 2d | 🚧 PR aberto |
...
### Ondas A10 — mapa de dependências
...
```bash
# Checks finais (todos devem retornar verde)
...
**Plano canônico:** [docs/PLATFORM_REVIEW_PLAN.md](PLATFORM_REVIEW_PLAN.md) — 32 tasks em 6 ondas, 138 findings consolidados de revisão multi-agente, 6 trade-offs CTO registrados, coverage matrix por stage.
...
**Status global (2026-05-07):** 🚧 Wave 1 ✅ entregue (8/8 tasks) · Waves 2-6 ☐ aguardam pickup. Plano também referenciado em [CLAUDE.md §"Onde procurar contexto adicional"](../CLAUDE.md#onde-procurar-contexto-adicional).
**Objetivo (1 frase):** executar os 32 itens consolidados da revisão de plataforma 2026-05-06 — fechar P0 latentes em main (tokens fantasma, regras dormentes, PII em pipeline_artifacts), destravar produção pública (F7B/F7E security + email + backup), e quitar 3 grandes débitos arquiteturais (schemas E5 strict, MLOps universal hooks, F9 cleanup).
**Princípios não-negociáveis:** (P1) NEXT UP do PLAN.md responde "qual task pegar agora" em <30s — manter atualizado; (P2) ADR `Proposto` antes de PR P0/P1 (CLAUDE.md §"Política operacional"); (P3) coverage gaps explícitos — E0/E1.5c/E7-crossval/E7-apply ficam para próxima revisão (Q3 2026); (P4) decisão CTO em conflito entre agentes registrada em `§Trade-offs registrados` do PLAN.
**Supervisão:** padrão A7/A10 (gates G1 ADR draft / G2 schema review / G3 PR pré-merge / G4 wave boundary).

### Por que esta sprint existe

A revisão multi-agente 2026-05-06 ([PR #92](https://github.com/davidrobert/mathoms/pull/92)) levantou 138 findings que são complementares à Sprint A10 (`goals.json` cutover) — A10 fecha 1 frente específica, A11 ataca o resto da plataforma (security pré-prod, observabilidade, schema hardening, frontend a11y, metodologia financeira). Sem entrada índice no BACKLOG, agente futuro descobriria o trabalho só por arqueologia em `docs/PLATFORM_REVIEW_PLAN.md` — fonte de verdade hoje, mas invisível à navegação por sprint. Esta entrada fecha o gap.

### Lanes A11 — pickup table

> **Pickup protocol** idêntico ao Sprint A6/A7/A10. Detalhe operacional (acceptance_criteria, files_touched, paired_doc_task, risk/rollback) está em [PLATFORM_REVIEW_PLAN.md](PLATFORM_REVIEW_PLAN.md) — esta tabela é índice.
> **Bloqueio duro:** Wave 2 só destrava após Wave 1 ✅. Wave 4 só após W4-T01 (backup off-site) validado em drill.

| Lane | Plano | Depende de | Onda | Status |
| --- | --- | --- | --- | --- |
| **W1** Hot patches + ADR backfill (8 tasks) | [PLAN §W1](PLATFORM_REVIEW_PLAN.md#wave-1--hot-patches--adr-backfill-sprint-imediato-5-dias-dev) | — | 1 (5d) | ✅ entregue 2026-05-06/07 |
| **W2** Pipeline + DB hardening (6 tasks) | [PLAN §W2](PLATFORM_REVIEW_PLAN.md#wave-2--pipeline--db-hardening-sprint-1-7-dias-dev) | W1 P0 ✅ | 2 (7d) | ☐ ready (W1 mergeada) |
| **W3** Auth + LLM ops + Email (5 tasks) | [PLAN §W3](PLATFORM_REVIEW_PLAN.md#wave-3--auth--llm-ops--email-sprint-2-12-dias-dev) | W2 ✅ | 3 (12d) | ☐ blocked |
| **W4** Production readiness (5 tasks) | [PLAN §W4](PLATFORM_REVIEW_PLAN.md#wave-4--production-readiness-sprint-3-10-dias-dev) | W3 ✅ + drill backup | 4 (10d) | ☐ blocked |
| **W5** Frontend + Methodology (5 tasks, paralelo W6) | [PLAN §W5](PLATFORM_REVIEW_PLAN.md#wave-5--frontend--methodology-sprint-4-10-dias-dev) | W1 ✅ (parcial) | 5 (10d) | ☐ ready |
| **W6** Tech debt cleanup (6 tasks) | [PLAN §W6](PLATFORM_REVIEW_PLAN.md#wave-6--tech-debt-cleanup-sprint-5-12-dias-dev) | W3 (T02 → W6-T02) | 6 (12d) | ☐ blocked parcial |
...
- **Hotspot principal:** [docs/PLATFORM_REVIEW_PLAN.md](PLATFORM_REVIEW_PLAN.md) — quando wave fecha, atualizar `frontmatter.ready_tasks` + status na tabela de cada wave + adicionar checkbox ✅ na seção da wave. Manter NEXT UP sincronizado.
...
- **Re-sync periódico em sessão >1h:** `git fetch origin && git log --oneline HEAD..origin/main` a cada ~30min. Se outra task A11 mergeou, releia [PLATFORM_REVIEW_PLAN.md §<wave em curso>](PLATFORM_REVIEW_PLAN.md) — princípios podem ter ganhado nuance.
- **Trade-offs registrados:** se discordar de uma decisão CTO em [§Trade-offs registrados do PLAN](PLATFORM_REVIEW_PLAN.md#trade-offs-registrados-decisões-cto), abrir issue antes de implementar conflitando — não rebater silenciosamente.

### Definition of Done — Sprint A11
...
# 3. Plano arquivado:
git mv docs/PLATFORM_REVIEW_PLAN.md docs/archive/PLATFORM_REVIEW_PLAN-YYYY-MM-DD.md
...
| **v2.2b** baselines Tático+USA (resíduo v2.2) | spillover de v2.2 | `report-v2-visual-baselines-tatico-usa` + `report-v2-usa-retomar` | inline (ver detalhes em v2.2 status) | v2.2 ✅ parcial | A | S (≤2h) | P1 | ✅ **completa 2026-04-27 (20/20)** · **Tático (12/20 — `d4e0dfe`+`029c3d9`):** `clickMode()` falhava por dois motivos sobrepostos. (1) o toggle real é `ReportActions` (não `ModeToggle`), com `<button role="tab">` envolto em `<TooltipTrigger>` — label "Tático"/"EUA" fica fora do `<button>`, então `getByRole("button", {name})` não casa; (2) modo "usa" foi removido de `VALID_MODES` em `adc3a15` (decisão de produto), então `?mode=usa` caía no default. Fix do helper: `setupReport(page, theme, mode)` aceita `mode` opcional e navega via deep-link `?mode=tatico\|usa` em vez de click. `usa` re-incluído em `VALID_MODES` (apenas no Set). Run [25002843680](https://github.com/davidrobert/mathoms/actions/runs/25002843680) gerou 12 baselines Tático T1-T6 × {light,dark}. · **USA (8/20 — `e19e4cf`+`00ab2c6`+`52176e7` 2026-04-27):** decisão de produto autorizou retomar o modo USA. Reverte parcialmente `adc3a15`: U1-U4 `enabled:true` em `config/report_layout.yaml` + bloco `navigation.usa` descomentado + codegen TS+Pydantic regerados; `ReportActions.VISIBLE_MODES` e `ModeToggle` voltam a expor a aba "EUA" no tablist; `ReportShell.test` re-afirma asserção positiva de `getByRole("tab", { name: "EUA" })`. Spec visual `sections.snapshots.visual.spec.ts` troca `test.describe.skip("Snapshots — modo USA")` por `test.describe(...)`. Run CI dispara `update_visual_baselines=true` para popular as 8 baselines U1-U4 × {light,dark}. Vitest 668/668 verde (1 skipped não-USA). Caso A (decisão estética/produto pura, sem débito de dados) — UsaSections.tsx components já existiam (v2.3 mapeou ex-S5/S6 → U1/U2). **Regressão pre-existente detectada** (fora de escopo v2.2b): 28 baselines estratégicas+APP+cover existentes "passam" no run #24952539088 base `0558ea3` (2026-04-26 manhã) mas "skipam" em runs HEAD com `count() === 0` para `section#S1[data-report-section]` — não causada por v2.2b. Investigar em lane separada (commits candidatos: `db6cf6f` cover identity, `35eee5f` Hero out of S1, `a534e9d` header refactor). |
...
- ✅ **Onda A: 3/3 entregue** — v2.1 ✅ + v2.3 ✅ + v2.2 ✅ parcial (28/48 baselines + gate validado) + v2.2b ✅ completa 2026-04-27 (Tático 12 + USA 8 baselines).
...
- ✅ **v2.2b completa** Tático + USA (USA re-habilitado 2026-04-27 via `e19e4cf`+`00ab2c6`+`52176e7` — decisão de produto retomada)
...
- ✅ v2.2b USA — re-habilitação 2026-04-27 (`e19e4cf`+`00ab2c6`+`52176e7`); 8 baselines U1-U4 × {light,dark} via `gh workflow run CI -f run_visual=true -f update_visual_baselines=true` na branch
...
| batch2.13 | **Spec mobile do relatório** — decidir e documentar: que seções saem, que charts ganham fallback, como Kanban vira lista em `<767px`. Adicionar em REPORT_PREMIUM_PLAN Delta novo ou seção. | P2 | 3h | ✅ 2026-04-27 (`4c76c4b`) | Docs-only escolhido. Spec entregue em [REPORT_MOBILE_SPEC.md](REPORT_MOBILE_SPEC.md) + delta em [REPORT_PREMIUM_PLAN §17.10](REPORT_PREMIUM_PLAN.md). Implementação fica em lane futura `report-mobile-impl`. |
