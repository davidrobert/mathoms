...
- **Lanes abertas agora (2026-04-22 — sync pós-A6g.3b polish merge):** **☐ livres:** A6e.4 fase 4a (faltam 7 routers: pipeline, workspaces, reports, transactions, llm, invitations, ws); A6g.3 backend sweep rodada 3 (`content_classifier` 621l, `pipeline_service` P1×4, `models/task.py`, repositories — prompt `track_a6g3_backend_style_sweep.md`, 🚧 ativo em `friendly-elion-088f27`); A6-human smoke (gate humano). **A6g.3b ✅ fechado 2026-04-22** — P5 backend 10→1 residual `saldo_diff` (tolerance, não money). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
