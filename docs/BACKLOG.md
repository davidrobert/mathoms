...
>
> **Última atualização:** 2026-04-22 (A6e.3c ✅ — 4 DTOs de FamilyMember/Category tipados, `dict[str, Any]` → `dict[str, object]` + TypedDicts; AST gate sobe de 31 → 35 CLEAN_FILES)
...
**Status global (2026-04-22 — pós-merge A6e.3b + A6e.events + A6e.4 slices 1-7 + A6g.6 + A6g.2c):**
- **Entregues ✅:** A5a-A5f · A6a · A6b · A6b.5 · A6c · A6d (2026-04-20) · **A6f.1** (pipeline-as-service HTTP boundary, ADR-112, 2026-04-21) · A6f.2/.3/.4/.5a/.6 · **A6g.1** (audit baseline 2026-04-21) · **A6g.2 1ª rodada** (T1.a/b/c + T2.a/b, 2026-04-21/22 — T3 scripts com goldens fica como A6g.2b pós-A6c.3) · **A6g.5** (tests sweep, 2026-04-21) · **A6g.7** (Go prep + ADR-113, 2026-04-22) · **A6e.3** (3 slices: FamilyMember + Category + Goal use cases, 2026-04-21) · **A6e.3b** (3 slices: ConfigBlob + Task + Document, 25 use cases + 61 testes, 2026-04-22) · **A6e.5** (`/api/v1/` + aliases + ADR-108, 2026-04-22) · **A6e.events** (domain events infra + 2 agregados migrados + ADR-115, 32 testes, 2026-04-22) · **A6g.4** (rodadas 1+2+3, T2 `ts_long_files` frontend zerado, 2026-04-22) · **A6g.6** (enforcement automatizado — Ruff + ESLint + pre-commit hooks + AST tests + audit baseline decrescente, ADR-114, 2026-04-22) · **A6g.2c** (rename `pipeline/llm/service.py` → `litellm_client.py` — ALLOWLIST `forbidden-names` zerada, 2026-04-22).
...
- **Restante:** A6e (.4 routers finos 🚧 ativo 9/14 fase 4a + AST · **.events-migration** ☐ — migrar ~14 `audit_log()` inline em routers para evento · **.events-followup** ☐ — ativar flag notif em prod + remover cron) · A6g.2b T3 scripts com goldens ⏸ blocked-by A6c.3 · A6g.3 🚧 parcial (P4+P8+P1 top-4 services feitos; P5 Decimal + invitation_service restante) · A6g.3b P5 Decimal migration (wire-breaking, coord frontend) ☐ · A6g.6b sweep (ruff --fix I001/F541 + ruff format + max-lines warn→error) ☐ destravada por A6g.6 · A6-human gate humano · A6c blocked-by A6-human · F7 (7A-7F + LGPD).
...
- **Lanes abertas agora (2026-04-22 — sync pós-A6e.3c merge):** **☐ livres:** A6e.4 slices 4+ (fase 4a faltam 5 routers); A6e.4 fase 4b (config/documents/tasks destravados por A6e.3b ✅); A6e.events-migration (~14 audit inline → eventos, destravada pela A6e.events ✅ — **sem prompt**); A6g.3 backend sweep (prompt `track_a6g3_backend_style_sweep.md` adicionado em `0963fa6`; destravada porque A6e.4 slices 1-7 ✅); A6g.6b sweep pós-enforcement (ruff --fix I001/F541 tocando 356 arquivos + ruff format nos 422 pendentes + promove max-lines warn→error — **sem prompt**; destravada por A6g.6 ✅); A6-human smoke (gate humano). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
...
| **A6g.3** backend sweep (parcial) | `a6g3-backend-style` | [track_a6g3_backend_style_sweep.md](agent_prompts/track_a6g3_backend_style_sweep.md) — Services, repos, helpers, typing | A6e.4 ✅ | 3 | 🚧 parcial 2026-04-22 — P4 optional defaults (5 → 0), P8 what-comments (2 → 0), P1 decomp em 4 services: `pipeline_adapter` (5→2), `goal_service` (4→3), `task_service` (4→1), `task_progress_service` (3→0). **P5 float money (12) deferido** — migração Decimal é wire-breaking change (JSON string); abrir lane dedicada com frontend/codegen sync. `invitation_service` + restante P1 ficam para continuação |
...
| A6g.3 | **Backend Python** (`backend/app/`) — integra com A6e (nomes, DTOs, routers finos). A6e.4 (routers ≤50 linhas) é o chute maior; A6g.3 cobre restante (services, repos, helpers, typing). Prompt: `docs/agent_prompts/track_a6g3_backend_style_sweep.md`. **1ª rodada 2026-04-22:** P4 optional defaults 5→0, P8 what-comments 2→0, P1 decomp em 4 services top (`pipeline_adapter` 5→2, `goal_service` 4→3, `task_service` 4→1, `task_progress_service` 3→0). **P5 float money (12) deferido** como A6g.3b — migração `float` → `Decimal` muda JSON wire format (Pydantic serializa Decimal como string); abrir lane dedicada com frontend/codegen sync. `invitation_service` (P1×3 longos) + `content_classifier` + repos ficam para continuação | 2 sessões por rodada (2 rodadas) | 🚧 parcial — 1ª rodada ✅ 2026-04-22; continuação ☐ |
...
| A6g.6b | Follow-up A6g.6: sweep ruff `--fix I001/F541` + `ruff format .` + promove `max-lines`/`max-lines-per-function` warn→error | Auto-fix em ~356 arquivos (imports) + reformata ~422 (ruff-format) + remove ignores; elevação de gate | 1 sessão | ☐ destravada por A6g.6 ✅ |
