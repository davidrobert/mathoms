...
- **Restante:** A6e (.4 routers finos 🚧 ativo 9/14 fase 4a + AST · **.events-migration** ☐ — migrar ~14 `audit_log()` inline em routers para evento · **.events-followup** ☐ — ativar flag notif em prod + remover cron) · A6g.2b T3 scripts com goldens ⏸ blocked-by A6c.3 · A6g.3 🚧 parcial rodada 3 (P1 em `content_classifier`/`pipeline_service`/`models/task.py`/repositories) · **A6g.3b** P5 Decimal money migration (13 ofensores → 0, wire-compat via MoneyBRL) ☐ **prompt pronto** — [track_a6g3b_decimal_money_migration.md](agent_prompts/track_a6g3b_decimal_money_migration.md) · A6-human gate humano · A6c blocked-by A6-human · F7 (7A-7F + LGPD).
...
| **A6g.6b** sweep ruff `--fix I001/F541` + `ruff format` + `max-lines` warn→error | `a6g6b-ruff-sweep` | Sweep pós-enforcement: (a) `ruff check --fix .` aplica I001 (unsorted-imports) + F541 (f-string sem placeholder) — reformata ~356 arquivos; (b) `ruff format .` aplica em ~422 arquivos pendentes; (c) ESLint `max-lines`/`max-lines-per-function` passa de `warn` → `error`. Remove ignores correspondentes de `pyproject.toml [tool.ruff.lint]`. Baseline decresce, gate se torna bloqueante. **Sem prompt** — mecânico, 1 sessão | A6g.6 ✅ | 3 | ☐ aberta |
...
| **A6g.3b** migração `float` → `Decimal` money (wire-compat) | `a6g3b-decimal-money` | [track_a6g3b_decimal_money_migration.md](agent_prompts/track_a6g3b_decimal_money_migration.md) — Elimina P5_float_money em `backend/app/` (13 ofensores em 2026-04-22). Cria `backend/app/schemas/money.py` com tipo `MoneyBRL`/`MoneyUSD` = `Annotated[Decimal, BeforeValidator, PlainSerializer(float, when_used='json')]` — Decimal em memória, number no JSON, zero wire break para frontend. Migra 7 campos goal + 4 transactions + refatora math em `goal_service.py` (fórmulas IF, aporte, dolar) + `task_progress_service.py` + `transaction_service.py`. **6 slices:** S0 prep tolerance rename (false positives) · S1 tipo MoneyBRL/USD + test · S2 goal DTOs + service math · S3 transactions DTOs + services · S4 OpenAPI snapshot + frontend sanity · S5 docs. **Deferido de A6g.3** porque requer refactor cascata de fórmulas Decimal (quantize, `Decimal(str(pct))` conversions) + OpenAPI diff revisado + frontend smoke | A6g.3 ✅ rodadas 1+2 · A6g.6 gate ativo | 3+ | ☐ aberta — **prompt completo pronto** |
...
| A6g.3b | **Money Decimal migration** (follow-up A6g.3) — elimina `P5_float_money` (13 ofensores em `backend/app/`) via tipo `MoneyBRL`/`MoneyUSD` = `Annotated[Decimal, BeforeValidator, PlainSerializer(float, when_used='json')]`. Decimal em memória para precisão, number no JSON para wire-compat com frontend. Prompt: `docs/agent_prompts/track_a6g3b_decimal_money_migration.md`. **6 slices:** S0 prep tolerance rename · S1 tipo MoneyBRL/USD + test · S2 goal DTOs (7 campos) + `goal_service` math (Decimal quantize, `Decimal(str(pct))` conversions) · S3 transactions DTOs (4 campos) + `transaction_service` + `task_progress_service` · S4 OpenAPI snapshot + frontend sanity · S5 docs. Deferido de A6g.3 porque exige refactor cascata de fórmulas (compute_if_derived, compute_aporte_derived, compute_dolar_derived, _pmt_constante_ate_fv) + diff OpenAPI revisado (request schemas ganham `anyOf [number, string]`) + frontend E2E re-cert | 1 sessão dedicada (~2.5h) | ☐ prompt pronto |
