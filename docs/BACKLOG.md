...
**Status global (2026-04-22 — pós-merge A6e.3b + A6e.events + A6e.events-migration + A6e.4 slices 1-7 + A6g.6 + A6g.2c + A6g.6b):**
...
- **Lanes abertas agora (2026-04-22 — sync pós-A6e.events-migration merge):** **☐ livres:** A6e.4 fase 4a (faltam 7 routers: pipeline, workspaces, reports, transactions, llm, invitations, ws); A6g.3 backend sweep rodada 3 (`content_classifier` 621l, `pipeline_service` P1×4, `models/task.py`, repositories — prompt `track_a6g3_backend_style_sweep.md`, 🚧 ativo em `friendly-elion-088f27`); **A6g.3b** migração Decimal money 🚧 polish pendente — sessão 2 (goal DTOs + math) ✅ 2026-04-22; restam S0/S4/S5 (tolerance rename, frontend sanity, baseline + ADR-090 nota); A6-human smoke (gate humano). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
