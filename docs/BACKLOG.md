...
| **Report Premium UI v2** (guarda-chuva — 20 sub-lanes em 5 ondas) | `report-v2-*` (sub-slug por lane) | Meta-prompt + paralelização: [track_report_v2.md](agent_prompts/track_report_v2.md). **Onda A 2026-04-26:** ✅ v2.1 (`cbb389a`) · ✅ v2.3 (`4aebe50`) · ✅ v2.2 parcial (`0558ea3` — 28/48 baselines + 2 fixes CI: `a856e0b`, `02216f8`). Resíduo: **v2.2b** (Tático+USA baselines, P1 ≤2h). **Onda E (charts UX) aberta 2026-04-26 — 3/8 entregues em main:** ✅ v2.E.1 (`da841c2` PeriodToggle + hook) · ✅ v2.E.2 (`8ee4bd6` TS types datasets) · ✅ v2.E.7 (`55f00fa`+`22ca7d0`+`334f5f7`+`529cd70` ScoreCard plug + DTO + backend; absorve v2.5). Demais 8 sub-lanes finalizam migração Recharts→Chart.js + slide window 12m em "Receita vs Despesa Mês a Mês". Prompt dedicado: [track_report_v2_charts_ux.md](agent_prompts/track_report_v2_charts_ux.md). **Pickup hoje (E.3-E.6 destravados):** até 4 agentes paralelos (v2.E.3 FluxoMensal · v2.E.4 ReceitaBar · v2.E.5 DespesasDoughnut · v2.E.6 ReceitaDespesaMensal); atenção ao hotspot `_shared.ts`/`_registry.ts` · v2.D.1+v2.8 destravados ([prompt dedicado fundido](agent_prompts/track_report_v2_changelog_engine.md)) · Onda B: v2.4 ([prompt dedicado](agent_prompts/track_report_v2_t2_aportes.md)) · v2.6 · Onda C: v2.7 · v2.9 · v2.10. Tabela detalhada + ondas + pickup protocol em [§Report Premium UI v2](#report-premium-ui--paridade-com-exemplo_de_relatoriohtml). | v1 ✅ (10 fases + 3 lanes residuais) | P0/P1/P2 (mistura por lane) | 🚧 **Onda A 3/3 (com débito v2.2b) · E 3/8 entregues, 4 destravadas** · Ondas B/C/D abertas para pickup paralelo |
...
| dev.8 | **Smoke local prod-mode end-to-end**: `docker compose -f docker-compose.prod.yml up`, registrar user, login, upload PDF, trigger pipeline, ver relatório | ✅ sim | 7A.11 (fatia) | 30min | ⏸ Onda 4 — bloqueado em pré-condição (portas 8000/3000/6379 ocupadas pelo `make dev-up` do usuário) |
...
Onda v2.E — charts UX (paridade visual final, paralelizável até 5 agentes)
   ...
   v2.E.8  Re-baseline visual + cleanup imports Recharts + ADR-13X (≤4h) ← TODAS as anteriores ✅
...
| **v2.4** T2 Aportes seção real (substituir stub) | §3.2 auditoria | `report-v2-t2-aportes` | [track_report_v2_t2_aportes.md](agent_prompts/track_report_v2_t2_aportes.md) | v1 ✅ · idealmente após v2.5 | B | R/O (1-5d) | P1 | ☐ aberta |
...
| **v2.6** cards/ legacy: deprecate ou migrar | §3.6 auditoria | `report-v2-cards-cleanup` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | B | R (1d) | P2 | ☐ aberta |
| **v2.7** DnD real Kanban (`@dnd-kit/core`) | débito #1 BACKLOG | `report-v2-kanban-dnd` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | C | R (1-2d) | P2 | ☐ aberta |
| **v2.D.1** SnapshotChangelogBuilder (enabler) | enabler de v2.8 | `report-v2-changelog-engine` | [track_report_v2_changelog_engine.md](agent_prompts/track_report_v2_changelog_engine.md) | v2.1 ✅ | D | O (3-5d) | P2 | ☐ bloqueada por v2.1 |
| **v2.8** ativar comparisons/changelog YAML + render | débito #3 BACKLOG / §3.1 | `report-v2-comparisons-changelog-on` | [track_report_v2_changelog_engine.md](agent_prompts/track_report_v2_changelog_engine.md) (mesmo PR que v2.D.1) | v2.1 ✅ + v2.D.1 ✅ | D | R (1-2d) | P2 | ☐ bloqueada por v2.1 + v2.D.1 |
| **v2.9** LLM-driven section_summaries em E5 | débito #2 BACKLOG | `report-v2-llm-summaries` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ + ADR-13X nova | C | O (3-5d) | P2 | ☐ aberta (requer ADR) |
| **v2.10** PDF visual diff em Playwright | §4.3 auditoria | `report-v2-pdf-visual-diff` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | C | R (1-2d) | P2 | ☐ aberta |
| **v2.2b** baselines Tático+USA (resíduo v2.2) | spillover de v2.2 | `report-v2-visual-baselines-tatico-usa` | inline (ver detalhes em v2.2 status) | v2.2 ✅ parcial | A | S (≤2h) | P1 | ☐ aberta — `clickMode()` em `frontend/tests/e2e/reports/sections.snapshots.visual.spec.ts:77-83` retorna `false` silenciosamente para `/Tático/i` e `/USA\|EUA/i`, pulando 20 testes. Fix esperado: ajustar seletor (provavelmente `getByRole("button")` não match — alternar para `getByText` ou `[data-mode-toggle]`). Após fix, re-run `gh workflow run CI -f run_visual=true -f update_visual_baselines=true`, baixar artefato `report-visual-baselines-generated`, copiar para `frontend/tests/e2e/reports/sections.snapshots.visual.spec.ts-snapshots/`, commit + push em main. |
...
| **v2.E.3** FluxoMensal Recharts→Chart.js + PeriodToggle | finalização migração ADR-117 Fase 7 | `report-v2-fluxo-mensal-chartjs` | [track_report_v2_charts_ux.md §3 v2.E.3](agent_prompts/track_report_v2_charts_ux.md) | v2.E.1 ✅ | E | R (1d) | P1 | ☐ **destravada — pickup imediato** |
| **v2.E.4** ReceitaBar Recharts→Chart.js + PeriodToggle (séries mensais) | finalização migração ADR-117 Fase 7 | `report-v2-receita-bar-chartjs` | [track_report_v2_charts_ux.md §3 v2.E.4](agent_prompts/track_report_v2_charts_ux.md) | v2.E.1 ✅ + v2.E.2 ✅ | E | R (1d) | P1 | ☐ **destravada — pickup imediato** |
| **v2.E.5** DespesasDoughnut Recharts→Chart.js + datalabels + PeriodToggle | finalização migração ADR-117 Fase 7 | `report-v2-despesas-doughnut-chartjs` | [track_report_v2_charts_ux.md §3 v2.E.5](agent_prompts/track_report_v2_charts_ux.md) | v2.E.1 ✅ + v2.E.2 ✅ | E | R (1d) | P1 | ☐ **destravada — pickup imediato** |
| **v2.E.6** ReceitaDespesaMensal Recharts→Chart.js + slide window 12m + tooltip stack + legenda agrupada | paridade `EXEMPLO_DE_RELATORIO.html:1794-1806` + script :7756-7939 | `report-v2-receita-despesa-chartjs` | [track_report_v2_charts_ux.md §3 v2.E.6](agent_prompts/track_report_v2_charts_ux.md) | v2.E.2 ✅ | E | R/O (1-2d) | P1 | ☐ **destravada — pickup imediato** (mais densa das 4 paralelas) |
...
| **v2.E.8** re-baseline visual + cleanup imports Recharts + ADR-13X | fechamento da onda | `report-v2-charts-rebaseline` | [track_report_v2_charts_ux.md §3 v2.E.8](agent_prompts/track_report_v2_charts_ux.md) | v2.E.3-7 ✅ TODAS | E | S (≤4h) | P0 | ☐ bloqueada até E.3-E.7 mergeadas em main |
...
- **Onda B aberta:** ~~v2.5~~ (absorvida em v2.E.7) · v2.6 (`report-v2-cards-cleanup`, P2, 1d) ·
  v2.4 (`report-v2-t2-aportes`, P1, R/O 1-5d, prompt dedicado).
- **Onda D destravada parcialmente:** v2.D.1 + v2.8 (prompt dedicado fundido) destravados por v2.1 ✅.
  Pode rodar em paralelo com Onda B.
- **Onda C aberta:** v2.7 · v2.9 · v2.10 (paraleláveis com B/D).
- **Onda E — 3/8 entregues 2026-04-26 (executados em paralelo por 3 agentes simultâneos), 4/8 destravadas, 1/8 final:**
  - ✅ **Entregues:** v2.E.1 (`da841c2`) · v2.E.2 (`8ee4bd6`) · v2.E.7 (`55f00fa`+`22ca7d0`+`334f5f7`+`529cd70`, absorve v2.5).
  - ☐ **Pickup imediato — até 4 agentes paralelos:** v2.E.3 FluxoMensal · v2.E.4 ReceitaBar · v2.E.5 DespesasDoughnut · v2.E.6 ReceitaDespesaMensal. **Atenção ao hotspot `_shared.ts`/`_registry.ts`** (protocolo CLAUDE.md §Hotspots — anunciar + edit+commit+push em ≤5min).
  - ☐ **Notas-chave para os 4 charts:**
    1. Backend hoje só emite `{label, data}` em `receita_datasets`/`despesa_datasets` — **enriquecimento client-side** (paleta `tokens.json` para `backgroundColor`, `stack` derivado do array de origem em E.6).
    2. Specs Vitest vão em **`frontend/tests/components/report/`** (não `src/.../__tests__/`) — config exige.
    3. `PeriodToggle` exportado em `@/components/report/ui/index.ts`; `usePeriodWindow` em `@/components/report/hooks/usePeriodWindow.ts`.
    4. `ChartSeries` (DTO/wire) em `frontend/src/types/chart-series.ts` — não confundir com `primitives/types.ts::ChartSeries` (UI).
  - ☐ **Último, sequencial:** v2.E.8 (re-baseline visual + cleanup + ADR-13X) só após E.3-E.6 ✅ em main.
