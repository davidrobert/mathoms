...
>
> **Última atualização:** 2026-04-22 (A6e.4 slice 10 ✅ — documents router thin 770→447 + 5 composites extraídos; **fase 4b 3/3 fechada**; THIN_ROUTERS sobe de 11 → 12)
...
**Status global (2026-04-22 — pós-merge A6e.3b + A6e.events + A6e.4 slices 1-7 + A6g.6 + A6g.2c + A6g.6b):**
- **Entregues ✅:** A5a-A5f · A6a · A6b · A6b.5 · A6c · A6d (2026-04-20) · **A6f.1** (pipeline-as-service HTTP boundary, ADR-112, 2026-04-21) · A6f.2/.3/.4/.5a/.6 · **A6g.1** (audit baseline 2026-04-21) · **A6g.2 1ª rodada** (T1.a/b/c + T2.a/b, 2026-04-21/22 — T3 scripts com goldens fica como A6g.2b pós-A6c.3) · **A6g.5** (tests sweep, 2026-04-21) · **A6g.7** (Go prep + ADR-113, 2026-04-22) · **A6e.3** (3 slices: FamilyMember + Category + Goal use cases, 2026-04-21) · **A6e.3b** (3 slices: ConfigBlob + Task + Document, 25 use cases + 61 testes, 2026-04-22) · **A6e.5** (`/api/v1/` + aliases + ADR-108, 2026-04-22) · **A6e.events** (domain events infra + 2 agregados migrados + ADR-115, 32 testes, 2026-04-22) · **A6g.4** (rodadas 1+2+3, T2 `ts_long_files` frontend zerado, 2026-04-22) · **A6g.6** (enforcement automatizado — Ruff + ESLint + pre-commit hooks + AST tests + audit baseline decrescente, ADR-114, 2026-04-22) · **A6g.2c** (rename `pipeline/llm/service.py` → `litellm_client.py` — ALLOWLIST `forbidden-names` zerada, 2026-04-22) · **A6g.6b** (sweep ruff I001/F541 + ruff format . + promove `max-lines` warn→error; `max-lines-per-function` diferido, 2026-04-22).
...
- **Restante:** A6e (.4 routers finos 🚧 ativo 9/14 fase 4a + AST · **.events-migration** ☐ — migrar ~14 `audit_log()` inline em routers para evento · **.events-followup** ☐ — ativar flag notif em prod + remover cron) · A6g.2b T3 scripts com goldens ⏸ blocked-by A6c.3 · A6g.3 🚧 parcial rodada 3 (P1 em `content_classifier`/`pipeline_service`/`models/task.py`/repositories) · **A6g.3b** P5 Decimal money migration 🚧 parcial (sessão 1 ✅ — tipo MoneyBRL + transactions; sessão 2 pendente — goal DTOs + math) · A6-human gate humano · A6c blocked-by A6-human · F7 (7A-7F + LGPD).
- **Caminho crítico (serial):** A6e.4 (finalizar 7 routers fase 4a + 3 fase 4b) → A6g.3 → F7A → F7B → F7D+dogfood → GA. A6-human smoke destrava A6c + A6g.2b em paralelo.
- **Lanes abertas agora (2026-04-22 — sync pós-A6e.4 slice 10 merge, fase 4b ✅):** **☐ livres:** A6e.4 fase 4a (faltam 7 routers: pipeline, workspaces, reports, transactions, llm, invitations, ws); A6e.events-migration (~14 audit inline → eventos, destravada pela A6e.events ✅ — **sem prompt**); A6g.3 backend sweep rodada 3 (`content_classifier` 621l, `pipeline_service` P1×4, `models/task.py`, repositories — prompt `track_a6g3_backend_style_sweep.md`, 🚧 ativo em `friendly-elion-088f27`); **A6g.3b** migração Decimal money 🚧 sessão 2 pendente — goal DTOs (7 campos) + `goal_service` math; prompt em `track_a6g3b_decimal_money_migration.md` §Slice 2; A6-human smoke (gate humano). **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
...
| **A6e.4** routers finos | `a6e4-thin-routers` | [track_a6e4_thin_routers.md](agent_prompts/track_a6e4_thin_routers.md) — 4900→≤1200 linhas (17 routers); **fase 4a** 9/14 thin (6 entregues por esta lane + 3 já eram thin via A6e.3/dashboard); **fase 4b** ✅ 3/3 (2026-04-22); teste AST enforcement em `backend/tests/architecture/test_routers_thin.py`. **Nota histórica:** 5 commits de 2026-04-20 com tag `(A6e.4)` referem ao **ConfigBlob per-aggregate slice** (`f48f06b`, `f2b0319`, `840b74c`, `eaa6370`, `1d7562f`), não a esta lane transversal — mesma colisão histórica que levou a renomear A6e.6 → A6e.events; aqui mantivemos `A6e.4` alinhado com ADR-101 R16. Filtre `git log --grep "A6e.4 slice"` para ver só esta lane. | A6e.3 ✅ (4a) / A6e.3b ✅ (4b) | 2 | 🚧 parcial — **9/14 thin em fase 4a**: goals (`9608058`), audit (`579aabc`), AST enforcer (`3327986`); **+4 em 2026-04-22 sessão noturna**: feature_flags 68→47 (slice 4), auth 72→48 + `AuthenticationError` → 401 global (slice 5), vault 80→61 (slice 6), notifications 110→66 (slice 7); **fase 4b ✅ 3/3 em 2026-04-22**: config 464→417 (slice 8, `d6cd3b3`), tasks 487→509 (slice 9, `09bcc9c`), documents 770→447 (slice 10, `4f13b4a`). THIN_ROUTERS set atual (12): `{audit, auth, categories, config, dashboard, documents, family_members, feature_flags, goals, notifications, tasks, vault}`. **Pendentes:** 7 routers fase 4a (pipeline, workspaces, reports, transactions, llm, invitations, ws). |
...
| **A6e.events-migration** migrar `audit_log()` inline → eventos | `a6e-events-migration` | Migrar ~14 call-sites de `audit_log(db, ...)` inline em routers para `AuditLogEvent` (padrão estabelecido em A6e.events slice 2; `CreateFamilyMember` já migrado). Cada call-site vira emit de `AuditLogEvent` no use case correspondente + handler grava `AuditLog` na mesma transação. **Sem prompt** — escopo mecânico slice-por-router | A6e.events ✅ destravou | 2 | ☐ aberta |
...
| **A6g.3b** migração `float` → `Decimal` money (wire-compat) | `a6g3b-decimal-money` | [track_a6g3b_decimal_money_migration.md](agent_prompts/track_a6g3b_decimal_money_migration.md) — Elimina P5_float_money em `backend/app/` via tipo `MoneyBRL`/`MoneyUSD` (Decimal em memória, number no JSON via PlainSerializer — zero wire break). | A6g.3 ✅ rodadas 1+2 · A6g.6 gate ativo | 3+ | 🚧 parcial 2026-04-22 — **slices 1 + 3 ✅:** tipo `MoneyBRL`/`MoneyUSD` em `backend/app/schemas/money.py` + 11 tests; transactions 4 campos migrados + cascata em `transaction_service` (Decimal filter/sum) + `task_progress_service` (`_parse_brl_target` → Decimal, percent via float cast, `executed_brl.quantize`) + `TaskProgressResponse` DTO + 19 tests. OpenAPI snapshot **zero diff** (serialization mode emite `type: number`). **Pendentes:** S0 tolerance rename, **S2 goal DTOs (7 campos) + refactor Decimal math em `goal_service.py`** (sessão dedicada), S4/S5 finishing |
| **F7F-Local** console interno pré-produção (IA-0) | `f7f-local` | [track_f7f_local.md](agent_prompts/track_f7f_local.md) — UI web Next separada em `frontend-ops/` (bind `127.0.0.1:3100`, flag `INTERNAL_OPS_UI_ENABLED`) + camada de serviço em `backend/app/services/internal_ops/` + auth yaml+bcrypt+JWT cookie + anonimização default ([ADR-116](DECISIONS.md#adr-116)). 4 slices: S1 services+auth backend · S2 frontend-ops shell · S3 telas por área (7F.10–7F.14) · S4 CLI secundário 7F.9 (opcional) | — (greenfield, independente de 7A/B/C) | 3 (Lane C6) | ☐ aberta — **prompt completo pronto** |
...
| A6e.events-migration | Migrar ~14 `audit_log()` inline em routers → `AuditLogEvent` | Padrão estabelecido em A6e.events slice 2 (`CreateFamilyMember` migrado); cada call-site vira emit no use case + handler | 1 sessão | ☐ destravada por A6e.events ✅ |
...
| A6g.3b | **Money Decimal migration** (follow-up A6g.3) — elimina `P5_float_money` via tipo `MoneyBRL`/`MoneyUSD` = `Annotated[Decimal, BeforeValidator, PlainSerializer(float, when_used='json')]`. Decimal em memória para precisão, number no JSON para wire-compat com frontend. Prompt: `docs/agent_prompts/track_a6g3b_decimal_money_migration.md`. **1ª sessão 2026-04-22 — slices 1+3 ✅:** tipo `MoneyBRL`/`MoneyUSD` + 11 tests; 4 campos transactions migrados + cascata `transaction_service` (filter/sum Decimal) + `task_progress_service` (`_parse_brl_target` → Decimal, `compute_progress` percent via float cast) + `TaskProgressResponse` DTO + 19 tests; OpenAPI snapshot **zero diff** (serialization mode emite `type: number`). **Pendentes para sessão 2:** S0 tolerance rename, **S2 goal DTOs (7 campos: aporte/dolar/if_goal) + refactor Decimal math em `goal_service.py`** (`_pmt_constante_ate_fv`, `_if_meta_targets`, `_aporte_cobrindo_gap_com_patrimonio`, `compute_if_derived`, `compute_aporte_derived`, `compute_dolar_derived` — todas precisam migrar para Decimal arithmetic com `.quantize(Decimal("0.01"))` + `Decimal(str(pct))` conversions). | 2 sessões dedicadas (~1.5h cada) | 🚧 parcial — sessão 1 ✅ 2026-04-22; sessão 2 (goal DTOs + math) ☐ |
