...
>
> **Última atualização:** 2026-04-26 (Report Premium UI v2 **Onda A 2/3 fechada** — v2.1 `cbb389a` 12 placeholders comparisons/changelog no YAML + schema/codegen; v2.3 `4aebe50` decisão (b) refinada: S5/S6 → U1/U2 USA documentado em PLAN §17.5; v2.2 ⚠ bloqueada por billing GitHub Actions. v2.D.1+v2.8 destravados pela v2.1 ✅. Os 3 agentes rodaram em worktrees isolados em paralelo conforme [track_report_v2.md](agent_prompts/track_report_v2.md). · 2026-04-25 v2 roadmap aberto — auditoria pós-v1 catalogou 3 inconsistências, 3 débitos declarados e 3 lacunas em 11 sub-lanes paralelizáveis em 4 ondas. · lane `report-v1-polish` ✅ 6/6 fechada — Report Premium UI v1 oficialmente entregue. · lane `adr-129-e6-kill` ✅ 6/6 fechada — renderer HTML server-side erradicado. · Report Premium UI — 10/10 fases úteis v1 entregues em main: F0 discovery → F10 apêndices. **Fases 11/12/13 canceladas via [ADR-129](DECISIONS.md#adr-129--descontinuação-completa-do-renderer-html-server-side)**. · F7F-Local MVP fechado · A6e.4 ✅ fase 4a completa 14/14.)
...
| **Report Premium UI v2** (guarda-chuva — 11 sub-lanes em 4 ondas) | `report-v2-*` (sub-slug por lane) | Meta-prompt + paralelização: [track_report_v2.md](agent_prompts/track_report_v2.md). **Onda A 2026-04-26:** ✅ v2.1 (`cbb389a`) · ⚠ v2.2 bloqueada (billing CI) · ✅ v2.3 (`4aebe50`). **Pickup hoje:** v2.D.1+v2.8 destravados ([prompt dedicado fundido](agent_prompts/track_report_v2_changelog_engine.md)) · Onda B aberta: v2.4 ([prompt dedicado](agent_prompts/track_report_v2_t2_aportes.md)) · v2.5 · v2.6 · Onda C aberta: v2.7 · v2.9 · v2.10. Tabela detalhada + ondas + pickup protocol em [§Report Premium UI v2](#report-premium-ui--paridade-com-exemplo_de_relatoriohtml). | v1 ✅ (10 fases + 3 lanes residuais) | P0/P1/P2 (mistura por lane) | 🚧 **Onda A 2/3 fechadas** (v2.2 aguarda billing GitHub Actions) · Ondas B/C/D abertas para pickup paralelo |
...
| **v2.2** trigger baselines visuais Linux + commit | §3.5 auditoria | `report-v2-visual-baselines` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | `report-a11y-finalize` ✅ + **GitHub Actions billing ativo** | A | S (≤4h) | P0 | ⚠ **bloqueada 2026-04-26** — workflow disparado com sucesso (`gh workflow run CI -f run_visual=true`, run [`24950981242`](https://github.com/davidrobert/financial-report/actions/runs/24950981242)) mas todos os 13 jobs abortaram em ≤17s com annotation: *"The job was not started because recent account payments have failed or your spending limit needs to be increased."* **Ação humana:** resolver billing em https://github.com/settings/billing, então redisparar `gh workflow run CI --ref main -f run_visual=true -f run_e2e=false` e seguir do passo 2 do prompt v2.2. Sem commits gerados. |
...
| **v2.5** score top-level no DTO | §3.4 auditoria | `report-v2-score-dto` | inline em [track_report_v2.md §3](agent_prompts/track_report_v2.md) | v1 ✅ | B | S (≤4h) | P2 | ☐ aberta |
...
**Pickup hoje (atualizado 2026-04-26):**
- ✅ **Onda A: 2/3 fechadas + 1 bloqueada** — v2.1 ✅ + v2.3 ✅ executadas em paralelo via 3 agentes 2026-04-26;
  v2.2 ⚠ aguarda billing GitHub Actions ser resolvido (sem código a entregar até lá).
- **Onda B aberta:** v2.5 (`report-v2-score-dto`, P2, ≤4h) · v2.6 (`report-v2-cards-cleanup`, P2, 1d) ·
...
**Saída do v2:** considerada ✅ quando todas as 11 sub-lanes (v2.1 a
v2.10 + v2.D.1) estão ✅ em `main` ou movidas para v3 com ADR
justificativa. CHANGELOG receberá entrada consolidada análoga à da v1.
