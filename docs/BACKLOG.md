...
| **A8.2** IRPF full schema (E1.6 — pipeline + analyzer + E5 wire) | `irpf-full-schema` | [track_irpf_full_schema.md](agent_prompts/track_irpf_full_schema.md) — destrava 6 KPIs do relatório premium (renda anual líquida, alíquota dual, capacidade PGBL, split trabalho×capital). [ADR-157](DECISIONS.md#adr-157--schema-irpf-completo-stage-extract_irpf_full). | A7 ✅ | 🚧 pipeline + tests ✅ entregues 2026-04-30 (ADR-157 + schema + prompt + validator + stage + analyzer + E5 wire + 22 tests); **frontend (componentes do relatório premium + YAML codegen) fica em sub-lane `irpf-full-schema-ui` com G4 (product-designer) sign-off separado** |
