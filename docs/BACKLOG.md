...
- **Restante:** A6e (.3 use cases 🚧 parcial · .3b ConfigBlob+Document+Task ☐ · .4 routers finos ☐ 4a pickable · .6 events ⏸) · A6g.2 🚧 / .3 / .6 ☐ (sweeps + enforcement) · F7 (7A-7F + LGPD). **A6e.5 ✅ mergeada** (ADR-108, 2026-04-22); **A6g.4 ✅ fechada** (rodada 3 A6g.4c, 2026-04-22) — todas as páginas `>500 l` decompostas, T2 zerado. **A6g.7 ✅** (ADR-113, 2026-04-22).
- **Caminho crítico (serial):** A6e.3 (use cases) → A6e.4 (routers finos) → F7A → F7B → F7D+dogfood → GA.
- **Lanes abertas agora (2026-04-22 — sync pós-A6g.4c merge):** Onda 1: A6g.2 🚧 ocupada; A6g.4 ✅ fechada (T2 zerado em `frontend/src/`); Onda 2: A6e.3 🚧; **livres com prompt pronto:** A6e.3b (destravada por A6f.1 ✅), A6e.4 (fase 4a), A6-human (gate manual). A6e.6 ⏸ preferencial pós-A6e.3. A6g.7 Go prep ✅ 2026-04-22 (ADR-113). Ver tabela e diagrama abaixo. **Sempre confirme com `git worktree list` + `git for-each-ref refs/remotes/origin/agent/`** antes de pegar — a tabela pode estar desatualizada.
