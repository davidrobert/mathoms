...
| **ADR-129 remoção E6 + endpoints HTML** | `adr-129-e6-kill` | Execução da [ADR-129](DECISIONS.md#adr-129--descontinuação-completa-do-renderer-html-server-side). Sub-fatias sequenciais: **1/6** ✅ backend API + use cases + modelo + migration drop `html_path` + `pipeline_task` sem HTML (commit `94f693d` em main 2026-04-24). Bônus absorvido: `seed_existing_reports` + `backend/seed_db.py` simplificados (era fatia 6 — eliminada). **2/6** ✅ pipeline (`pipeline/stages/e6.py` + `pipeline/stage_materialization.py` deletados; stages `E6`/`E6-final` removidos de `STAGE_REGISTRY` + `FULL_ORDER` + `DETERMINISTIC_ORDER` + `STAGE_RENAME_MAP`; `VALID_FROM_STAGES` no schema também; testes ajustados — commit `9f4c616` em main 2026-04-24). **3/6** ✅ scripts (`scripts/e6_render.py` + `scripts/e6/` + `scripts/e6_regen.py` deletados; refs CLI em `scripts/e7_review.py` + `scripts/e_reset.py` atualizadas; tests E6 órfãos em `tests/test_stage_wrappers.py` + `backend/tests/test_golden_pipeline.py` removidos; comentários de `config/report_layout.yaml` atualizados; `dev/code_style_baseline.json` regenerado — commits `7f0363b` → `2b18a29` em main 2026-04-24). **4/6** ☐ frontend dead code (`getReportHtmlUrl*` em `frontend/src/lib/api/reports.ts`, labels E6 em `frontend/src/lib/pipelinePhases.ts` + `frontend/src/app/(app)/reports/[id]/page.tsx`, mock em `frontend/tests/mocks/handlers.ts`). **5/6** ☐ design-tokens (emit CSS standalone em `design-tokens/build.py`). **6/6** ☐ docs finais (`docs/e6_render_readme.md` deletado, `EXEMPLO_DE_RELATORIO.html` mantido como referência visual). | ADR-129 mergeada | P1 | 🚧 fatias 1-3 ✅ · fatia 4 (frontend dead code) próxima |
| **F9 stage rename em bloco** | `f9-stage-rename` | Execução da [ADR-093](DECISIONS.md#adr-093) — rename dos identificadores legados (`E2`, `E3`, `E5`, `E5.N`, `E7-apply`…) para nomes descritivos (`extract_statements`, `reconcile_transactions`, `analyze_finances`, `generate_narratives`, `apply_review`…). Mapa canônico em [`pipeline.stage_spec.STAGE_RENAME_MAP`](../pipeline/stage_spec.py). 7 sub-fatias sequenciais: **9.0** auditoria (`_scratch/audit_stage_references.py` + teste de exhaustividade do mapa); **9.1** `git mv pipeline/stages/e*.py` → descritivos; **9.2** substituir strings literais em Python arquivo-a-arquivo, `pytest` entre cada; **9.3** Alembic migration `q5r6s7t8u9v0_rename_stage_identifiers` (`UPDATE pipeline_artifacts/pipeline_stage_logs SET stage = <new>`) + 5 testes upgrade/downgrade; **9.4** `git mv scripts/e*.py` → descritivos + alias compat `e_reset.py --from X` (1 release); **9.5** guardrail `tests/unit/pipeline/test_no_legacy_stage_names.py` parametrizado, soft-fail default + hard-fail via `MATHOMS_ENFORCE_STAGE_RENAME=1`; **9.6** remover `MaterializationBridge`, `_init_config()` global, aliases compat. **Pré-deploy:** backup obrigatório + `SELECT DISTINCT stage` confirma mapa exaustivo + comunicar quebra de queries Grafana/Retool externas. Bônus já catalogado: [batch2.15](#docs-reviewbatch2--reescrita-de-documentos-decisões-de-escopo-pendentes) (exemplo prático de query DB antes/depois em ADR-093). | A6c ✅ (bridge removida) · A6d ✅ (Caminho B puro) · janela de manutenção combinada | F9 | ☐ aberta — **prompt a escrever** |
