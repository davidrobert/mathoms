...
| LC8-02 — o guard de escrita do E4 resolve por **stage**, nunca por `artifact_key`: os 7 baldes batem contra um `oneOf` de 5 ramos, um deles **placeholder** (`required: ['status']`) que um balde transacional casaria — medido, `{status}` valida. E há instância viva: o `patrimonio` do run (15 chaves, 87 itens) **reprova em `$`** e é gravado sob `warn`; a jusante `_non_ledger_verdict` imprime *coberto · 0 itens* para ele | contrato | Alto | P1 | procede (novo · o cético **ampliou** o enunciado: era risco hipotético, a medição achou instância viva) | procede-aberto | [[A42.l19]] |
