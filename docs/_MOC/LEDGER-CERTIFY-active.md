...
| LC05 — **[skill]** `_non_ledger_verdict` (`dev/ledger_certify_core.py:170`, era `:161`) é catch-all com **default `coberto`**: conta containers que esses baldes não usam (`patrimonio` usa `itens`=67, não `dados`; `fluxo_mensal_detalhado` usa `meses_ordenados`=44) ⇒ imprime "0 itens · coberto" com o payload completo em mãos, e a glosa "fora do grão transacional" é **factualmente falsa** para `fluxo_mensal_detalhado` (sai de `result.cash_flow`, mesma população classificada). Carimba `coberto` sobre a dimensão de **62,5% do peso do score** | conservação (skill) | Alto | P1 | procede | **procede-fechado (2026-09-01)** | **partido em dois donos:** a [[A42.l19]] entregou o registry por chave com default `não-verificável` (#1871); a [[A42.l3]] entregou a **glosa** (#1946) — o registry virou `{balde → NonLedgerChecker}`, com a proveniência **daquele** balde ao lado de onde contar, e `fluxo_mensal_detalhado` deixa de afirmar origem E2/baseline. O guard de classe mede a **saída**: a 1ª versão comparava as glosas do dict e sobrevivia à mutação que restaurava a frase única |
| LC06 — **[skill]** a rubrica declara a P0 nº 1 (dedup patrimonial [[ADR-271]]/[[ADR-246]]) coberta, mas o check que roda (`investment_double_count`) varre o balde E4 `investimentos` (18 pos, origem E2) — **população e vetor diferentes** de `investimentos_consolidados` do E1.5c (49 entradas, eixos cross-year/cross-declarante). P0 nº 1 **nunca exercitada** em r1–r4 | conservação (skill) | Alto | P1 | procede | **procede-fechado (2026-09-01)** | [[A42.l3]] #1951 — os 6 invariantes de saída em `dev/ledger_baseline_invariants.py`, sobre o consolidado que **já viajava** dentro do balde `patrimonio` (`build_patrimonio_artifact` devolve o baseline normalizado inteiro): nenhuma plumbing nova. A partição de julgabilidade viaja com cada veredito, e **PAT-1 declara não-julgabilidade quando o corpus tem 1 ano só** — aí max-ano e Σ-anos coincidem e ele seria `P ∨ ¬P`, o modo de falha que a [[A42.l16]] mediu no CV18 |
...
| LC5-02 — `layer_ok` sai verde com PONTO CEGO impresso a duas linhas: `paridade_fecha` é auto-identidade (partição do próprio conjunto do colapsador), e `sem_ponto_cego` existe **fora** do predicado agregado (`ledger_collapse_layer.py:68-70`, `:90-99`) | contrato | Alto | P0 | procede (novo) | **procede-fechado (2026-09-01)** | [[A42.l3]] #1946 — `sem_ponto_cego` entra no predicado. Medido antes: `paridade_fecha` fica `True` mesmo com um detector que **não compartilha nenhuma chave** com o colapsador (`so_no_detector` sobe a 3 e ela não se move) — a docstring dizia *externa*, e era essa afirmação que autorizava o ponto cego a ficar fora do agregado. `alvo_enderecavel` **segue fora**, pelos 4 critérios do `senior-cto` de 2026-08-05 |
| LC5-03 — o checksum por grupo prova auto-consistência do produtor, não conservação E2→E3: `_ledger_verdict` lê três campos escritos pelo mesmo produtor e `carregadas` nunca é confrontado com o input E2. 97/97 grupos saem `conservado` ao lado de "E2→E3: count não fecha" | contrato | Alto | P0 | procede (novo) | **procede-fechado (2026-09-01)** | [[A42.l3]] #1947 — entra a `LedgerAnchor`, o resíduo da perna E2→E3, que cruza **três** produtores (artefatos E2, artefatos E3, log de execução do E3). Resíduo ≠ 0 **ou não medido** teta o grupo em `coberto`; `perda` não é rebaixada. Fecha a classe: o ramo `dups=0 ⇒ valor provável` é auto-consistente igual e também passa pelo teto. **Residual declarado:** a âncora só transfere ao eixo *entregue* com drift zero — o log de exclusões do run pinado não está em mãos (§Deferimento datado da lane) |
...
| LC5-06 — o catch-all `_non_ledger_verdict` carimba `coberto` em balde que não sabe ler: procura três containers e o payload de `patrimonio` e de `fluxo_mensal_detalhado` não tem nenhum ⇒ "0 itens · coberto" com o payload completo em mãos | conservação | Alto | P1 | procede (LC05 do §r4 reproduzido no U1) | **procede-fechado (2026-09-01)** | mesma entrega do `LC05` do §r4 (a linha o reproduz): registry pela [[A42.l19]] #1871, glosa por-balde pela [[A42.l3]] #1946 |
...
| `LC6-05` | **[[A40.l98]]** (aberta 2026-08-30) | re-enunciada acima: são três produtores, não dois · ✅ **entregue** #1865 (`37cff25f`); duas frases da linha caíram na medição — ver a disposição acima |
| `LC6-06` | **[[A40.l98]]** | mesma família de base (aporte/amortização no denominador) · ✅ **entregue** #1865: o produtor da terceira taxa não era campo, era a **derivabilidade** de `fluxo_liquido / receita_total` no exec context do parecer; a base passa a ser declarada nas labels. Remover os três campos segue disponível como opção mais forte, dono `financial-planner` |
| `LC6-07` | **[[A40.l102]]** | re-enunciado acima: é **um** par, D e D+1. Measure-first, sem enforce · **PR-0 entregue** em #1864 (`42ad07dd`): a classe `proximidade_d1` é contável em passada própria, sem alvo por construção, **zero** rows removidas. Segue `procede-aberto` porque a duplicata continua publicada — o que resta é o enforce, gateado por teste positivo por candidato (cadeia de saldo), que depende de o parser C6 emitir o `day_last_saldo` que hoje descarta |
...
>
> **Estabilidade: 37,68% → 42,38%** (pooled `|A∩B|/|A∪B|`, 836 artefatos `E1.5a` / 28 grupos
> K≥5; mediana 29,19% → 31,16%; cardinalidade 9,7 = a do baseline). ⚠️ A 1ª redação deste
> bloco publicou **61,78%** — número comprado com fusão indevida de ativos distintos da mesma
> instituição, corrigido no mesmo dia (ver a tarja na [[A42.l15]] §ENTREGUE). ⚠️ O **23,5%** do enunciado original **não é** o comparável
...
| Código | Dimensão | Severidade | Prioridade | Veredito | Disposição | Trilha |
|---|---|---|---|---|---|---|
| LC9-04 — o **X4** (ancoragem monetária do parecer) é **falso-verde por construção**: de 10 literais monetários, **9** vivem em `riscos[*].ancoras[].valor_renderizado`, campo que o backend preenche (`parecer_orchestrator.py:805` `stamp_ancora_values`) copiando `path → valor` do **mesmo** payload que o check relê ⇒ órfão impossível; a superfície **autoral do modelo** é **n=1** | saúde-harness | Alto | P1 | CONFIRMADO · NOVO — **achado do instrumento contra si mesmo**, e o instrumento é meu | **procede-fechado** ([[A42.l24]]) | publiquei `FECHA ✅ n=10/10` sobre denominador em que 9 de 10 não podiam falhar. **Mesma classe do `LC8-01`** (§r8). **Entregue:** o `veredito()` exige um **terceiro** denominador (`n_falsificavel`, keyword-only), e o X4 separa carimbado de autoral pelo resolvedor do **próprio estampador**, **por ocorrência** — subtrair por valor apagaria a cópia autoral do mesmo número, que é o caso real deste run. Medido: `n_comparado=10 n_esperado=10 n_falsificavel=1 (10% do examinado)`. ⚠️ **o `INAPLICÁVEL` que eu prescrevi não procede** — `n=1` pode reprovar (vive em `riscos[].evidencia`, que nada sobrescreve) e não reprovou; o que protege o leitor é publicar os 10%, não trocar o rótulo |
| LC9-05 — o **X5** (proveniência de execução) examina **17 de 18** stages logados (`n_esperado=17`), e o excluído é `analyze_finances` em `needs_review` — **o stage que constrói o payload que carrega a regressão desta rodada** | saúde-harness | Alto | P1 | CONFIRMADO · NOVO (correção do painel, verificada por mim) | **procede-fechado** ([[A42.l24]]) | o check que eu consertei na [[A42.l21]] para *"poder sair verde"* saía verde **ignorando exatamente o stage sob suspeita**. **Entregue:** `n_esperado` passa a contar **todos** os stages logados (`18/18`, era `17/17`) e a exclusão sai **nomeada no veredito** — `fora do predicado por status: `analyze_finances` (needs_review)` —, nunca no denominador |
| LC9-06 — a **mesma linha** do razão publica o delta E3→E4 com **dois sinais opostos**: o campo sai negativo e o detalhe positivo, porque `dev/ledger_conservation.py:220` calcula `value_in - value_out` só para o texto | consistência | Médio | P1 | CONFIRMADO · NOVO — dois leitores honestos do painel chegaram a conclusões opostas, e **nenhum errou de leitura** | procede-fechado | a direção do viés era **irresolvível a partir da saída**. **fechado** [[A42.l25]] · [[ADR-434]] §D1: `delta_cents` vira produtor único e `DELTA_LABEL` (`Δvalor(destino−origem)`) põe a convenção no rótulo. **E o achado teve vítima medida:** o `LC9-07`, abaixo, nasceu falso por ler o detalhe em vez do campo |
...
> Run `7d860f0b` · executor `04551a0b` · preflight: 4 WARN declarados (o `worker` reprovou com código de 9h antes do HEAD e foi reiniciado **antes** do disparo).
> Instrumento: **moveu** — 6 commits em `dev`/`.claude/skills` desde o executor do `U3`, logo nenhum "idêntico" é publicável aqui (§10 `U3` item 1). Promovido para `dev/unified_xchecks.py` + `dev/_unified_xchecks/`.
> Cru + síntese com valores: `storage/1b9f2cf5-.../reviews/U4-2026-08-30/SINTESE.md` (off-git).
> Escrituração: [[A42.l18]] (`LC8-01`) e [[A42.l19]] (`LC8-02`). Efeito nos gates registrado em `docs/sprint/A42/_README.md` §Gate de saída da sprint e `docs/sprint/A40/_README.md` §Gate de saída e encerramento.
> Cobertura: matriz 7×3 — 13 respondidas · 2 `BLOQUEADA` com o veredito que falta nomeado · 6 sem cobertura com motivo · **zero silenciosas**.
> **Distância ao `main` re-medida no fecho** (§10 `U1` item 12, não só no preflight): o `main` andou **1** commit durante a rodada — `42ad07dd` ([[A40.l102]]), que refatora a máquina de colapso cross-documento. Cruzado por âncora contra os achados: alcança **apenas** o `LC8-04`, marcado `re-verificar contra HEAD`. Nenhum outro achado desta rodada tem âncora no intervalo.

| Código | Dimensão | Severidade | Prioridade | Veredito | Disposição | Trilha |
|---|---|---|---|---|---|---|
| LC8-01 — a perna de **valor** da conservação E3→E4 é **inerte por construção**: `ledger_conservation.py:265` passa `0` **literal** no campo `dups` (a linha 160, E2→E3, passa a variável real) e `ledger_certify_core.py:244` + `ledger_conservation.py:215` somam **ambos** `abs(valor)` sobre a **mesma população pré-dedup** (declarado no docstring) ⇒ `Δvalor = 0` é invariante à inversão de sinal débito↔crédito **e** às 858 rows que o dedup do E4 remove | conservação | Alto | P1 | procede (novo · **verificado no produtor pelo loop**, não inferido) | procede-fechado | **fechado** [[A42.l18]] · PR [#1870](https://github.com/davidrobert/mathoms/pull/1870) (`cfaf3f38`): o produtor passa a **declarar** o valor do destino (`dedup_collapsed_cents`/`transferencias_cents`, [[ADR-426]]) e o harness lê a declaração; `dups` recebe `dedup_collapsed`. A metade **sinal** do achado não é conservável nesta perna (nenhuma tx das fixtures declara `tipo` — 0 de 63 (medido 2026-08-31): a direção é DERIVADA do sinal) — fronteira declarada, não silenciada |
| LC8-02 — o guard de escrita do E4 resolve por **stage**, nunca por `artifact_key`: os 7 baldes batem contra um `oneOf` de 5 ramos, um deles **placeholder** (`required: ['status']`) que um balde transacional casaria — medido, `{status}` valida. E há instância viva: o `patrimonio` do run (15 chaves, 87 itens) **reprova em `$`** e é gravado sob `warn`; a jusante `_non_ledger_verdict` imprime *coberto · 0 itens* para ele | contrato | Alto | P1 | procede (novo · o cético **ampliou** o enunciado: era risco hipotético, a medição achou instância viva) | procede-fechado | **fechado** [[A42.l19]] · PR [#1871](https://github.com/davidrobert/mathoms/pull/1871) (`5504d91c`): o guard resolve por `(stage, artifact_key)` ([[ADR-427]]) e o `patrimonio` herda `baseline_patrimonial.schema.json`, o contrato que já gateia sua fonte E1.5c — 7/7 baldes validam em `strict`. A medição **ampliou o enunciado**: eram **dois** ramos mortos (o de receitas/despesas declarava `periodo` como object; o produtor emite string) e o ramo `{dados}` aceitava **5** baldes sem restringir nada. `_non_ledger_verdict` resolve o contêiner pela chave — o falso *coberto · 0 itens* atingia também o `fluxo_mensal_detalhado` |
| LC8-03 — `_e3_count` soma só `transacoes_total + transacoes_duplicadas_removidas` e **ignora `remocoes`**, embora `_ledger_verdict` — duas funções acima — leia `remocoes`; um canal declarado e reconciliável ao inteiro sai como *count divergente* com **duas causas declaradas que são ambas falsas** | consistência | Médio | P2 | procede (novo; **refuta** a leitura inicial de que o sinal do drift era o oposto do previsto — o harness chama sem `collapse_enforce`, cujo default é `False`, então o lado fresco **mede e não remove**) | procede-fechado | **fechado** [[A42.l20]] · PR [#1907](https://github.com/davidrobert/mathoms/pull/1907): `_e3_count` passa a normalizar por `declared_removed_count` — o **mesmo** produtor da conservação E2→E3, não uma segunda implementação. **Controle A/B** (mesmo DB/ws/run pinado, só o código muda): `count divergente` **4 → 0**, casados **108 → 112**; os 4 déficits somam **907**, o mesmo `X3b`. A divergência era **100% de configuração**, resíduo zero. Nenhum grupo antes casado migrou para divergente (108 + 4 = 112) — risco medido, não suposto. A lista de causas ganhou a **terceira** (config do harness ≠ config do run) nos 4 sítios. ⚠️ A citação *"duas funções acima no mesmo arquivo"* envelheceu: a [[A42.l19]] extraiu `_ledger_verdict` para `dev/ledger_unit_verdicts.py` depois desta medição |
| LC8-04 — **condicionada:** `collapse_retention.removals_publicadas` = 47 onde o canal `cross_document_collapse` dos artefatos E3 do mesmo run declara 907, com `colapsaveis` = 394 no meio | consistência | — | — | **não se afirma defeito**: os três contadores podem nomear **estágios distintos** da mesma operação, e `collapse_precondition.liberado` é `false` neste run (`clausulas_reprovadas: ["vivacidade=4/5"]`) | condicionada · **re-verificar contra HEAD** | medição que resolve: declarar por igualdade de conjunto qual população cada contador mede · gatilho: primeiro run com `liberado: true`. ⚠️ **A âncora moveu durante a rodada:** o `42ad07dd` ([[A40.l102]], mergeado enquanto a `U4` corria) refatorou `cross_document_collapser.py`, acrescentou `cross_document_keys.py` e `cross_document_proximity.py`, e **modificou** (não acrescentou) `cross_document_collapse_types.py`, que é pré-existente, além de tocar `test_collapse_retention_travas.py` — os três contadores desta linha vêm de um run executado contra `04551a0b`, **antes** disso |
| LC8-05 — sonda LC06: `investimentos_consolidados` **63 → 63** entre `U3` e `U4` — o **primeiro par estável** da série 61→60→63→63; `imoveis` 7 (D2=2) e `veiculos` 7 (D2=7) seguem `não-verificável` por fail-closed | identidade/dedup | — | — | MEDIÇÃO-DE-CONHECIDO de `LC6-02` — e o quarto ponto dela: 3 dos 4 ids da pausa são byte-idênticos aos do `U3`, mas `instituicao_ausente` **migrou de id** pela segunda rodada consecutiva | procede-aberto | [[A42.l15]] · **ver §Disposição do `LC6-02` (2026-09-01)** — a [[A42.l15]] shipou (#1939) e deixou de ser gatilho vivo |

**Verificação dos fechados do `U3`** (pelo `git log f0ac69a2..04551a0b`, **não** pelo MOC — §10 `U2` item 5): os **3** consertos seguram. `LC7-01` ([[A42.l17]]) segura **para o que declarou** — a lane refuta, no próprio corpo, que rotear compre determinismo. O que a `U4` acrescenta: a única alegação positiva dela, *"reduz variância"*, tem agora a primeira medição e **não mostra redução** — o churn de descrição na unidade que alimenta a chave natural foi **4·2·1·4 de 8** pré-fix e **5 de 8** pós-fix, acima de toda a faixa anterior. Com `n=1` não se afirma piora; afirma-se **ausência de redução detectável**, e que o churn de identidade que motivou a lane está intacto.
