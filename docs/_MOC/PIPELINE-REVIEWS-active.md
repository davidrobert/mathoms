...
| RV4-17 | Perna de volume do gate anti-regressão é morta: `compare_reviews.py:154` busca folha `transacoes_total` que não existe no view-model E5, `_sum_leaf` devolve `None` e o guard `if b and …` (`:232`) torna o check inalcançável; 3 dos 10 campos de `run_health` do snapshot durável são null | saúde-execução | Médio | P2 | S | Baixo | PARTIAL | **procede-fechado (2026-09-01)** | [[A42.l3]] #1949 — a perna foi **removida** e a disposição confirmada: varri o schema E5 e não existe folha de volume no view-model (o único `transacoes` inteiro é `fluxo_caixa.provisionado.transacoes`, o resíduo pós-corte). Ela parecia viva em teste porque a **fixture fabricava a folha**. Fecha a classe: `_RUN_HEALTH_REQUERIDO` + `_RUN_HEALTH_INFORMATIVO` por **igualdade de conjunto** contra o produtor, e perna sem insumo vira `INDETERMINADO` com **exit 3**. Achado colateral: 2 dos 9 campos de `run_health` **nunca tiveram consumidor**, contra o comentário do produtor que afirmava o oposto |
| RV4-18 | `StageLogTail.emit` descarta todo `extra` do log record (`observability/logger.py:170-177`) e 6 módulos logam fora do namespace capturado — os WARNINGs de drift de schema chegam ao registro durável como eventos idênticos sem `validation_path`/`validator_keyword`/`occurrence_count` | saúde-execução | Médio | P2 | S | Baixo | CONFIRMED | **procede-fechado (2026-09-01)** | [[A42.l3]] #1949 — `StageLogTail.emit` passa a reusar `_extra_fields`, o **mesmo** redator do formatter (segunda cópia da denylist divergiria no primeiro campo sensível novo). Teste de que dois drifts de paths distintos deixam de ser o mesmo evento, e de que CPF no `extra` não vaza para o `output_summary` |
...
| RV4-45 | Auditoria de paridade FinOps é fail-open sem a env var (SQLite cria arquivo vazio → "tabela ausente" → exit 0) e compara dois sinks alimentados pelo **mesmo** hook — "não consegui medir" é indistinguível de "medi e passou" | saúde-execução | Baixo | P3 | S | Baixo | PARTIAL | **procede-fechado (2026-09-01)** | [[A42.l3]] #1949 — os dois ramos (env var ausente; tabela ausente no banco apontado) saem com **exit 3**, e o 0 fica reservado a paridade medida. A saída de sucesso passa a declarar o que a paridade prova: os dois sinks vêm do **mesmo hook**, logo é consistência de escrita, não custo correto |
...
| RV5-10 — `compare_reviews` HARD em folha → 0 sem cruzar conservação irmã (CV2 passou) | saúde-execução | Médio | P2 | procede | **re-escopado → `RV6-02`** | **roteamento corrigido 2026-09-01** (closeout da [[A42.l3]]): a `Disposição` apontava para a [[A42.l3]], mas o §r6 já havia re-escopado o item dizendo em letra que *"[[A42.l3]] não é sobre isso e [[A42.l4]] exclui cross-stage"*. O item real é a conservação irmã intra-artefato — `RV6-02` (P0), owner senior-cto+data-engineer, plano [[PLAN-deterministic-authority]]. A l3 shipou (#1949) e a rota teria virado zumbi |
...
| PV9-13 — `folga_mensal` remove 100% dos gastos pontuais e a prescrição de teto **ancora nela**; duas sobras publicadas na mesma janela divergem em ~19 pp e a maior é a que prescreve | consistência | Alto | P1 | procede (novo) | **procede-fechado** ([[A40.l94]] · #1828 `05561dc0` · [[ADR-422]]) | invariante proposto: `\|folga_mensal − taxa_poupança × receita recorrente\| ≤ ε` — ~~falha hoje~~ **entregue e verde** (`test_folga_mensal_nao_soma_pontual_realizado`) |
...
| PV9-27 — `schema_validation_drift` em 6 de 18 stages, todos passando em modo `warn` | contrato | Médio | P2 | PARCIAL (a [[ADR-409]] já decidiu a fila e **rejeita** o flip global; os 6 colapsam em 4 schemas já classificados) | procede-aberto | sobra **uma** linha nova: o schema do E5 estava em "reavaliar após 1 run novo" e **este é o run novo** — o drift persiste. **2026-08-31 — o E4 ficou PARCIALMENTE elegível** ([[A42.l19]] · [#1871](https://github.com/davidrobert/mathoms/pull/1871) · [[ADR-427]]): o guard passou a resolver por `(stage, artifact_key)`, o que removeu o bloqueio do §D (*`oneOf` colapsa o path em `$`*). **Correção da própria linha, mesmo dia:** eu havia escrito *"os 7/7 baldes validam em strict, re-medido na main"* — era sobre a **fixture golden**, e o go/no-go da §B é sobre `pipeline_artifacts`. Medido no corpus (71 runs, 98 dias): `e4_cashflow` 142/0 e `e4_investimentos` 71/0 são **GO**; `e4_seguros` (5 payloads) e `e4_pontos_milhas` (**1** payload em 71 cópias) têm massa insuficiente pelo §D; `e4_fluxo_mensal` era NO-GO por `minItems` e foi consertado em [#1894](https://github.com/davidrobert/mathoms/pull/1894); `patrimonio` resolve para `baseline_patrimonial`, **fora da fila** pelo §F. **São 2 elegíveis, não 7.** Decidido promover os 2 (arbitragem `senior-cto`); pré-requisitos mergeados em #1894; falta o log de startup do worker e o flip. `owner: data-engineer` |
...
| PV9-30 — `StageSpec.writes` é contrato **consumido e sabidamente falso**, e a falsidade está numa docstring em vez de num gate: um stage declara escrever um artefato que nunca existe, e o validador de ordenação valida contra ele | contrato | ~~Alto~~ **Médio** | ~~P1~~ **P2** | procede (novo) · **re-escalado 2026-08-31 pela medição** — ver [[A42.l23]] | procede-aberto | tornar `writes` verdadeiro e gatear: `writes ≠ ∅ ⇒ artefato emitido`. ~~**É o X5 corrigido**~~ **2026-08-31 — o remédio prescrito aqui está REFUTADO** ([[A42.l21]] · [#1906](https://github.com/davidrobert/mathoms/pull/1906)): o X5 foi corrigido fazendo o **oposto** — parou de usar `writes` como discriminador. `writes ≠ ∅ ⇒ artefato emitido` reprovaria 4 stages benignos por run (2 que declaram zero trabalho, 1 que escreve sob outra key por desenho, 1 read-only), e o `PV10-10` já mediu que o campo é ornamental. A perna de produto — **tornar `writes` verdadeiro** — segue aberta e é o que resta desta linha; o gate sobre ela, não. **Medido 2026-08-31 — o núcleo do achado PROCEDE e estava sem gatilho** (§Convenção item 3): `validate_full_order` compara **declaração com declaração** e nada ancora nenhum lado na realidade. Um stage novo declarando `reads` sobre `generate_narratives` (declaração falsa) **PASSA**, exatamente como sobre `analyze_finances` (controle verdadeiro) — o validador não distingue. **Mas o raio de explosão hoje é zero:** mutação de `writes` nos 18 stages mostra **8 load-bearing** (a espinha: `extract_baseline` → `consolidate_baseline`; `extract_invoices`/`_statements`/`_with_llm` → `reconcile` → `categorize` → `analyze_finances`) e **10 inertes** — e as 2 declarações falsas estão nos inertes. Daí `Médio`/`P2`: o dano está a **um `reads=` de distância**, não presente. Remédio re-escrito na [[A42.l23]] (ancorar a declaração na fonte do módulo despachado, reusando o cross-check do `X5`), com a ambiguidade de fundo nomeada: `writes` conflaciona *ordenação* com *fato de I/O*, e declarar a verdade de I/O de `generate_narratives` quebraria a ordenação. |
...
| PV10-10 — `StageSpec.writes` declara alvo que 2 stages não escrevem (`generate_narratives` escreve sob a chave de **outro** stage; `validate_cross` tem **zero** call-site de `write`) | contrato | Baixo | P3 | **re-enunciado + rebaixado** — `PV9-30` dizia 3 ofensores; são **2, em 2 classes**, e `extract_with_llm` **não é ofensor** (fallback condicional legítimo, com writer real) | procede-aberto | ~~**CAI a perna que dava peso:** mutação com `writes=()` e com `writes=("xpto_inexistente",)` **passa em `validate_full_order` nos dois casos** ⇒ o campo é **ornamental**, não load-bearing.~~ ⚠️ **REFUTADO em 2026-08-31 ([[A42.l23]]):** a mutação foi aplicada a `generate_narratives` e `validate_cross` — os **dois** stages de **cauda**, que ninguém lê e onde o campo é inerte **por construção**. Repetida nos 18 stages, ela **REPROVA em 8** (`extract_baseline`, `consolidate_baseline`, `extract_invoices`, `extract_statements`, `extract_with_llm`, `reconcile_transactions`, `categorize_transactions`, `analyze_finances`). O campo é load-bearing em **44%** dos stages; a conclusão *"ornamental"* foi generalizada da amostra que não podia refutá-la. O que continua verdade é o **fato** desta linha — as 2 declarações falsas — e que elas caem no complemento inerte. O docstring do próprio `StageSpec` já o chama de ficção. ~~⚠️ **resíduo de instrumento:** o predicado do X5 vai reprovar `extract_with_llm` em **todo run sem fallback** — classe de falso-positivo conhecida e não codificada~~ → **codificada em 2026-08-31** ([[A42.l21]] · [#1906](https://github.com/davidrobert/mathoms/pull/1906)). Medido: a classe era **maior** que o previsto aqui — o stage tem **duas** saídas de zero trabalho (`skipped: true` e `total_processed: 0`) e reprovava em 5 de 25 runs recentes pela segunda, que o enunciado não citava |
...
> ⚠️ **Correção 2026-09-01 (PR #1962 `c551e832` · [[ADR-433]] `Decidido`).** As duas cadeias
> **não são independentes**, e a origem não é a não-determinação do LLM: `RR9-01` e `RR9-02`
...
| Código | Dimensão | Severidade | Prioridade | Veredito | Disposição | Trilha |
|---|---|---|---|---|---|---|
| PV13-01 — o **papel vira identidade**: `titular_key_normalizer.py:49,56` devolve o **`raw`** quando nenhum membro compartilha token, então o literal de papel passa como `titular_key` válido (**3 de 58** itens) e vira chave de cunhagem em `property_identity_enricher.py:38-52` | consistência | Crítico | P1 | CONFIRMADO · NOVO — **um defeito com três sintomas**, não três achados | procede-aberto | a lente de razão unificou os 3 sinais que a F3.a declarou "em sentido contrário": raiz no normalizador ⇒ `property_id` nulo ⇒ `investimentos_classes_analyzer.py:266` **falha aberto** e `nao_classificado_pct` sobe **sem o numerador crescer**. Pelo §6 registra-se no **produtor**. Dona [[A40.l113]]; interação com a [[A40.l96]] **declarada na lane em 2026-09-01** (#1962; a declaração não existia até então) · ⚠️ **medido 2026-09-01:** a cadeia `titular_key` cru ⇒ `property_id` nulo **não procede** — em `imoveis_consolidados` os literais de papel são **0 de 9**, e os 8 sem id falharam por `domain.property_identity_uncanonical`. Os 3 literais vivem em `investimentos_consolidados` e são **exatamente** as 3 posições de 2026 que causaram o defeito da [[A40.l113]] — o fallback pode parar de disparar neste corpus e ficar latente |
| PV13-08 — o parecer publica **dois valores para a mesma grandeza** na mesma seção, com **4,15 pp** de spread (um autoral do modelo, um carimbado pela máquina), e nada reconcilia porque `parecer_prose_money.py:16` deixa **percentual fora** do invariante — medido: **27 percentuais em 47 campos de prosa, 0 sob invariante** | consistência | Alto | P1 | CONFIRMADO · NOVO | **procede-fechado** ([[A40.l120]] · [#2001](https://github.com/davidrobert/mathoms/pull/2001) · 2026-09-02) | dona [[A40.l120]] · ⚠️ **re-atribuída e enunciado refutado no closeout da [[A40.l117]] (2026-09-02):** a dona era a [[A40.l117]] e passa à [[A40.l120]], desmembrada por arbitragem do `senior-cto` em 2026-09-01. **A causa escrita nesta linha não procede:** `Caixa = 0` neste corpus, logo a base não explica nada — `90,25 + 4,14 = 94,39` é o carimbo ao centésimo e o spread de 4,15 pp **é a linha Previdência**. A causa é `alocacao_alvo_deviation.py:17` dobrar toda previdência em renda fixa sem declarar no rótulo, agravada por `$.goals` só ser projetado na seção que **foi evictada** (o modelo nunca recebeu o número). E `number_in_prose` (`parecer_prose_money.py:16`) é **telemetria sem comparador**: pôr percentual "no invariante" não pegaria o spread. O enunciado fica como está. · ✅ **FECHADO no closeout da [[A40.l120]] (2026-09-02):** o comparável do alvo passa a ser **projetado** — medido, `renda_fixa_atual_pct` chega ao exec context de um run real, na seção mantida. O rótulo do KPI passa a declarar que previdência está inclusa, e nasce gate de coerência com tolerância = **meio passo da precisão escrita** (nenhum limiar novo). Veredito de domínio: previdência **permanece** agregada, pelo critério de rebalanceabilidade da [[ADR-141]] — classificar pelo subjacente foi rejeitado por **falta de insumo medida** (o schema do informe não tem campo de fundo). ⚠️ Segue aberto e roteado NA LANE: o guarda de flip do `next_aporte_classe`, que muda cálculo e exige ADR própria. |
| PV13-09 — o guardrail de autocontradição erra a seção **pela terceira posição**: `parecer_guardrails_divida.py:165` fixa a seção num literal e o modelo rotula outra ⇒ o parecer **elogia** e **alerta** sobre o mesmo objeto com `autocontradicao_removidos: 0` | consistência | Alto | P1 | CONFIRMADO · **reincidência medida** da [[A40.l80]] (#1800) | **procede-fechado** ([[A40.l116]] · #1969) | **o achado novo é o teste**: `tests/test_parecer_guardrails_divida.py` **importa a constante** e monta a fixture com ela ⇒ **invariante ao valor**, não pode falhar — medido, o HEAD passava 23/23 com a constante mutada para `"S9"`. ⚠️ **o remédio deste registro estava errado**: dizia "derivar do layout em vez do literal", e derivar do layout dá exatamente `S1` — o valor que a [[A40.l116]] mediu em **0 de 14 runs** (o modelo dá `S3` em 9, `S4` em 5). `section_id` é rótulo re-sorteado por run, não propriedade do objeto: nenhum literal serve, e o match saiu para `tema_canonico` + o sinal do E5. Dona [[A40.l116]] |
| PV13-10 — `tool_iterations: 19` contra `max_tool_iterations: 6` — teto estourado **3,2×** sem alarme | saúde-harness | Médio | P2 | CONFIRMADO · NOVO | **procede-fechado** ([[A42.l24]]) | emissor e teto medem **populações distintas**: as 19 são 10 carimbos de métrica + 9 de âncora, **zero** iniciadas pelo modelo. **Entregue:** `X7` novo mede o teto contra a população que o emissor conta, com oráculo por **assinatura** (`LLMService.call` não aceita `tools` ⇒ round-trip estruturalmente 0 ⇒ `INAPLICÁVEL`, e isso é resultado); e o `maximum: 6` de `_meta.tool_iterations` — que anotava o campo do **emissor** — sai do schema, conformando a semântica que a [[ADR-341]] já decidira (OBS-1 · A37.l1). **Continua aberto o gate:** `SCHEMA_BY_STAGE` não valida `review_finances_holistic` (`RV4-23`, dona [[A42.l6]]) |
| PV13-11 — o prompt do parecer **se contradiz sobre `tools`**: a linha 441 do YAML convida `get_e5_section` e a **179 do mesmo arquivo** afirma que o modelo não tem ferramentas; o convite morto é injetado no corpo **sob budget** | contrato | Médio | P1 | CONFIRMADO · NOVO — causa proximal dos 3 `campos_faltantes` | **procede-fechado** ([[A40.l117]] · [#1966](https://github.com/davidrobert/mathoms/pull/1966) · 2026-09-01) | ⚠️ **auto-refutação registrada**: eu havia afirmado que `get_e5_section` era alcançável, inferindo de docstring. Medido: `call()` não tem parâmetro `tools`, **zero** call-sites não-teste, e as 19 chamadas são todas pós-LLM. Rebaixei meu próprio achado de Alto para Baixo. Dona [[A40.l117]] · ⚠️ **corrigido no closeout da [[A40.l117]] (2026-09-02): três afirmações desta linha não reproduzem.** (a) eram **8** sítios model-facing, não 2 — **6 prometem, 2 pressupõem** —, e a **persona** (`config/agents/planner_persona.md:136,176,198,207`) é o original de que o system prompt é cópia; a linha 441 do YAML é a menos importante das oito. (b) **"sob budget" é falso**: `backend/app/services/parecer_distiller.py:467-470` concatena `hints` **depois** de `_fit_body_to_budget`, e o único convite dentro do corpo orçado era o `_eviction_marker` (`:246-255`), removido pelo #1966. (c) o veredito diz *"causa proximal dos 3 `campos_faltantes`"* e são **8** — o 3 é o subconjunto em seção evictada, e cortar a promessa **não prevê** que o contador caia: 4 dos 8 são `$.endividamento.dividas[N].taxa_juros_aa`, campo que existe no E5 e tem **zero** ocorrências no manifest, que é a [[ADR-206]] funcionando. O enunciado e o veredito ficam como estão — são evidência do que foi escrito (precedente `RV8-32`). |
| PV13-12 — a **cobertura de lineage é medida contra a FIXTURE**, não contra a produção: o gate fixa **14** raízes monetárias do dogfood e o payload real tem **18** ⇒ 4 raízes ficam fora do universo, e a cobertura publicada sai **36%** contra **28%** reais | contrato | Médio | P1 | CONFIRMADO · NOVO — viés **otimista**, e cresce sozinho · **composição re-medida** | procede-fechado ✅ [[A27.l3]] (2026-09-01) | a [[A27.l2]] (#1872) shipou o gate **com controle positivo**, e o mecanismo funciona: o defeito era no **sujeito da medição**, a mesma distinção que a [[A42.l14]] estabeleceu para conservação. **Medido: os quatro números reproduzem (14 · 18 · 35,7% · fixture ⊂ produção), e a composição do 18 não** — `narrativas` entrava no denominador por `pontos_revisao`, que é `sum(1 for f in flags if f["needs_review"])`, contagem lida como R$ pelo monetário-por-default. As três reais são `previdencia_pgbl`, `real_estate` e `tributario`, que a fixture nunca emite (sem IRPF, imóvel locado nem PJ) ⇒ a cobertura real é **29,4%**, não 28%. Entregue: universo vira **roster de origens** com o número publicado sobre a união, CLI que reprova raiz-fora-do-roster sobre payload de produção, e contrafactual que **reprova o desenho anterior**. O achado roteado (`tributario` emitido e não declarado no `e5_analysis.schema.json`) foi **fechado** no [#1967](https://github.com/davidrobert/mathoms/pull/1967) — varredura completa: era a **única** raiz emitida e não declarada, em 52 de 54 artefatos. ⚠️ **closeout 2026-09-02 — o conserto tem dois furos da mesma classe que ele fecha:** `_is_monetary_leaf` exige `int|float` e o E5 serializa parte do dinheiro como string decimal (`irpf_kpis` e `protecao_patrimonial` ficam fora do denominador) ⇒ **29,4% é teto, piso 26,3%**; e `Roster.observing` não é monotônico — re-observar o mesmo rótulo com payload mais pobre encolhe o universo e **sobe** o número, medido nos dois sentidos com a suíte verde. **Fechados no mesmo dia**, com o contrato E5 somado como origem `schema` do roster: publicado **5/20 = 25,0%**, e os 3 consertos falsificados por mutação. [[A27.l3]] §Fecho |
| PV13-13 — o guard de escrita do E4 passa com **0 erros** e **não mede profundidade**: `required`/`additionalProperties:false` só na raiz, item de `imoveis_consolidados` com `required` nulo ⇒ item vazio, campo lixo e num→str atravessam | contrato | Alto | P1 | CONFIRMADO · NOVO — limite revelado pela medição do conserto da [[A42.l19]] | **ENTREGUE** #1968 | **a regressão de identidade do `RR9-01` passou por este guard**, e não por acidente de configuração: ele não olha o grão do item, que é onde a identidade mora. Dona [[A42.l26]]  · **ENTREGUE** em [#1968](https://github.com/davidrobert/mathoms/pull/1968) (`e3f595a7`, [[ADR-436]]). **A medição refutou parte do enunciado desta linha:** num→str em chave que **casa** `^(31_12_)?\d{4}$` **já reprovava** por tipo antes do conserto — o vetor vivo era a chave que **não** casa, porque `valores_31_12` não declarava `additionalProperties`. E o enunciado subdimensionava: eram **8** casos, não 3 — item vazio nas **três** coleções, mais três em `patrimonio_por_ano` (objeto-do-ano vazio, lixo dentro dele, chave não-ano) |
| PV13-14 — as citações do parecer levam à **seção errada** em 4 de 11 riscos (2 de proteção citam a de imóveis; a de sucessão cita a de renda do IRPF) | clareza-ux | Médio | P1 | PARCIAL — ⚠️ **a metade "âncora morta" foi REFUTADA**: as **27 citações resolvem, 27 de 27** | **procede-fechado** ([[A40.l117]] · [#1996](https://github.com/davidrobert/mathoms/pull/1996) · 2026-09-02) | a lente afirmou "≥13 erradas + demais texto morto"; medi as duas metades separadamente e a severidade caiu. Dona [[A40.l117]] · ⚠️ **estado atualizado no closeout da [[A40.l117]] (2026-09-02):** a **decisão está tomada** — a máquina autora o `section_id`, e o mapa deriva do layout via `card_id` (arbitragem do `senior-cto`, 2026-09-01). Não entrou no #1966 por **três precondições** nomeadas na lane: (1) admitir `S_PROTECAO` restrito a `metrica`/`ponto_forte`; (2) declarar `Diagnóstico de dados` → **S2** (hoje `S_parecer` publica auto-ponteiro); (3) resolver a divergência de **ordem** da cascata (tema-primeiro × âncora-primeiro), que exige rodada nova com agente que não tenha lido a lane. Dona segue [[A40.l117]]. · ✅ **FECHADO no closeout da [[A40.l120]] (2026-09-02):** as três precondições caíram e o destino passou a ser **derivado pela máquina** ([[ADR-438]]). No corpus real os 2 riscos de proteção migram S4→S9, a S9 sai de **0 para 4** riscos e o terminal `S_parecer` fica em **0**. |
| PV13-15 — ETA subdeclarada **reproduz**: 22% neste run, e o mecanismo intacto no extremo — um stage com mediana de **28 ms** sobre todas as execuções contra **161.449 ms** sobre as reais (**5.766×**) | clareza-ux | Médio | P2 | **PARCIAL** — a metade do **extremo** é de discriminador distinto (medido); a metade *"22% neste run"* **não foi medida** e o registro não guarda o método dela | procede-aberto | ⚠️ **medido na entrega da [[A42.l22]]**: o 5.766× vem de `total_processed > 0`, não da flag `skipped`. O corte da lane move `extract_with_llm` de **28 → 25 ms**, não a 161.449. As 45 linhas do extremo são **no-op silencioso** (rodou, `processed: []`, sem declarar `skipped`) e estão confinadas a `extract_with_llm`, que **não emite ETA** ⇒ latente, sem superfície hoje. É a **2ª grafia de "zero trabalho"** que a [[A42.l21]] já nomeou. **P2 · dono `data-engineer` · [[ADR-342]]** (gate anti-silêncio: "stage rodou e devolveu vazio") — lane própria. ⚠️ O critério **não** é `total_processed`: os dois emissores de ETA usam `files_processed`/`declarations_extracted`, e um gate nessa chave sairia verde sem tocá-los. O predicado é coluna tipada `items_processed`, não chave de JSON |
...
| PV13-18 — `needs_review` segue **fora do vocabulário de desfecho** da superfície: o run termina `completed` com um stage em `needs_review` e nada na UI diz isso | clareza-ux | Médio | P2 | MEDIÇÃO-DE-CONHECIDO — reincide do `U4` | procede-aberto | a [[A40.l84]] cobriu a **entrada** (guard de review pendente), não o **léxico** do desfecho. Ver [[A40.l105]] (`shipped` · #1984) · ⚠️ **Disposição 2026-09-02 (closeout da [[A40.l105]]): o ponteiro acima estava errado — a l105 NÃO fecha nada aqui.** Ela shipou `with_gap` para run que pausou e retomou, e `with_gap` faz o banner **retornar `null`** (`ReportDataQualityBanner.tsx:73`): troca **afirmação falsa** por **silêncio**, e o predicado desta linha é sobre o silêncio. Medido na mesma passada, o léxico do run: `RunContextLine` (`HistoryRow.tsx:27-73`) só lê `paused_at_stage` no ramo de **descartado**, então run `completed` que pausou e retomou devolve `null` também na tela de pipeline. O que a l105 entrega aqui é **substrato** (o fato durável já é lido, na mesma query), não fecho. **Sem dona — precisa de lane própria**, cobrindo as duas superfícies; não aberta no closeout de propósito (P2 `clareza-ux`, não move o gate de saída P0/P1 da [[A40]]), e a decisão de não abrir é registrada aqui para a `U6` não a redescobrir como nova |
...
| Código | Dimensão | Severidade | Prioridade | Veredito | Disposição | Trilha |
|---|---|---|---|---|---|---|
| PV12-01 — a **ETA exibida durante o run** é subdeclarada em até **72%**: `get_median_durations_for_workspace` filtra `status == 'completed'` e mistura no-ops auto-declarados (milissegundos) com execuções reais (minutos) na mediana que vira a previsão. Medido com e sem as linhas de flag: um stage vai de 15.030 para 53.358 ms (**−71,8%**), outro de 322.317 para 382.270 (**−15,7%**) | saúde-execução | Médio | P2 | procede (novo) — `PV9-02` registra que a coluna `status` não reflete o skip; **o consumidor não-interno e a magnitude são novos**, e o cético nomeou a medição de falsificação que foi executada e **não** refutou | procede-**corrigido na entrega** | [[A42.l22]] (`shipped`) — ⚠️ **a magnitude e a superfície não se encontram**: os −71,8% reproduzem só sobre a população INTEIRA; sob a janela de 20 que a produção lê, o erro é **−0,1%** e **0,0%** nos dois únicos stages que emitem ETA (`extract_baseline`, `extract_irpf_full`) — e `extract_comprovantes_bens`, o do headline, **não emite ETA**. O defeito procede por POPULAÇÃO (a ETA só é lida no ramo em que o stage executa), não por magnitude. ⚠️ **Débito aberto pela própria entrega:** o predicado é a 1ª query JSON-path do backend e **contradiz** a decisão escrita da [[A40.l18]] (`degradation_metrics.py:12-17`, *"validada no dialeto errado é falso-verde"*) — a verificação em PG foi manual, e **nenhum job de CI roda `backend/tests/**` contra Postgres**. **P2 · donos `data-engineer` + `sre-devops`**: rodar a suíte da mediana contra o PG que o CI já sobe, OU superseder a A40.l18 formalmente |
| PV12-02 — a **cobertura** de lineage não é medida por gate nenhum e o eval é **auto-referente**: `check_lineage_refs` verifica só existência; os 29 casos cobrem **4 `entry_field` distintos**; a raiz de endividamento está no registro com **zero** caso; `expected_rule_ref` sai do próprio registro e o teste assevera `expected ⊆ registry`. Acrescentar raiz ao E5 sem lineage **não move métrica nenhuma** — e ~87% das raízes declaradas no schema não têm rastro | contrato | Médio | P2 | procede (novo) — é o mesmo modo de falha de *"gate que só pode dar verde"*, aqui na métrica que deveria vigiar a própria cegueira que carimba 88% dos blocos como `sem-veredito` | procede-fechado ✅ [#1872](https://github.com/davidrobert/mathoms/pull/1872) (2026-08-30) — as duas cláusulas caíram: a cobertura passa a ser medida (`dev/lineage_coverage.py` + gate com controle positivo) e o eval declara `expected` por extenso. **Remedido no fechamento, e o enunciado não reproduz:** o `~87%` acima conta prosa e metadado (`narrativas`, `alertas`, `data_analise`) num denominador de **espécie errada** — teto inalcançável, e KR que não chega a 100% ninguém persegue. A cobertura acionável é **5/14 = 35,7%** das raízes que publicam dinheiro. "Endividamento com zero caso" é estreito: são **4 dos 8** `rule_id`. As 9 raízes sem rastro seguem como dívida — agora **contável**, e o gate impede que cresça | [[A27.l2]] ✅ shipped |
| PV12-03 — **aprovação-com-avisos é indistinguível de nunca-ter-pausado**: `_runs_with_degraded_stage` filtra só `status == 'degraded'`, então um run que pausou, foi conferido e retomou sai `complete`. O banner **não** mente neste relatório (os 5 documentos em revisão seguram a barra "sem pendências"), logo o defeito é do **predicado**, não do sintoma de hoje | saúde-execução | Médio | P2 | procede (novo, **rebaixado pelo cético**: o enunciado dizia que o banner afirmava limpo no PDF; a ordem dos guards refuta) | **procede-fechado** ([[A40.l105]] · #1984) | [[A40.l105]] · ⚠️ **o rebaixamento do cético foi REFUTADO por medição 2026-09-02 (#1984)**: a pausa não tem canal nenhum na UI (`computeDataQualitySignals` não a lê, `countActiveSignals` não a conta) ⇒ `runOutcome` é o **único** gate da `CleanBar`, e com zero documentos em revisão a barra renderiza. A contingência era do **workspace do `U4`** (5 docs em revisão), não do mecanismo. Entregue: `_outcome` ganha `paused_at_stage is not None ⇒ with_gap`, na mesma polaridade positiva dos outros dois termos |
| PV12-04 — o `X5` da rodada unificada **só pode sair VERMELHO** e agrega 3 causas sob 1 rótulo: `extract_with_llm` é skip mal-carimbado, `generate_narratives` **escreve** (sob outra key, por desenho de merge documentado) e `validate_cross` é read-only. O conjunto é **constante** em `U2`/`U3`/`U4` ⇒ poder discriminante **zero** | contrato | Médio | P2 | procede (novo) — mesma patologia de `PV9-30`, sinal invertido: o remédio é declarar o conjunto esperado por **igualdade de conjunto**, para que um 4º ofensor apareça. **✅ [#1906](https://github.com/davidrobert/mathoms/pull/1906)** — defeito procede e o enunciado **subestimava em duas frentes**: eram **4** causas, não 3 (`writes=∅` já isentava `unlock_documents`/`route_documents`, sem rótulo próprio), e *"zero trabalho"* tem **duas grafias** — `skipped: true` (8 stages) e `total_processed: 0` (4 stages), que os **mesmos** stages usam conforme a saída que tomam. A 1ª versão do conserto, fiel ao enunciado, reprovava `extract_with_llm` em **5 de 25** runs: é a classe de falso-positivo que o `PV10-10` deu por conhecida e **não codificada**. Histórico completo pós-conserto: **103 `FECHA` · 21 `DIVERGE` · 5 `INAPLICAVEL`** (antes: `DIVERGE 3` constante em todo run) | procede-fechado | [[A42.l21]] |
