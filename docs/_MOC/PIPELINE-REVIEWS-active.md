...
| RV8-04 — `reserva_emergencia.composicao_liquida.investimentos_titular` diverge de `patrimonio.investimentos_titular` por fator >2× no mesmo payload: a reserva absorve a fatia sem dono sob rótulo de membro, e o bloco não tem campo de ressalva | correção | Crítico | P0 | procede (novo) | procede-aberto | terceiro balde `investimentos_sem_titular`; liquidez "Excessiva" não autoriza realocar sob fatia não certificada |
| RV8-05 — manifest do parecer é whitelist e não projeta `investimentos_nao_atribuidos`, `cobertura_investimentos`, `nao_classificado_pct`, `diagnostico_confianca`, `guarda_de_sinal` nem `pl_ressalva`; `_meta.tool_trace` fica em 3 paths de uma só raiz. A maquinaria de incerteza da A40 é inerte no único consumidor que prescreve | qualidade-llm | Crítico | P0 | procede (novo) | procede-aberto | bloco de cobertura no manifest + `narrative_hint` de supressão; promover o drift E5↔manifest de `warn` a `fail` |
| RV8-06 — `cobertura_de_membros` (`investimentos_cobertura.py:207-222`) itera **papéis** e `cobertura_investimentos[].membro` é enum fechado em titular/cônjuge: a fatia órfã não tem célula onde existir e `review_reasons_da_cobertura` só projeta `nao_apurado`. DE-7 do r7 é infixável no vocabulário atual | completude | Crítico | P0 | procede (re-ancorado do DE-7) | procede-aberto | terceira `CoberturaMembro(membro="nao_atribuido")` acima do piso da [[ADR-406]] |
| RV8-07 — catálogo de citação colapsa em 2 raízes por `_PRIORITY_ROOTS` + `max_bytes`: ancorabilidade **zero** sobre o E5 real contra ~39% no snapshot sintético que gateia; e `coverage_failed` é fail-open (só conta âncora emitida que não resolve), devolvendo 0 com a maioria dos itens sem âncora nenhuma | qualidade-llm | Crítico | P0 | procede (re-ancorado do PE-1) | procede-aberto | semear catálogo pelo conjunto visível; `itens_sem_ancora` vira KPI logado e gate do golden mensal |
| RV8-08 — guard de review pendente vive só na camada HTTP (`resume_run.py:21-29`); `pipeline_service.resume_pipeline_run` não consulta `stage_reviews`, e o comentário em `pipeline_task.py:1129` declara o invariante como se fosse global. Runs completam sobre review que ninguém aprovou | saúde-execução | Crítico | P0 | procede (novo) | procede-aberto | mover o predicado para `_flip_run_to_resuming`; `_finalize_run` recusa `completed` com review pendente |
| RV8-09 — `record_review_reasons` tem call-site único dentro do ramo de pausa: stage WARN-first entrega razão no artefato e ela **nunca** chega à tabela. Exatamente os stages projetados para não pausar perdem 100% do diagnóstico | saúde-execução | Crítico | P0 | procede (novo) | remediado — fecha por medição no r9 | [[A40.l81]] `shipped` 2026-08-25 (#1697) · SHA `954f892f` · [[ADR-411]] `Decidido` · **a rota escrita nesta linha era insuficiente**: mover o sink sozinho colhe **zero** no `consolidate_baseline` (o `detail` dele não tem bloco `validation`), e o gate que shipou compara `detail`↔tabela por Σ `occurrence_count` — não artefato↔tabela por contagem · **predicado que o r9 mede**: num run sem pausa a tabela não fica vazia (corpus de dogfood: 10 rows / Σ 46) **e** a perna de diagnóstico do `compare_reviews` deixa de estar cega (exige baseline em snapshot v3) |
