...
| RV4-02 | `_narrate_top5_decisoes` descarta `decisoes[0]`: `_fmt_aporte_head` ocupa "Prioridade 1" incondicionalmente e a fila é enumerada de `[1:5]` — decisão registrada pelo dono não chega à única seção que responde "o que fazer", e duplica quando outra decisão da fila é de aporte (`charts_narrator.py:417-433`) | clareza-ux | Alto | **P0** | M | Médio | CONFIRMED | procede-aberto | product-designer + financial-planner (2 lentes, mesma causa) · render confirmado em `S10SinteseSection.tsx:16-17` · [[A40.l10]] (admitido como item P0, 2026-08-04) |
