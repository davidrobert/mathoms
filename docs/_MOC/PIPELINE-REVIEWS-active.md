...
| PV9-27 — `schema_validation_drift` em 6 de 18 stages, todos passando em modo `warn` | contrato | Médio | P2 | PARCIAL (a [[ADR-409]] já decidiu a fila e **rejeita** o flip global; os 6 colapsam em 4 schemas já classificados) | procede-aberto | sobra **uma** linha nova: o schema do E5 estava em "reavaliar após 1 run novo" e **este é o run novo** — o drift persiste. **2026-08-31 — o E4 saiu da fila por elegibilidade** ([[A42.l19]] · [#1871](https://github.com/davidrobert/mathoms/pull/1871) · [[ADR-427]]): o guard passou a resolver por `(stage, artifact_key)` e os **7/7** baldes validam em `strict` (re-medido na `main` de 2026-08-31). O flip em si **não** foi feito — segue governado pela [[ADR-409]] e sem lane; `owner: data-engineer` |
