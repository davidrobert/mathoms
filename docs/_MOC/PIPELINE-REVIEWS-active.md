...
| PV9-30 — `StageSpec.writes` é contrato **consumido e sabidamente falso**, e a falsidade está numa docstring em vez de num gate: um stage declara escrever um artefato que nunca existe, e o validador de ordenação valida contra ele | contrato | Alto | P1 | procede (novo) | procede-aberto | tornar `writes` verdadeiro e gatear: `writes ≠ ∅ ⇒ artefato emitido`. ~~**É o X5 corrigido**~~ **2026-08-31 — o remédio prescrito aqui está REFUTADO** ([[A42.l21]] · [#1906](https://github.com/davidrobert/mathoms/pull/1906)): o X5 foi corrigido fazendo o **oposto** — parou de usar `writes` como discriminador. `writes ≠ ∅ ⇒ artefato emitido` reprovaria 4 stages benignos por run (2 que declaram zero trabalho, 1 que escreve sob outra key por desenho, 1 read-only), e o `PV10-10` já mediu que o campo é ornamental. A perna de produto — **tornar `writes` verdadeiro** — segue aberta e é o que resta desta linha; o gate sobre ela, não. |
