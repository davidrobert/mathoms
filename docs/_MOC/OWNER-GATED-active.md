...
| 1º `make go-parity` Tier-1 (prova ao vivo da [[A40.l24]]) | Rodar `make go-parity WS=<uuid>` e confirmar: marcador `LLM-FREE: ANTHROPIC_API_KEY scrubbed` nos **dois** braços, 0 row nova em `llm_call_log`, o extrato da Caixa com o **mesmo** tamanho nos dois braços, e — pós-[[ADR-360]] (#1156), que fechou o **bloqueio nº 1** — o controle Py↔Py com **0 diff residual** sem allowlist para o cone de IF | ✅ gate corrigido e mergeado (`9b7d330e`, #1157); prova de mutação em `tests/test_go_parity_llm_free_gate.py`; seed do cone determinístico em `main` | Desbloqueia o gate técnico da F2 (nada mais avança sem este run, por declaração do [[TRACK-f2-cutover]]) | ~40min (3 runs/braço) | [[A40.l24]] |
| Flip do cutover Go (F2) | Autorizar flip global `MATHOMS_PIPELINE_SERVICE_URL` em prod, após gate humano — **e decidir 1 item que o harness não fecha:** pós-flip o shell Go entrega `ANTHROPIC_API_KEY` a todo subprocess de stage, ligando a extração de visão sem gate do `caixa.py` (hoje morta no worker) inclusive em run determinístico, sem budget/log/sanitização. Fechar [[A41.l3]] antes, ou aceitar com o custo declarado | gate técnico em curso; asserção de 0-LLM do Tier-1 corrigida pela [[A40.l24]] — falta a prova ao vivo (linha acima); simetria de credencial do **Tier-2** enforçada em `assert_preconditions` | Início do soak ≥2 semanas → F3 decommission | — | [[PLAN-go-shell]] · [[TRACK-f2-cutover]] §B.3 |
| Flip [[ADR-360]]/[[ADR-361]] + nota de recalibração do cone de IF | (a) flipar as duas `Proposto` → `Decidido`; (b) autorizar **nota one-shot** no 1º relatório pós-merge — seed constante + `n` 50k + censura de percentil deslocam todo o bloco de IF, e sem a nota a leitura racional de "IF em 2040 → 2041" é "meu plano piorou" | ✅ **todo o código em `main`** (#1156, #1158, #1162 `790c1c5f`); gateado por teste | Sprint pode fechar o cone de IF; destrava a prova ao vivo do Tier-1 | ~10min | [[A40]] §Entregas fora de lane |
| Revisão `sre-devops` da política de CI do #1160 | Decidir: passar revisão **retroativa** ou aceitar como está. O CLAUDE.md §Protocolo de delegação lista "Política CI/CD … FinOps" como gatilho **obrigatório** de `sre-devops`; o #1160 mudou política de um job que é required check e gravou na [[ADR-210]] a regra de dimensionamento (~2× da mediana), e mergeou sem essa revisão — a sessão rodava sob instrução de não invocar subagente sem pedido explícito. Risco do diff é baixo (comentário + `--durations=25` + adendo de doc; o teto 12→20 foi do #1157), o que entrou sem especialista é a **regra** | ✅ medição completa no adendo 2026-08-03 da [[ADR-210]] (56 jobs; volume vs regressão separados; sharding rejeitado com a conta) | Fecha a lacuna de processo; se a revisão discordar da regra de ~2×, o número volta à mesa | ~15min | [[A40]] §Infra de CI |
