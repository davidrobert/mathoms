...
| Flip do cutover Go (F2) | Autorizar flip global `MATHOMS_PIPELINE_SERVICE_URL` em prod, após gate humano | gate técnico em curso — **ressalva:** a asserção "0 invocação LLM" do Tier-1 hoje conta artefato de stage `%llm%` e **não vê** chamada de visão bem-sucedida (Caixa); Tier-1 verde ainda **não** prova 0 LLM até [[A40.l24]] entrar | Início do soak ≥2 semanas → F3 decommission | — | [[PLAN-go-shell]] |
