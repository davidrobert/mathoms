...
| Flip do cutover Go (F2) | Autorizar flip global `MATHOMS_PIPELINE_SERVICE_URL` em prod, após gate humano | gate técnico em curso | Início do soak ≥2 semanas → F3 decommission | — | [[PLAN-go-shell]] |
...
| Rotação Fernet em prod | `celery -A backend.app.worker call rotate_fernet_secrets` (dry-run antes); gate fecha com `failed=0` + 2º dry-run com `rotated=0` | task Celery existe (ADR-171) | Pré-condição de G0 — **P0 independente do flip** | ~15min | [[A34.l3]] |
