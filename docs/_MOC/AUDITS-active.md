...
## r10 — `vault-2026-08-24-r10` (scope `all` · mode `comprehensive` · painel 6 lentes)

> Skill audit-vault ([[ADR-302]]) · amostra **rotativa** `--run 10` (NÃO `--full`).
> Gates 7/7 verdes (398 ADRs, 1219 notas, 9611 wikilinks, **0 broken**; `gate_flagged=0`,
> `changed=0` — nenhum finding veio da camada 1). Coletor: **328 candidatos** — 66 arquivos
> (adr 23 · sprint 17 · reference 16 · plan 7 · claude 1 · prompt 1 · root 1) + **262 linhas
> de registro** (universo 869). Lentes: information-architect · senior-cto · data-engineer ·
> financial-planner · prompt-engineer · sre-devops · loop principal. Camada 4: **11 verifies
> re-medidos pelo loop principal** (não cópia da lente) — 11/11 CONFIRMADOS. Bruto:
> `_scratch/audit-vault-2026-08-24-r10.md`.
>
> **Cadência anti-zumbi:** r9 (2026-07-28) fechou 100% — F01-F10 `procede-fechado`/
> `não-acionável` (#1104, #1107, #1110, #1111), F11 POLISH parcial com 7 itens
> `aceito-wontfix` pré-beta. **Zero `procede-aberto` remanescente para re-triar.**
> **Watch do r9 REABERTO:** o cluster ADR-285, declarado "encerrado" no r9, tem **71
> ocorrências vivas em 20 paths** (F21) — o r9 mediu instâncias nomeadas, não o predicado.
> (O **71** é a medição de abertura. A própria célula F21 re-mediu para **56** no
> #1646; o blockquote fica como está — é snapshot datado —, e o número vigente
> mora na célula.)

| Código | Severidade | Veredito | Disposição | Trilha |
|---|---|---|---|---|
| F01 — [[ADR-255]] §Backfill prescreve `backend/app/services/internal_ops/recompute_e4.py --workspace-id <id>`; o real é `dev/recompute_e4_for_run.py`, que exige `--workspace-id` **e** `--run-id` (`:99-101`) + `--apply`. Runbook `recompute_e4_dedup.md:51-52` tem a invocação certa | DOC-BLOCK | procede (verificado V-loop) | procede-fechado | P1 · owner: data-engineer · fix docs-only (alinhar ADR ao runbook) · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): ADR-255 aponta `dev/recompute_e4_for_run.py` com `--workspace-id`+`--run-id`+`--apply` e delega ao runbook |
| F02 — `reference/tenancy.md:300-302` — template "toda rota F8+ tem um teste que…" chama `make_workspace_member(owner)` depois de `make_workspace`, que **já** cria a membership (`builders.py:118-135`), violando `uq_workspace_member` (`workspace_member.py:78`). Probe empírico: `IntegrityError` | DOC-BLOCK | procede (V1) | procede-fechado | P1 · owner: senior-cto · nenhum teste real segue o template · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): template perde a chamada duplicada; comenta `skip_membership` para o caso negativo |
| F03 — `reference/tenancy.md:99-108` rotula "✅ DO" endpoint **sem** `response_model`; o mesmo endpoint real tem (`api/goals.py:127`) e `test_openapi_response_models.py:67-93` **bloqueia merge** sem ele. Checklist §7 (8 itens) não cita `response_model` nem `make update-openapi-snapshot` | DOC-BLOCK | procede (V-CTO) | procede-fechado | P1 · owner: senior-cto · [[ADR-102]] R18 · [[ADR-109]] · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): exemplo ganha `response_model`; checklist §7 ganha 2 itens (contrato + snapshot) |
| F04 — [[ADR-295]] §Decisão 1 nomeia a camada `value_mismatch`, substituída por `pairing_mismatch` ([[ADR-296]]; `parecer_strict_enforcement.py:24`). Símbolos `_token_matches` e `item_dropped` não existem. `superseded_by: []`, sem `amended_at` | DOC-BLOCK | procede (V-CTO) | procede-fechado | P1 · owner: senior-cto · remédio é **emenda datada**, não reescrita · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): emenda datada apontando ADR-296 (camada) + ADR-366 (retenção total) + os 2 símbolos inexistentes |
| F05 — [[ADR-200]] §D1: o esqueleto YAML do manifest **reprova** contra `docs/_schemas/note-planner.schema.json` (`additionalProperties:false` rejeita `manifest_version`/`schema_version`/`sections`); o real é `version`/`context_sections` (`parecer_planejador.yaml:98,133`). Falsificação executada | DOC-BLOCK | procede (V-DE) | procede-fechado | P1 · owner: data-engineer · hook `planner-manifest-coverage` reprova quem seguir a ADR · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): emenda datada com o `required` real do schema e os 7 format hints |
| F06 — [[ADR-200]] §D4 manda bumpar `manifest_version` "só em breaking"; o campo real é `version`, lido com **fail-open** `raw.get("version","0.0")` (`parecer_manifest.py:134`) e entra na cache key com TTL 7d. **5 de 20** commits do YAML mudaram o manifest sem bump | DOC-BLOCK | procede (V-PE) | procede-fechado | P1 · owner: prompt-engineer · serve parecer stale por até 7 dias · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): mesma emenda: campo é `version`, fail-open `"0.0"`, TTL 7d, 5/20 commits sem bump |
| F07 — [[ADR-204]] declara trigger de imutabilidade (HTTP 412), hook em `report_publication.py` e `test_planner_review_immutability.py`; **nenhum dos três existe** (zero `CREATE TRIGGER` em alembic). `db_artifact_store.py:452-458` sobrescreve sem consultar status | DOC-BLOCK | procede (V5) | procede-aberto | P0 · owner: data-engineer · quem implementar a cascata §D6(b) não escreve guard próprio · **pergunta ao owner:** a ADR-204 está errada (corrigir o texto) ou o mecanismo falta (abrir lane)? Não auto-resolvido — garantia (a) |
| F08 — [[ADR-408]]`:28` cita `extract_with_llm.py:359,404` como o call-site que passa `document_id=None`; essas linhas são `ReviewReason` (e o comentário acima diz que preencher ali **viola** a FK, #1535). O `store.write` real é `:314`, não citado | DOC-BLOCK | procede (V6) | procede-fechado | P0 · owner: data-engineer · [[A40.l73]] · fix ali reintroduz a violação de FK que #1535 fechou · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): ADR-408 passa a citar `:314` e adverte que `:359,404` são `ReviewReason` (o #1535) |
| F09 — `runbooks/suggestion_backfill.md:66-67` manda auditar em `logs/internal_ops_audit.log`; o sink real é a **tabela** `internal_ops_audit` (`internal_ops/audit.py:68-70`, [[ADR-309]]). Única instância viva em doc operacional | DOC-BLOCK | procede (V3) | procede-fechado | P1 · owner: sre-devops · operador conclui que o apply não auditou · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): runbook manda na tabela `internal_ops_audit`, com o SQL |
| F10 — `reference/SMOKE_TEST_HUMAN.md:171` prescreve `sqlite3 mathoms.db "… FROM audit_log …"`; o DB do smoke é `mathoms-smoke.db` (`Makefile:51,392`) e a tabela é `audit_logs` (`audit_log.py:32`). Comando real devolve `no such table` | DOC-BLOCK | procede (V4) | procede-fechado | P1 · owner: sre-devops · gate A26.l5 fecha sobre evidência não coletável · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): `mathoms-smoke.db` + `audit_logs` |
| F11 — `RUNBOOK.md:85-87` ↔ `SLO.md:34` formam **ponteiro circular** de RPO/RTO, e as duas pontas apontam para `BACKLOG.md#f7--produção--lgpd` (shim de 40 linhas, âncora morta). O runbook canônico `runbooks/disaster_recovery.md` (RPO ≤ 24h · RTO ≤ 4h) é **órfão**: `grep -c` = 0 nos dois | DOC-BLOCK | procede (V11) | procede-fechado | P1 · owner: sre-devops · falha em madrugada de perda de host · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): RUNBOOK e SLO apontam `runbooks/disaster_recovery.md`; §12 passa a listá-lo |
| F12 — [[ADR-116]] §D3 prescreve YAML de operador com `email`/`password_hash` e login `{email,password}`; o loader exige `username`/`hashed_password` e **fail-fast** (`internal_ops_auth.py:99-105`). RUNBOOK §7.2 já está certo — a ADR é a fonte divergente | DOC-BLOCK | procede (V-SRE) | procede-fechado | P2 · owner: sre-devops · console interno não sobe p/ quem copiar a ADR · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): emenda datada com `username`/`hashed_password` e as 2 funções reais |
| F13 — `runbooks/vault_full_audit.md:152,161` enumera 5 escopos para a Fase 2 e **omite `moc`** (bucket válido desde [[ADR-302]] §Emenda 2026-08-21 = 265 unidades); contagens §1/§3 defasadas ~2× (`reference` 54→**62**, `adr` 297→**398**, total 409→**869**). §7 conclui "baseline zerado" — falso no caminho faseado | DOC-BLOCK | procede (V-coletor) | procede-fechado | P1 · owner: information-architect · runbook desta própria skill · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): Fase 2 ganha `moc`; contagens re-medidas (869) + instrução de rodar o coletor |
| F14 — `reference/rules/life-insurance-coverage.md:54` prescreve como copy canônica um disclaimer que **nomeia marca metodológica**, proibido em superfície user-facing (COPY_GUIDELINES §13.1). A cadeia fecha: `life_insurance_coverage.py:167` `sources=` → `protection_bundle_populator.py:171 rationale=` → `AcoesMitigacaoCard.tsx:64 <p>{rec.rationale}</p>` → S9 ([[A40.l35]] `shipped` #1476). `check_sigilo_terms.py` **não varre `pipeline/`** — a copy nasce em Python e chega ao TSX como dado | DOC-BLOCK | procede (V7) | procede-aberto | **P0** · owner: financial-planner + sre-devops · mesma classe do ponto cego que COPY_GUIDELINES:562 já pagou · **pergunta ao owner:** o disclaimer precisa nomear a marca? Se não, é troca de copy em `pipeline/` + estender `check_sigilo_terms.py` — toca código, lane XS. Garantia (a) |
| F15 — `reference/rules/life-insurance-coverage.md:18-21,56-57` declara **hold normativo** ("ausência de apólice não é cobertura zero; sem contrato, `missing_data`"; "só por segurado"). Nenhuma das duas pernas tem enforcer: `life_policy_inventory_complete_as_of` existe e **não tem leitor no cálculo** (contraste: o irmão `debt_inventory_complete_as_of` **é** lido em `protection_snapshot_compute.py:147-154`); `_coverage_by_category` agrega ignorando `insured_family_member_id`, contra [[ADR-387]] §D6 | DOC-BLOCK | procede (V-FP) | procede-aberto | **P0** · owner: financial-planner · família sem apólice recebe gap = ideal · **pergunta ao owner:** o hold é norma a implementar ou o doc é aspiracional? **Fechar F15 sem F14 aumenta a exposição de F14.** Garantia (a) |
| F16 — [[A40.l12]] (`status: planned`, P1) §Item B prescreve "ler `trs_meta_pct`"; o campo foi **removido** pela [[A40.l47]] (#1459) e as únicas ocorrências no código são comentários documentando a remoção. A ação prescrita é a promoção saque→retorno que [[ADR-191]] §Emenda 2026-08-14 **proíbe**; `A40/_README.md:991` ainda roteia PD-20 para lá | DOC-BLOCK | procede (V8) | procede-parcial-fechado (2026-08-24) | **P0** · owner: financial-planner · lane viva prescrevendo defeito de domínio já corrigido · **prescrição neutralizada** [#1646](https://github.com/davidrobert/mathoms/pull/1646): Item B riscado + `§Refutação R1` nomeada (garantia (d) — a célula, não nota abaixo). **Segue aberto:** que grandeza a família deveria configurar no card de TRS. `_README` §PD-20 ainda roteia para a lane |
| F17 — [[ADR-135]] (`Decidido`) anuncia em blockquote de sinal que `ir_brackets` "deixa de existir / o nome antigo deixa de resolver"; a coluna segue viva e nullable (`fiscal_parameter.py:42-45`, "mantido durante a janela expand/contract") e a [[ADR-389]] dona do contrato ainda é `Proposto` (lane de contract `A40.l37` `open`) | DOC-BLOCK | procede (V-IA+DE) | procede-fechado | P1 · owner: data-engineer · agente ou não trata row legada, ou escreve migration de drop · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): emenda datada: a contração está deferida; ADR-389 `Proposto`, A40.l37 aberta |
| F18 — [[ADR-358]] é `Proposto` há 21 dias mas já é **doutrina enforçada**: `_HARD_LAYERS` em produção (`parecer_strict_enforcement.py:24`) + teste que cita a ADR + **9 lanes A40 `shipped`** a tratam como vinculante. CLAUDE.md §"ADR Proposto antes de PR P0/P1" manda flipar no merge | DOC-BLOCK | procede (V9) | procede-aberto | P1 · owner: senior-cto · **flip é atestação** (exige SHA que implementa), não housekeeping. Instância menor: [[ADR-368]] · **não flipada de propósito** — garantia (e): flip é atestação e exige conferir cláusula a cláusula + nomear o SHA. Precedente r9·F03 (ADR que parecia pronta tinha só D1 shipada) |
| F19 — `runbooks/atualizacao_codigos_rfb.md:24,70` (gatilho "todo fevereiro") justifica "não bumpe `PROMPT_VERSION`" com o cache LLM da [[ADR-307]]; o e16 **não usa cache** (`use_cache=False` em `litellm_client.py:149`, não passado por `extract_irpf_full`). A razão registrada é falsa e `version:` do YAML é código morto | DOC-BLOCK | procede (V-PE) | procede-fechado | P1 · owner: prompt-engineer · em fev/2027 duas tabelas RFB gravam a mesma `prompt_version` · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): a razão falsa (cache) cai; a decisão de bump fica declarada como pergunta ao owner |
| F20 — [[ADR-027]] (+ [[ADR-270]] §6) promete `LLMValidationError → needs_review` com "nenhum dado perdido; user edita e faz resume"; nos stages de extração o caminho é **exceção**: run vira `failed`, artefatos revertidos, e `resume_run` exige `status == needs_review`. Só o E6 converte | DOC-BLOCK | procede (V-PE) | procede-aberto | P1 · owner: prompt-engineer · contrato de produto inalcançável onde a ADR o promete · **pergunta ao owner:** contrato de produto errado ou implementação faltando? Garantia (a) |
| F21 — **Cluster [[ADR-285]] REABERTO** (`services/X.py` → `services/<subpkg>/X.py`): **71 ocorrências em 20 paths** (adr 21 · sprint 45 · plan 3 · _MOC 1 · reference 1), re-medido pelo loop. O r9 declarou o watch encerrado medindo instâncias nomeadas, não o predicado. Inclui `ARCHITECTURE.md:569,1362` e o padrão href-certo/texto-errado (`ADR-231:44`) | DOC-DRIFT (classe) | procede (V2) | procede-aberto | P1 · owner: information-architect · **lane, não batch**: fechar exige gate `check_doc_code_paths.py` (nenhum gate resolve path de código em prosa — `check_doc_links.py:16` fixa `DOCS=docs/`). FP medido no protótipo: ~50% sem filtro de "(futuro)"/histórico · **23 ocorrências vigentes corrigidas** [#1646](https://github.com/davidrobert/mathoms/pull/1646) (cluster 71 → **56** pares path/arquivo, re-medido). Intocadas de propósito as de moldura histórica (075/083/212/171/084/150/112, track `a3cli`, 45 em `docs/sprint/**`) — corrigi-las é revisionismo. **O gate existe desde 2026-08-25** ([#1654](https://github.com/davidrobert/mathoms/pull/1654), lane [[A40.l78]] `shipped`): `dev/check_doc_code_paths.py` + hook `doc-code-paths`, polaridade no lado do **código** (dispara na deleção/rename, não na escrita do doc). **Segue `procede-aberto` por cobertura, não por ausência de gate:** o `DOC_SCOPE` é `docs/adr/` + `docs/reference/`, ou seja **22 das 71** ocorrências desta célula (31%) — `sprint 45 · plan 3 · _MOC 1` ficam fora (lane e plano **por desenho**; `_MOC` sem decisão, deferido com dono na l78). Fechar esta célula deixaria as 49 restantes sem registro nenhum |
| F22 — **8** ADRs vivas (199-205, 209) citam `docs/plan/PLANNER_REVIEW/_README.md` como plano canônico; arquivado em `docs/archive/PLANNER_REVIEW-2026-07-09.md`. Mesma classe cega de F21 (path em prosa) | DOC-DRIFT | procede | procede-fechado | P2 · owner: information-architect · batch com F21 · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): 11 ponteiros em 8 ADRs para `docs/archive/PLANNER_REVIEW-2026-07-09.md` |
| F23 — [[ADR-255]] residuais: (a) `:49` cita `services/document_classification.py` (real `services/documents/`); (b) declara "entregue em #429" o teste `tests/test_e4_cross_doc_dedup.py`, que **nunca existiu** (`git log --diff-filter=A` vazio); (c) §Schema E4 prescreve `$defs.transactionIdentity`, ausente de `e4_unified.schema.json` — os campos trafegam só por `additionalProperties:true`, e o critério de aceite #7 é vácuo; (d) nota audit-r6 aponta `dedup_natural_key_v2=True` em `e4_categorizer_adapter.py:129`, onde é `False` | DOC-DRIFT | procede (V-DE) | procede-fechado | P2 · owner: data-engineer · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): os 3 itens: teste inexistente, `$defs` ausente (critério #7 vácuo), endereço da flag |
| F24 — `sprint/A40/_README.md`: tabela de lanes diverge do frontmatter em **5 linhas**, na classe que o próprio doc (`:234`) declara defeito — `l6` priority P1⟂P0, `l51` P2⟂P1, `l37`/`l39`/`l41` `depends_on` "—" ⟂ dependência real. `dev/lane_pickup.py` cruza `depends_on`: quem triar pela tabela pega em ordem errada | DOC-DRIFT | procede (V-IA) | procede-fechado | P1 · owner: product-manager · contagem 75/75 correta, zero órfã · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): tabela do `A40/_README` casa o frontmatter nas 5 linhas |
| F25 — [[A40.l23]] `status: shipped` (#1334) com tag `status/open` — única das 75 lanes A40 com divergência campo⟂tag. `build_doc_index` lê o campo (`_generated` correto), mas consulta por tag em Obsidian/Dataview mostra lane entregue como disponível | DOC-DRIFT | procede (V-IA) | procede-fechado | P2 · owner: information-architect · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): tag da A40.l23 vira `status/shipped` |
| F26 — `plan/PUBLIC_RELEASE/_README.md` `status: in_progress` + `sprint_atual: A34`, sem `paused_at`/`pause_reason`; a sprint A34 é `sprint_status: paused` e todas as 13 lanes não-`shipped` estão `planned`. `PLAN_PROGRESS.md:61` agrupa entre os planos em andamento. O vault usa o vocabulário correto em I18N | DOC-DRIFT | procede (V-IA) | procede-aberto | P2 · owner: product-manager · **pergunta ao owner:** pausar o plano é decisão de escopo. Garantia (a) |
| F27 — **Registros `*-active` (grão de linha, [[ADR-302]] §Emenda 2026-08-21):** 9 linhas vivas com trilha inteiramente `shipped`; 1 saudável (LC01 r4 documenta o residual na própria célula) e **6 a reconciliar** — `PIPELINE-REVIEWS#r4/RV4-12` (a prosa do r5 `:307` diz "fechado na r4 pelo PD", a **célula** segue `procede-aberto`: quem lê a tabela antes da nota pega trabalho morto), `REPORT-REVIEWS#r3/RV3-33` (`depends_on: A40.l4`, resolvida #1139; a re-triagem dos 7 era critério de aceite bloqueante), `#r3/RV3-01` (P0 Crítico, trilha `A40.l2` #1368), `#r3/RV3-28` (dono `A40.l7` #1375 shipou com o núcleo aberto), `#r4/RV4-13,15,18` (célula afirma "[[A40.l47]] (**aberta** 2026-08-12)" — shipou #1459), `LEDGER-CERTIFY#r4/LC10` (vetor `A40.l2` PR3 shipou sem re-medição) | DOC-DRIFT | procede | procede-aberto | P1 · owner: donos das skills pares · **fronteira [[ADR-343]]: audita-se o registro, não o mérito**. Correção canônica edita **a célula** (checklist §6). Detector primário é a `lane-closeout` no merge; esta é a rede de segurança · **intocado por desenho** — garantia (f): `--fix` nunca muda disposição de linha de registro; reconciliação é da cadência das skills donas ([[ADR-343]]) |
| F28 — `reference/tenancy.md` §8 (`:355`) descreve como vigente uma dívida **quitada** (`_get_workspace` = 0 ocorrências; `config.py`/`documents.py`/`reports.py` migrados) e dá deadline F8.4 já vencido, calando sobre as 6 entradas residuais de `tenancy_baseline.txt`; §6-§7 (`:313,321,328,340`) institucionalizam `/api/workspaces/...` contra o próprio TL;DR (`:14`) e a [[ADR-108]] | DOC-DRIFT | procede (V-CTO+DE) | procede-fechado | P2 · owner: senior-cto · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): §8 nomeia o residual real (6 queries) e vira procedimento; URLs para `/api/v1` |
| F29 — Precisão factual pontual: [[ADR-109]] `:29` cita `python-jose` (real PyJWT desde #357; a ADR é gate de `security.py` e nunca foi emendada) · [[ADR-089]] `:37-41` aponta `calculators.py` como layout vigente (o arquivo é `DEPRECATED` in-code; canônico é `patrimonio_calculator`) · [[ADR-295]] §D1 diz "item sai, demais seguem" mas severidade alta retém o **parecer inteiro** (`_withhold_all`, [[ADR-366]]) · golden e16 pinado em `1.1.0` com `PROMPT_VERSION=1.2.0` (README manda passo que deixa vermelho) · `python_dependencies.md:49` + [[ADR-254]]`:320` citam `make setup`, inexistente (é `dev-bootstrap`) · [[ADR-116]] §D2 descreve `delete_user(mode=…)` (real: 2 funções) · `.claude/skills/ledger-certify/SKILL.md` manda `scripts/resolve_ledger.py` do repo-root (real `.claude/skills/ledger-certify/scripts/`; as 3 skills irmãs usam a forma completa) | DOC-DRIFT | procede | procede-fechado | P2 · owner: misto · batch · **fechado** [#1646](https://github.com/davidrobert/mathoms/pull/1646): ADR-109 PyJWT · ADR-089 ponteiro p/ os calculadores canônicos · `make dev-bootstrap` · ADR-116 `delete_user` · SKILL da ledger-certify |
| F30 — POLISH: [[ADR-255]] §Próximos passos manda "flip → Decidido" e o frontmatter já é `Decidido` · [[ADR-089]] cita Caminho A (removido em A6c.3) · `GO_SHELL/f1-go-service.md` `consumed` com Fase 1 `- [ ]` e Fases 2-4 `- [x]` · [[ADR-040]] adia Stripe para F8 (fase fechada, sem gatilho de retomada — padrão vigente é §Deferimento datado, [[ADR-356]]) · `CLAUDE.md:1295` diz "27 schemas", medido **30** · `SMOKE_TEST_HUMAN.md:39` manda exportar `MATHOMS_FERNET_KEY` que `Makefile:395` ignora · [[ADR-200]] aponta parser para `pipeline/llm/manifest_loader.py` (real `backend/app/services/parecer_manifest.py`) | DOC-POLISH | procede | aceito-wontfix | wontfix pré-beta (convenção r9 F11), exceto se carona no batch de F29 |

> **r10: 20 DOC-BLOCK (F01-F20) · 9 DOC-DRIFT (F21-F29) · POLISH (F30).** Nenhum
> finding veio da camada 1 — os 7 gates estão verdes e o rendimento inteiro é gap de
> julgamento, que é o desenho da skill ([[ADR-302]]).
>
> **Batch `--fix` executado em 2026-08-24** ([#1646](https://github.com/davidrobert/mathoms/pull/1646),
> 6 commits docs-only) — **20 fechados · 2 parciais (F16/F21) · 7 abertos**. Toda
> edição saiu com citação dupla; as disposições acima foram atualizadas nesta
> camada 5, não dentro do batch (garantia (f)).
>
> **Os 7 abertos são todos decisão de owner, não trabalho pendente de agente:** F07,
> F14, F15, F20 e F26 têm a pergunta na própria célula; F18 não foi flipada porque
> flip é atestação (garantia (e), precedente r9·F03); F27 é da cadência das skills
> donas. **F21 é a única dívida técnica de verdade** — a classe segue aberta sem
> `check_doc_code_paths.py`, cujo protótipo mediu ~50% de FP sem filtro de "(futuro)"
> e de moldura histórica ([[ADR-302]] §armadilha 9). Lane a abrir.
>
> **Atualização 2026-08-27 — a lane foi aberta e entregue.** [[A40.l78]]
> `shipped` (#1654 + o PR do fecho): gate no lado do código, 0 falso no replay de
> 400 commits. **A célula F21 continua `procede-aberto`** — o gate alcança 22 das
> 71 ocorrências; o resto está fora do `DOC_SCOPE`. Não confundir "gate entregue"
> com "classe fechada".
>
> **Dois erros do próprio run, corrigidos antes de virarem dano:** (a) um import
> chutado no exemplo do `tenancy.md` (`backend.app.schemas.goals`) foi conferido
> contra o real (`schemas.dto.goal.if_goal`) **antes** do commit — teria trocado um
> erro factual por outro; (b) o corpo do #1646 afirmou "cluster 71 → 48" subtraindo
> ocorrências de linha de uma métrica que conta pares (path, arquivo). Re-medido:
> **56**. Registrado aqui porque é a armadilha §7 (medição-fantasma) aparecendo
> **dentro** da auditoria que a caça.
>
> **Divergência entre lentes, resolvida:** IA classificou F17 como BLOCK e data-engineer
> como DRIFT sobre a mesma evidência. Mantido **BLOCK**: a afirmação está no blockquote
> de sinal (autoridade máxima) de uma ADR `Decidido`, e erra nos dois sentidos (quem
> confia não trata row legada; quem desconfia descarta a ADR inteira).
>
> **Falsos-positivos evitados (7):** "(futuro)" declarado (ADR-200 `manifest_loader`,
> runbook `override_legacy_drop`) · plano `paused` citando entrega futura (I18N) ·
> path legado em moldura histórica correta (CANONICAL_ENGINE_P0, ADR-135 sobre
> `parametros_fiscais.json`, ADR-102 §Contexto datado) · snapshot datado
> (`PUBLIC_RELEASE/audit-2026-07-08`) · `Decidido`+`superseded_by` é convenção do vault
> (29/30 ADRs) · coexistência LiteLLM/SDK (ADR-024) é legítima · template row dentro de
> fence não foi emitido pelo coletor (0 FP nessa via).
>
> **Nota de amostragem:** F18 e F22 nasceram **fora** da amostra rotativa `--run 10`,
> durante investigação de outros findings. A rotação decide onde olhar, não o que se
> pode reportar.
