...
|---|---|---|---|---|
| F01 — [[ADR-255]] §Backfill prescreve `backend/app/services/internal_ops/recompute_e4.py --workspace-id <id>`; o real é `dev/recompute_e4_for_run.py`, que exige `--workspace-id` **e** `--run-id` (`:99-101`) + `--apply`. Runbook `recompute_e4_dedup.md:51-52` tem a invocação certa | DOC-BLOCK | procede (verificado V-loop) | procede-aberto | P1 · owner: data-engineer · fix docs-only (alinhar ADR ao runbook) |
| F02 — `reference/tenancy.md:300-302` — template "toda rota F8+ tem um teste que…" chama `make_workspace_member(owner)` depois de `make_workspace`, que **já** cria a membership (`builders.py:118-135`), violando `uq_workspace_member` (`workspace_member.py:78`). Probe empírico: `IntegrityError` | DOC-BLOCK | procede (V1) | procede-aberto | P1 · owner: senior-cto · nenhum teste real segue o template |
| F03 — `reference/tenancy.md:99-108` rotula "✅ DO" endpoint **sem** `response_model`; o mesmo endpoint real tem (`api/goals.py:127`) e `test_openapi_response_models.py:67-93` **bloqueia merge** sem ele. Checklist §7 (8 itens) não cita `response_model` nem `make update-openapi-snapshot` | DOC-BLOCK | procede (V-CTO) | procede-aberto | P1 · owner: senior-cto · [[ADR-102]] R18 · [[ADR-109]] |
| F04 — [[ADR-295]] §Decisão 1 nomeia a camada `value_mismatch`, substituída por `pairing_mismatch` ([[ADR-296]]; `parecer_strict_enforcement.py:24`). Símbolos `_token_matches` e `item_dropped` não existem. `superseded_by: []`, sem `amended_at` | DOC-BLOCK | procede (V-CTO) | procede-aberto | P1 · owner: senior-cto · remédio é **emenda datada**, não reescrita |
| F05 — [[ADR-200]] §D1: o esqueleto YAML do manifest **reprova** contra `docs/_schemas/note-planner.schema.json` (`additionalProperties:false` rejeita `manifest_version`/`schema_version`/`sections`); o real é `version`/`context_sections` (`parecer_planejador.yaml:98,133`). Falsificação executada | DOC-BLOCK | procede (V-DE) | procede-aberto | P1 · owner: data-engineer · hook `planner-manifest-coverage` reprova quem seguir a ADR |
| F06 — [[ADR-200]] §D4 manda bumpar `manifest_version` "só em breaking"; o campo real é `version`, lido com **fail-open** `raw.get("version","0.0")` (`parecer_manifest.py:134`) e entra na cache key com TTL 7d. **5 de 20** commits do YAML mudaram o manifest sem bump | DOC-BLOCK | procede (V-PE) | procede-aberto | P1 · owner: prompt-engineer · serve parecer stale por até 7 dias |
| F07 — [[ADR-204]] declara trigger de imutabilidade (HTTP 412), hook em `report_publication.py` e `test_planner_review_immutability.py`; **nenhum dos três existe** (zero `CREATE TRIGGER` em alembic). `db_artifact_store.py:452-458` sobrescreve sem consultar status | DOC-BLOCK | procede (V5) | procede-aberto | P0 · owner: data-engineer · quem implementar a cascata §D6(b) não escreve guard próprio |
| F08 — [[ADR-408]]`:28` cita `extract_with_llm.py:359,404` como o call-site que passa `document_id=None`; essas linhas são `ReviewReason` (e o comentário acima diz que preencher ali **viola** a FK, #1535). O `store.write` real é `:314`, não citado | DOC-BLOCK | procede (V6) | procede-aberto | P0 · owner: data-engineer · [[A40.l73]] · fix ali reintroduz a violação de FK que #1535 fechou |
| F09 — `runbooks/suggestion_backfill.md:66-67` manda auditar em `logs/internal_ops_audit.log`; o sink real é a **tabela** `internal_ops_audit` (`internal_ops/audit.py:68-70`, [[ADR-309]]). Única instância viva em doc operacional | DOC-BLOCK | procede (V3) | procede-aberto | P1 · owner: sre-devops · operador conclui que o apply não auditou |
| F10 — `reference/SMOKE_TEST_HUMAN.md:171` prescreve `sqlite3 mathoms.db "… FROM audit_log …"`; o DB do smoke é `mathoms-smoke.db` (`Makefile:51,392`) e a tabela é `audit_logs` (`audit_log.py:32`). Comando real devolve `no such table` | DOC-BLOCK | procede (V4) | procede-aberto | P1 · owner: sre-devops · gate A26.l5 fecha sobre evidência não coletável |
| F11 — `RUNBOOK.md:85-87` ↔ `SLO.md:34` formam **ponteiro circular** de RPO/RTO, e as duas pontas apontam para `BACKLOG.md#f7--produção--lgpd` (shim de 40 linhas, âncora morta). O runbook canônico `runbooks/disaster_recovery.md` (RPO ≤ 24h · RTO ≤ 4h) é **órfão**: `grep -c` = 0 nos dois | DOC-BLOCK | procede (V11) | procede-aberto | P1 · owner: sre-devops · falha em madrugada de perda de host |
| F12 — [[ADR-116]] §D3 prescreve YAML de operador com `email`/`password_hash` e login `{email,password}`; o loader exige `username`/`hashed_password` e **fail-fast** (`internal_ops_auth.py:99-105`). RUNBOOK §7.2 já está certo — a ADR é a fonte divergente | DOC-BLOCK | procede (V-SRE) | procede-aberto | P2 · owner: sre-devops · console interno não sobe p/ quem copiar a ADR |
| F13 — `runbooks/vault_full_audit.md:152,161` enumera 5 escopos para a Fase 2 e **omite `moc`** (bucket válido desde [[ADR-302]] §Emenda 2026-08-21 = 265 unidades); contagens §1/§3 defasadas ~2× (`reference` 54→**62**, `adr` 297→**398**, total 409→**869**). §7 conclui "baseline zerado" — falso no caminho faseado | DOC-BLOCK | procede (V-coletor) | procede-aberto | P1 · owner: information-architect · runbook desta própria skill |
| F14 — `reference/rules/life-insurance-coverage.md:54` prescreve como copy canônica um disclaimer que **nomeia marca metodológica**, proibido em superfície user-facing (COPY_GUIDELINES §13.1). A cadeia fecha: `life_insurance_coverage.py:167` `sources=` → `protection_bundle_populator.py:171 rationale=` → `AcoesMitigacaoCard.tsx:64 <p>{rec.rationale}</p>` → S9 ([[A40.l35]] `shipped` #1476). `check_sigilo_terms.py` **não varre `pipeline/`** — a copy nasce em Python e chega ao TSX como dado | DOC-BLOCK | procede (V7) | procede-aberto | **P0** · owner: financial-planner + sre-devops · mesma classe do ponto cego que COPY_GUIDELINES:562 já pagou |
| F15 — `reference/rules/life-insurance-coverage.md:18-21,56-57` declara **hold normativo** ("ausência de apólice não é cobertura zero; sem contrato, `missing_data`"; "só por segurado"). Nenhuma das duas pernas tem enforcer: `life_policy_inventory_complete_as_of` existe e **não tem leitor no cálculo** (contraste: o irmão `debt_inventory_complete_as_of` **é** lido em `protection_snapshot_compute.py:147-154`); `_coverage_by_category` agrega ignorando `insured_family_member_id`, contra [[ADR-387]] §D6 | DOC-BLOCK | procede (V-FP) | procede-aberto | **P0** · owner: financial-planner · família sem apólice recebe gap = ideal |
| F16 — [[A40.l12]] (`status: planned`, P1) §Item B prescreve "ler `trs_meta_pct`"; o campo foi **removido** pela [[A40.l47]] (#1459) e as únicas ocorrências no código são comentários documentando a remoção. A ação prescrita é a promoção saque→retorno que [[ADR-191]] §Emenda 2026-08-14 **proíbe**; `A40/_README.md:991` ainda roteia PD-20 para lá | DOC-BLOCK | procede (V8) | procede-aberto | **P0** · owner: financial-planner · lane viva prescrevendo defeito de domínio já corrigido |
| F17 — [[ADR-135]] (`Decidido`) anuncia em blockquote de sinal que `ir_brackets` "deixa de existir / o nome antigo deixa de resolver"; a coluna segue viva e nullable (`fiscal_parameter.py:42-45`, "mantido durante a janela expand/contract") e a [[ADR-389]] dona do contrato ainda é `Proposto` (lane de contract `A40.l37` `open`) | DOC-BLOCK | procede (V-IA+DE) | procede-aberto | P1 · owner: data-engineer · agente ou não trata row legada, ou escreve migration de drop |
| F18 — [[ADR-358]] é `Proposto` há 21 dias mas já é **doutrina enforçada**: `_HARD_LAYERS` em produção (`parecer_strict_enforcement.py:24`) + teste que cita a ADR + **9 lanes A40 `shipped`** a tratam como vinculante. CLAUDE.md §"ADR Proposto antes de PR P0/P1" manda flipar no merge | DOC-BLOCK | procede (V9) | procede-aberto | P1 · owner: senior-cto · **flip é atestação** (exige SHA que implementa), não housekeeping. Instância menor: [[ADR-368]] |
| F19 — `runbooks/atualizacao_codigos_rfb.md:24,70` (gatilho "todo fevereiro") justifica "não bumpe `PROMPT_VERSION`" com o cache LLM da [[ADR-307]]; o e16 **não usa cache** (`use_cache=False` em `litellm_client.py:149`, não passado por `extract_irpf_full`). A razão registrada é falsa e `version:` do YAML é código morto | DOC-BLOCK | procede (V-PE) | procede-aberto | P1 · owner: prompt-engineer · em fev/2027 duas tabelas RFB gravam a mesma `prompt_version` |
| F20 — [[ADR-027]] (+ [[ADR-270]] §6) promete `LLMValidationError → needs_review` com "nenhum dado perdido; user edita e faz resume"; nos stages de extração o caminho é **exceção**: run vira `failed`, artefatos revertidos, e `resume_run` exige `status == needs_review`. Só o E6 converte | DOC-BLOCK | procede (V-PE) | procede-aberto | P1 · owner: prompt-engineer · contrato de produto inalcançável onde a ADR o promete |
...
>
> **Sem `--fix` nesta invocação** — a skill **propõe**, não executa: nenhum item foi
> editado. Recomendação de triagem: (a) **P0 imediatos** F14/F15/F16 (alcançam o
> relatório do cliente hoje) + F07/F08 (induzem regressão de FK/imutabilidade em lane
> viva); (b) **batch docs-only** F01/F09/F10/F12/F13/F19 + F22 + F29 — mecânicos, com
> citação dupla, zero decisão de owner; (c) **lane própria** F21 (gate
> `check_doc_code_paths.py` — FP de ~50% medido no protótipo exige filtro de "(futuro)"
> e de moldura histórica antes de virar gate: [[ADR-302]] §armadilha 9); (d) **atestação**
> F18 (flip da ADR-358 exige nomear o SHA que a implementa); (e) **cadência das skills
> donas** F27.
>
