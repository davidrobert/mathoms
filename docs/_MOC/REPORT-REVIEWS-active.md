...
| RV3-10 — `dependentes_menores_18` como `rationale` de gap de proteção contra `irpf_kpis.dependentes.count=0`: premissa da recomendação nº 1 contestada dentro do próprio payload | consistência | Alto | P2 | **refutado** (2026-08-05, [[A40.l10]]) | não-acionável | as duas contagens medem **populações diferentes** e divergir é legítimo: `protecao_wiring.py::_snapshot_membro` deriva `is_dependente` do cadastro da família (`role ∉ {titular, conjuge}` + idade de `data_nascimento`), `irpf_analyzer.py::_dependentes` conta a ficha da declaração do ano-base ([[ADR-305]]: último ano **completo**, defasado 1-2 anos). Divergem nos 2 sentidos sem erro: filho nascido após o ano-base, declarado pelo outro genitor, modelo simplificado; e `RelacaoDependente` inclui `conjuge_companheiro`/`pai_mae`/`sogro_sogra`, que no cadastro **não** são dependentes. Tratar `count == 0` como refutação encodaria regra falsa e alarmaria toda família com filho pequeno. **Substituído em escopo** por [[A40.l10]] §Correção de premissa: taxonomia por proveniência da premissa |
