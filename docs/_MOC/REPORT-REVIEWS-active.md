...
| RR6-07 — o **único** snapshot de view-model do repo tem `janela_meses: 1`, e com `n_janela = min(12, 1)` as duas janelas **colapsam na identidade** ⇒ nenhum golden do repo pode discriminar defeito de janela, nem antes nem depois do conserto | gate silencioso | Alto | P1 | procede (novo) · **verificado** | **PARCIAL** — a metade "nenhum golden discrimina" caiu em 2026-08-29 (#1828 criou `pontuais-com-aporte-3_reconciled.json` + `test_fixture_discrimina_folga`); sobrevive o snapshot de view-model com `janela_meses: 1` | **precede** qualquer teste de regressão que o `RR6-05` gere: escrever o gate contra esta fixture é escrever um gate que passa nos dois mundos. É o item que trava a ordem de conserto |
