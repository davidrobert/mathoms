...
| Código | Dimensão | Severidade | Prioridade | Veredito | Disposição | Trilha |
|---|---|---|---|---|---|---|
| RR9-01 — **a identidade de imóvel churna entre runs e os dois classificadores falham FECHADOS**: descrição ganha caractere duplicado ⇒ `endereco_canonical` e `property_id` colapsam juntos (**5 de 7 → 1 de 9**) ⇒ `patrimonio_imovel_classifier.py:58-62` e `:76-78` mandam `pid` nulo para o `else` ⇒ `patrimonio.residencia` e `patrimonio.imoveis_geradores` publicados **como zero** | correção | Crítico | P0 | CONFIRMADO · NOVO — medido A/B sobre **corpus documental idêntico** | procede-aberto | o produto afirma que a família **não tem casa própria nem imóvel de renda**, sem aviso: os classificadores não distinguem *"não é"* de *"não sei"*. Os 9 campos da consolidação seguem 9/9, o que prova que o enricher **rodou e falhou** (se não tivesse rodado seriam 0 de 9). Sem `property_id` o dedup cross-IRPF ([[ADR-246]] passo 3b) **vira no-op** ⇒ 3 pares duplicados no publicado. Dona [[A40.l113]]  · ⏳ **Disposição 2026-09-01 (#1962 `c551e832` · [[ADR-433]]):** fechados o eixo do ano por classe (D1/D2) e o estado ternário (D3). **Parcialmente refutado** — o caractere duplicado não é a causa (8 de 9 descrições não têm via+número) e a residência saiu zero **tendo** `property_id` e override. **Segue aberto:** `imoveis_geradores` (não se move com o ano — depende da âncora estruturada, medida como inexequível com o contrato vigente) e os 3 pares duplicados (eixo `codigo_rfb`). Âncora pós-#1962: `patrimonio_imovel_classifier.py::classificacao_do_imovel` |
| RR9-02 — **o ano-base afirma um 31/12 que ainda não fechou, e o eixo zera imóveis, veículos e a dívida**: uma tela de posição do Itaú capturada em **29/03/2026** entra como `valores_31_12["2026"]` ⇒ `max(years)` leva o eixo do domicílio para um 31/12 que não ocorreu ⇒ `_resolve_item_valor` não acha a chave em nenhum item de 2025 e cai no `valor` cru ⇒ 7 imóveis, 7 veículos e as 4 dívidas do titular saem **zero** | correção | Crítico | P0 | CONFIRMADO · NOVO · **enunciado corrigido 2026-09-01 por medição** | **RESOLVIDO** PR #1961 + #1962 ([[ADR-433]]) | `endividamento.total_dividas` **zero** com 4 financiamentos listados; `score.componentes[taxa_endividamento]` `nota=10,0` com peso **18,8%** e `status="emitted"`; superfície publica "Endividamento Mínimo/Nulo" como **Ponto Forte** em 2 inventários. O irmão `_resolve_saldo` ([[ADR-401]]) **tem** fallback — daí a lista somar certo **na mesma página**. ⚠️ **correção de atribuição**: o comentário em `endividamento_analyzer.py:138` nomeia `_total_dividas_for`, defeito real mas **dormente**. ⚠️ **duas premissas do enunciado original caíram na medição** ([[A40.l114]] §Medição): o LLM converteu exercício→ano-calendário **corretamente em 3 de 3** declarações, e o `2026` vem de um documento que **É de 2026** — logo o remédio "reconciliar contra os anos observados nos documentos" seria **inerte**. O invariante que pega a classe é **temporal**. ⚠️ **a manchete desta rodada é o eixo de ano**: `patrimonio.bruto −48,1%` sai inteiro dele (2.012.174,02 ÷ 3.879.177,72 = 0,519, ao centésimo) — **mas o crédito é da [[A40.l113]]**, cuja [[ADR-433]] (eleição de ano-base por classe de ativo) mergeou antes e resolve o corpus **sozinha**. Medido pós-rebase: os números ficam idênticos com qualquer um dos dois consertos desligado. O que permanece zero é `residencia`/`imoveis_geradores`, também da [[A40.l113]]. Dona [[A40.l114]] do produtor único de saldo, do tripwire, da supressão no score e do `_total_dividas_for` |
| RR9-04 — `#S4` **afirma e nega** imóveis de investimento: abre afirmando ~34,8% do bruto, o card abaixo diz que **não há nenhum classificado**, o KPI repete a negativa e o ranking lista **dois em #1/#2** — 3 asserções de ausência contra 3 de presença; a seção mede **338px**, só o estado vazio | consistência | Crítico | P0 | CONFIRMADO · NOVO | procede-aberto | **é o `RR9-01` visto pela superfície**: sem id não existe classificação, e cada consumidor resolve o vazio do seu jeito. Dona [[A40.l113]]  · ⏳ **Disposição 2026-09-01:** escopo entra no **item 1** do §Deferimento da [[A40.l113]] (supressão do agregado com motivo) — a lane passa a citá-lo. `real_estate.valor_total_imoveis` **não** foi fechado pelo #1962: resolve valor por `property_id` e 8 de 9 itens não o têm |
| RR9-16 — **o sanitizer de PII mede o contexto de ENTRADA e nunca o output**: CPF com dígitos finais em claro em prosa **e** em `evidencia`, e agência/conta **completas** na mesma página | contrato | Alto→**Médio/Alto** | P1 | CONFIRMADO · **enunciado corrigido em 3 pontos na execução** | **RESOLVIDO** PR #1964 ([[ADR-435]]) | a política protegia 1 identificador e publicava 2 — isso procede e está fechado. **O enunciado errava em 3 pontos**, todos medidos sobre o payload real: (a) *"nada mede o output"* é **falso** para o relatório — `redact_view_model` redige `/reports/{id}/data` desde a [[A40.l6]] e dava **0 hits**; o defeito era **vocabulário** do gate. A frase vale para o parecer e para um **terceiro** canal que o achado não citava (`suggestions`, prosa do LLM copiada verbatim); (b) a docstring **não** é globalmente falsa — o achado confunde `_IDENTIFIER_KEYS` (por chave, 1 elemento) com `scrub_identifiers` (por valor, cobre CPF/CNPJ **crus**); o diagnóstico de fundo procede, a caracterização não; (c) o CPF **não** sai em `evidencia` neste run — está em `descricao`. **Severidade:** conta+agência = Alto; CPF parcial no display **não é achado** ([[ADR-259]] §4 já sanciona 5 dígitos em claro contra os 2 daqui) — restava o egresso ao provider, **Médio**. Medição: 0→7 hits, 7→0 aplicado, **zero FP** em 5.562 strings. Dona [[A40.l115]] |
...
| RR9-21 — os **4 achados de clareza-ux do `U4` reproduzem** integralmente nas duas viewports | clareza-ux | — | — | MEDIÇÃO-DE-CONHECIDO | **não é achado novo** | reproduzem porque as lanes seguem `open`: [[A40.l105]] · [[A40.l106]] · [[A40.l107]] · [[A40.l108]]. Registrado para não contar duas vezes · ⚠️ **Disposição 2026-09-02 (closeout da [[A40.l105]] · #1984):** a l105 **fechou** — *"seguem `open`"* vale para as outras três, e a reprodução do `U5` é anterior. Duas ressalvas medidas no closeout, sem alterar o veredito: (a) dos 4, só [[A40.l106]] (`RR8-01`) e [[A40.l107]] (`RR8-02`) são `clareza-ux` — a l105 nasce do `PV12-03` (`saúde-execução`) e a [[A40.l108]] do `RR8-03` (`consistência`); (b) o defeito da l105 **não é observável em captura de viewport** (a `CleanBar` não renderiza neste workspace, segurada pelos 5 documentos em revisão), logo *"reproduzem integralmente nas duas viewports"* não pode tê-lo medido |
