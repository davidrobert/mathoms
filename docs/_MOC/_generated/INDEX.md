...
| ADR-236 | adr | Proposto |  | Cone Monte Carlo de IF inclui aporte mensal (paridade com projeção determinística) | `adr/236-monte-carlo-if-with-pmt.md` |
...
| MOC-sprint-a16 | moc |  |  | Sprint A16 — Flip ADR-235 (`nu_proprietario` no enum `classification`) | `sprint/A16/_README.md` |
...
| TRACK-a16-adr235-nu-proprietario-flip | track | ready | A16 | Track A16 — Flip ADR-235 `nu_proprietario` para Decidido (migration + call-sites + ADR updates + E6 prompt + CI gate) | `sprint/A16/tracks/a16-adr235-nu-proprietario-flip.md` |
