...
## Lanes (95 no disco · 95 nesta tabela — ver nota ao fim)
...
| [[A40.l94]] | Folga mensal reclassifica gasto pontual realizado como sobra recuperável | P0 | open | aberta 2026-08-29 do `RR6-01` da rodada unificada **U2** · identidade fechada ao centavo: `folga_mensal × 12 == poupança 12m + pontuais 12m`, e os dois percentuais de "quanto sobra" dividem o **mesmo** denominador divergindo em **19,4 pp** — a diferença é exatamente `total_pontuais_janela` · **alcança o usuário**: quem dimensiona aporte pela folga dimensiona 27% acima do que a poupança sustenta, e a maior das duas sobras é a que prescreve · §Sequência: a base dos pontuais já está contaminada por aporte e transferência (`LC6-05`), então consertar a folga sozinha move o número para outro valor errado · **entra na cláusula de reinício do contador** (muta E5) |
| [[A40.l95]] | Numerador da concentração imobiliária inclui bem que o motor declara não-gerador | P0 | open | aberta 2026-08-29 do `RR6-02` da **U2** · o numerador soma a nu-propriedade que `real_estate.excluded_properties[1]` exclui com o motivo literal "não gera caixa nem está disponível para venda livre"; sem ela o KPI vai de 50,62% para **49,08%**, abaixo do limiar 50,0 com operador `<` · desarma de uma vez ponto urgente, risco Alta do parecer, alerta de real estate, KPI vermelho e componente do score com peso 12,5% · o braço cego, sem ver a análise, marcou a mesma alavanca `indeterminado-por-viés` (margem de 0,62 pp) · leia [[A40.l80]] §C6 antes de abrir escopo — o vocabulário de `base` dos `kpi_targets` pode já ser dela · **entra na cláusula de reinício** (muta E5) |
| [[A40.l96]] | Tabela de maiores ativos atribui titular a valor que o sistema declara órfão | P0 | open | aberta 2026-08-29 do `RR6-03` da **U2** · a tabela preenche titular em **15/15** linhas enquanto o mesmo relatório publica que ~metade da carteira financeira não tem titularidade; a refutação óbvia ("as órfãs não estão entre as maiores") é **aritmeticamente impossível** — as 15 linhas somam 92,7% da base, e a fatia órfã não cabe no residual por 2,4× · ⚠️ **direção não determinada**: pode ser o roll-up patrimonial que erra, e nesse caso o risco Alta e a supressão da realocação é que são espúrios e o dano **inverte** · primeira entrega é a **medição discriminante** (proveniência por item do titular no E4 vs a agregação do patrimônio), não um fix · **fora da cláusula de reinício** até a direção ser conhecida |
