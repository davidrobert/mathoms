...
> **Atualização 2026-09-01 — a rodada unificada `U5` mantém o contador em 0/2, e as duas
> lanes novas ENTRAM na cláusula de reinício.** A `U5` ([[LEDGER-CERTIFY-active]] §r9 ·
> [[PIPELINE-REVIEWS-active]] §r13 · [[REPORT-REVIEWS-active]] §r9) foi um re-run completo
> E0→E6 + parecer + revisão sobre o **mesmo corpus documental**, e abriu **2 P0**:
> [[A40.l113]] (identidade de imóvel churna, classificadores falham fechados) e
> [[A40.l114]] (ano de referência sem documento atrás, total de dívida zerado).
>
> **Elas entram na cláusula, ao contrário das duas da `U4`, e o critério é o mesmo:**
> *"muta E3/E5 a montante de todo run"*. As da `U4` eram instrumento e guard de escrita, e
> **não deslocavam valor publicado**. Estas deslocam **95 de 400** escalares do payload —
> `bruto` **−48,1%**, `residencia` e `imoveis_geradores` **→ zero**, `total_dividas`
> **→ zero**, `if_gap` **+27,3%**. **São OITO lanes na cláusula** — [[A40.l94]], [[A40.l95]], [[A42.l15]], [[A40.l96]], [[A40.l98]], [[A40.l111]], [[A40.l113]] e [[A40.l114]] —, e o contador de 2 re-runs
> só inicia depois de as oito estarem terminais. ⚠️ **Correção 2026-09-01:** este bloco dizia SETE somando +2 sobre CINCO e ignorou a extensão logo abaixo, que já admitira a [[A40.l111]] e declarara SEIS. O padrão "são N" errou por incremento cego duas vezes; a lista nominal acima passa a ser a forma canônica.
>
> **A rodada foi disparada como DIAGNÓSTICO, a pedido do dono, com os gates fechados** — e
> a consequência foi registrada **antes** do disparo: a [[A40.l96]] segue `in_progress` na
> cláusula, logo nada aqui pontua gate. O que a `U5` acrescenta é que o corpus é o mesmo e
> o relatório mudou pela metade: **três rodadas unificadas consecutivas davam os números
> anteriores**. Não é dívida antiga; é regressão de 2 dias.
>
> **Lição de método que vale além do contador.** Nenhum dos dois P0 tinha sinal
> bloqueante: o run saiu `completed` com 2 avisos de pausa.
>
> ⚠️ **Correção 2026-09-01 (PR #1962):** este parágrafo prescrevia um gate sobre a
> **contagem de `property_id` resolvidos** entre runs. A medição da [[A40.l113]] refutou a
> prescrição: a residência saiu zero sendo o **único** item que **tinha** `property_id` —
...
> superfície, guard pós-LLM e prompt.
...
## Lanes (119 no disco · 119 nesta tabela — ver nota ao fim)
...
| [[A40.l95]] | Numerador da concentração imobiliária inclui bem que o motor declara não-gerador | P0 | ✅ **#1901 · #1904 · #1911 · #1924 · #1926 · #1928 · #1932** | aberta 2026-08-29 do `RR6-02` da **U2**; **medida e re-enunciada** no mesmo dia ([[ADR-420]] `Proposto`, #1826) · o numerador soma a nu-propriedade que `real_estate.excluded_properties[1]` exclui com o motivo literal "não gera caixa nem está disponível para venda livre"; sem ela o KPI vai de 50,62% para **49,08%**, abaixo do limiar 50,0 com operador `<`, e **58,9% dela decide o cruzamento** · desarma ponto urgente, risco Alta do parecer, alerta de real estate e KPI vermelho — **o score NÃO cai** (7,438 → 7,463, arredonda 7,4 nos dois; a redação anterior desta linha prometia queda de componente com peso 12,5%) · **o corte não é `geradores`:** o discriminador é rebalanceabilidade, `especulacao` fica e `uso_pessoal` sai · **a [[A40.l80]] NÃO é dona** — medido no ataque: o §Escopo dela nomeia denominador, e trocar o numerador reabriria o ramo que a [[ADR-412]] §E5 fechou; um item volta para lá (declarar o numerador, o C14 dela deslocado um campo) · ~~⚠️ **bloqueada por fixture:** nenhum golden do repo tem `imoveis_geradores > 0`~~ **precondições fechadas 2026-08-31**: o numerador passou a ser DECLARADO no payload (**#1901**, número-neutro — o D5 estava órfão, devolvido à [[A40.l80]] que encerrou em #1835 sem absorvê-lo) e a fixture do dogfood passou a discriminar o destino de cat_2 (**#1904** — o imóvel opaco de R$600k virou cinco somando os mesmos R$600k, `bruto` e `imoveis_investimento` byte-idênticos, `imoveis_geradores` 0 → 150.000) · **falta o passo 3** (o fix de §D1/§D3), cujo bloqueio é **documental**: a escada da [[ADR-353]] já está em produção e o que falta é o consumidor de frontend, mas o flip dela pende da [[A40.l11]] (`planned`, **P2**) enquanto este passo é P0 — e a [[ADR-425]] (`Decidido`, um dia depois da [[ADR-420]]) já importa essas mesmas constantes com a 353 `Proposto` · **`shipped` 2026-09-01** ([[ADR-420]] `Decidido`): numerador corta por rebalanceabilidade (82,19% → 77,19% no dogfood, com `bruto`/`liquido`/cat_2 **byte-idênticos** — o fix não move dinheiro), `BASE_VERSAO_CORRENTE` 1 → 2, `score_version` **não** · a §D2 foi **emendada** (a dependência da [[ADR-353]] vale para o **piso**, não para o flip da nota) e o piso ficou deferido, re-roteado para quem executar a [[A40.l11]] · **critério 5 fechou pelo §D1 sem código de contagem** — o card conta `INVESTMENT_CLASSIFICATIONS` e o numerador novo é esse conjunto ∪ {desconhecido}, então as cardinalidades coincidem no corpus classificado; o resíduo virou a [[A40.l112]] · **critério 6 fechou inteiro**, por subtração do qualificador falso (o balde cat_2 virou "Outros imóveis"), e as 2 superfícies que sobraram filtram `{locado, comercial}` — ali "de renda" é **verdade** e a lista da ADR as contava por engano · o bump de série revelou defeito alheio: fixture cambial declarava `serie_corrente` e fixava o literal `1` · **#1931 fechado como redundante** (ramifiquei de branch em vez de `origin/main` e o #1932 o absorveu) · **entra na cláusula de reinício** (muta E5) |
| [[A40.l96]] | Tabela de maiores ativos atribui titular a valor que o sistema declara órfão | P0 | ✅ **shipped 2026-09-01** em **5 PRs** (#1823 · #1905 · #1910 · #1925 · #1930 · **#1936**) · [[ADR-430]] | aberta 2026-08-29 do `RR6-03` da **U2** · a tabela preenche titular em **15/15** linhas enquanto o mesmo relatório publica que ~metade da carteira financeira não tem titularidade; a refutação óbvia ("as órfãs não estão entre as maiores") é **aritmeticamente impossível** — as 15 linhas somam 92,7% da base, e a fatia órfã não cabe no residual por 2,4× · ~~⚠️ direção não determinada~~ · **MEDIDA 2026-08-29 (PR1, zero produção)** e por uma **terceira via**: as duas hipóteses caem — o roll-up está certo ([[ADR-412]] §D3) e o Top 15 lê `fonte: irpf_bens`, sendo o **extrato** o lado sem titular (`itau`/`rico` emitem `titular: None`) · **o sinal existe e morre entre o E1 e o E4**: o artefato E1 `members` traz `banco_membro` (11 instituições) e `contas[]` (18) — incluindo as 3 órfãs — mas o E4 lê o `family_members.json` materializado de `bank_accounts`, tabela com **0 rows no banco inteiro** · somam-se `AccountResolver` marcando `ambiguous` por **cardinalidade de contas** e não por divergência de membro (`itau`/`rico` têm 2 contas cada, **ambas do mesmo membro**), e a saída do resolver entrando **sem** canonicalização ([[ADR-243]]) · **contrafactual sobre os 8 subconjuntos** (corrigido no closeout 2026-08-29 — a redação anterior, "qualquer subconjunto próprio deixa o número idêntico", era **falsa**): 5 dos 6 próprios não-vazios são inertes, mas **`{D1,D3}` move para 46,25% órfão / ~33,3% publicado — e segue acima do `piso_pct: 1.0`**, logo parece progresso com as 8 superfícies acesas; só `{D1,D2,D3}` leva a 0,13% / ~0,10% ⇒ risco Alta, `prescricao_realocacao_suprimida`, confiança *"insuficiente"* e a ação P1 *"reconciliar a titularidade"* (inexecutável pela família) são **espúrios** — 8 superfícies · **o argumento aritmético do achado cai como mecanismo** (as duas porcentagens têm denominadores distintos), a conclusão sobrevive por outra rota · **entra na cláusula de reinício** — muta E4 e E5 · PR2 exige co-design `data-engineer` + `financial-planner` antes da ADR · **CO-DESIGN 2026-08-31 (#1905)**: o painel **refutou 3 afirmações** — (i) *"a família não pode executar a ação P1"* é **falsa** ([[ADR-229]] §1 já decidiu tier-1/clique-humano e a UI está shipada: as 18 contas do E1 **já são cards clicáveis**); (ii) o `nubank` **não** é ambiguidade viva no eixo carteira (é conta-corrente, não posição) — o erro era meu, commitado 1h antes; (iii) a dicotomia persistir-vs-ler-artefato é falsa, porque a **[[ADR-226]] §5 decidiu (a) em 2026-05-19 e nunca foi implementada**, e a [[ADR-229]] §1 decidiu o oposto no dia seguinte sem declarar supersedure · **achado bloqueante novo (D4)**: `confidence` era calculado, logado e **descartado** no boundary — sem provenance o fecho trocaria falso-negativo por **falso-positivo** ([[ADR-394]]) · **ordem invertida: D2 antes de D1**, porque sob o predicado velho o merge de hint **fabrica `ambiguous` novo** e pioraria workspace que hoje resolve · **SHIPPED 2026-09-01 (#1936)**: fecho medido pelo run `40d1af2a` — `pct_carteira_financeira` **49,03% → 0,1%** (abaixo do piso), `motivo: None`, `prescricao_realocacao_suprimida: False`, **nenhum risco de titularidade no parecer**, e o E4 publica a chave canônica em **99,87%** · **a previsão por composição bateu na vírgula** (0,13% órfão = as 4 posições Binance) · **o número que importa mais: `pct_inferido` = 48,93%** — os ~49% não sumiram, **mudaram de significado** (de *"sem dono"* para *"dono por inferência, não por declaração"*), que é exatamente o que o D4 preservou · **o gate de não-inércia pegou 2 defeitos meus**: o controle de D3 não desligava D3 (`{D1,D2}` dava o mesmo que o fecho) e a fixture não continha o caso que torna `{D1,D3}` perigoso — corrigidos, `{D1,D3}` reprova a **75,12%** · `needs_review` **retirado do critério por decisão** (sentinela load-bearing na [[ADR-346]] §4b) · **deferidos com dono**: fallback da célula do Top 15 (**sem call-site** — eixo A só tem 2 baldes), papel `casal`, `acima_do_piso` herdando o piso de nomeação, 42 artefatos E1 com CPF cru (expurgo existe, só chamador em teste) |
| [[A40.l97]] | Índices perdidos por `copy_from`: 3 UNIQUE derrubaram invariante e o gate de drift era cego a índice | P0 | ✅ **#1841 + #1902** | aberta 2026-08-30 como **colateral** da auditoria da [[ADR-235]] na [[A40.l95]] · `batch_alter_table(copy_from=)` faz drop+recreate em SQLite e o snapshot nunca declara `Index`: **38 índices** do model ausentes no DB migrado, em 13 migrations, todas com o padrão · 3 UNIQUE derrubavam invariante, entre eles `uq_report_publications_active`, cujo read-path usa `scalar_one_or_none()` ⇒ `MultipleResultsFound` no predicado de mês fechado ([[ADR-187]]) · **Postgres nunca perdeu** (`DefaultImpl` não recria) — é SQLite-only, logo dev/dogfood/suíte · reparo dos 3 UNIQUE e extensão do gate entregues em **#1841** · ~~falta decidir se os ~35 não-unique ainda são desejados~~ **a medição reformulou a pergunta** (#1902): cadeia emitida em dialeto PG + pareamento por (tabela, colunas, unique) — **Postgres tem 30 deles desde sempre**, logo D4 invertia quem decidiu; restaurá-los é **paridade**, e quem estava fora do combinado era o SQLite · dos 35, **5 não eram ausência**: 4 rename (mesmo índice sob outro nome — recriar duplicaria) e 1 gap real que `copy_from` não causou (`ix_task_suggestions_status`, que **nenhum** DB jamais teve) · o pareamento por colunas sozinho deu **1 falso rename** (`ix_txov_active_workspace` é PARCIAL) — só o DDL literal separou · catraca: 39 entradas saíram do catálogo com **zero drift novo** · `DB_SCHEMA_REFERENCE` nomeava 4 índices inexistentes · [[ADR-423]] `Decidido` + emenda (33, não 38; D5/D6) · dry-run no dogfood **zero colisões**, com 1 dos 3 vacuamente zero (0 rows) · follow-up que **não** é dívida desta lane: reavaliar os 9 índices de `tasks` em **prod** ([[ADR-162]]) · **não** entra na cláusula de reinício (não muta E3/E5) |
| [[A40.l98]] | Base de gasto pontual tem três produtores com filtros disjuntos, e o que prescreve é o que menos filtra | P1 | ✅ **#1865** | aberta 2026-08-30 no closeout da [[A40.l94]], que a nomeou como dona dos 3 itens deferidos e apontava para uma lane **inexistente** · do `LC6-05` da **U2**, com `LC6-06`/`LC6-07` roteados junto · **re-medição:** o registro dizia "uma aplicação, dois pontos" e são **três** produtores — enricher (`{aporte_investimento}`), a **lista** do card (`consumo_pontuais.py`, que aplica `InternalTransferDetector`) e o **KPI** (`_collect_candidates`, que não aplica nenhum dos dois): lista e KPI do mesmo card filtram coisas diferentes, e o que prescreve é o que menos filtra · no dogfood **57,5%** da base da janela é movimentação patrimonial em `nao_identificado` (R$ 194.886,65 para outro banco do titular + R$ 32.000 em conversões Wise) · **P1 e não P0** porque a [[ADR-422]] tirou a contaminação das prescrições **determinísticas** (folga, teto) — mas o parecer segue recebendo `total_pontuais` no exec context e emitindo risco com ele · carrega a regra de domínio *"`nao_identificado` fora de numerador que prescreve"*, decidida em co-design e existente **só em doc** (aqui e na [[A40.l94]] §Deferimento), **nunca em código** · **entregue 2026-08-30** em 6 commits (policy única + limiar de fonte única + união das categorias + `base_pontuais` com leitor + `nao_identificado` fora do numerador + mesma população nos dois lados + `LC6-06`); `manifest_version` 2.9.0 → 2.13.0 · **quatro premissas caíram na medição** — o detector é inerte dentro do E5, os dois lados leem o MESMO artefato E4 (a divergência é a camada de override), o escape "estorno negativo" da [[A40.l101]] tem outro mecanismo (o E4 manda o crédito para **receitas**, e o par fica mais otimista), e a [[ADR-425]] D4 não tem régua (D3 sem implementação, com sinal trocado) · destrava o item 2 da [[A40.l102]] · **entra na cláusula de reinício do contador** (muta E5) |
...
| [[A40.l105]] | Aprovação-com-avisos é **indistinguível de nunca-ter-pausado** no desfecho do run (`_runs_with_degraded_stage` só filtra `degraded`), e é o desfecho que alimenta o banner de qualidade | P2 | ✅ **#1984** | aberta 2026-08-30 do `PV12-03` da **U4** · cético **rebaixou**: a `CleanBar` não renderiza neste relatório porque os 5 documentos em revisão a seguram ⇒ o defeito é do **predicado**, não do sintoma de hoje · ✅ **entregue 2026-09-02** — e o **controle positivo derruba a ressalva do cético**: a pausa não tem canal nenhum na UI (`computeDataQualitySignals` não a lê, `countActiveSignals` não a conta), então `runOutcome` é o **único** gate da `CleanBar`; com zero documentos em revisão a barra "sem pendências" renderiza. A contingência era do **workspace da U4** (5 docs em revisão), não do mecanismo ⇒ **severidade sobe** · o termo é `paused_at_stage`, **não** a decisão da conferência: `StageReview.status` custaria 3ª query e discriminaria pior — `edited` também publica artefato que o pipeline não produziu sozinho, e `validation_issues` é **fail-open** (`NULL` quando a projeção falha, [[ADR-165]] onda 2), logo chavear nele calaria justamente o caso em que se sabe menos · corte conservador porque a assimetria manda: errar para `with_gap` custa **silêncio** (`runOutcome` de propósito não entra no `count`, [[ADR-357]] · [[A40.l18]]), errar para `complete` custa **afirmação falsa** no PDF · **sem ADR nova** — o predicado mora no docstring do enforcer, não em ADR, e o termo **conforma** à polaridade positiva já decidida ali ([[ADR-143]]) · 3 testes, um deles **contra-prova** (run sem pausa segue `complete`; sem ela `paused is not None` passaria constante e o gate seria vacuidade) · ⚠️ **closeout achou 3 itens, 2 contra mim** — o maior é **sobre-crédito meu ao `PV13-18`**: eu escrevera que esta lane o fecha pela metade e ela fecha **zero** (`with_gap` faz o banner `return null`, e o predicado dele é sobre o **silêncio**); ele fica **sem dona**. O falso-verde do processo e a prova de que as 8 falhas locais são pré-existentes ficam no §Fecho da lane |
| [[A40.l106]] | O relatório **não emite índice de seção algum no mobile** — 32.728 px de rolagem sem navegação, enquanto o desktop emite o índice completo logo após o breadcrumb | P2 | open | aberta 2026-08-30 do `RR8-01` da **U4** · verificado pelo loop sobre a captura de render das 2 viewports; o shell do app é **byte-idêntico** nas duas, logo o que falta é a navegação **do relatório** |
| [[A40.l107]] | A conversão tabela→cartão no mobile é **por componente, não por regra**: 4 convertem, **11 com ≥3 colunas não**, e há um **terceiro** comportamento (descarta coluna e segue tabela) | P2 | open | aberta 2026-08-30 do `RR8-02` da **U4** · cético **CONFIRMOU e corrigiu a contagem para cima** (o enunciado dizia ≥6) · caveat: overflow horizontal não é observável em dump linearizado |
| [[A40.l108]] | Um mesmo ano nomeia o **cenário central** estocástico e o de **estresse**, enquanto o cenário **base** do mesmo apêndice é outro — dois motores de projeção com anos-base diferentes | P2 | open | aberta 2026-08-30 do `RR8-03` da **U4** · o cético **refutou** o enunciado original (três anos sem rótulo) e, ao refutar, achou a colisão · `RV3-14` cobre a capa; a colisão é nova |
| [[A40.l109]] | A lista do card lê o artefato mais recente sob um relatório pinado e imutável | P1 | open | aberta 2026-08-30 no co-design do item 2 da [[A40.l102]]; achado do `data-engineer`, **verificado de forma independente** · KPI e declaração saem de `report.analysis_artifact` (**pinado** ao run); a tabela sai de `read_latest_artifact` (**o mais recente**), e o endpoint não aceita `report_id` nem `run_id` · relatório publicado antigo mostra KPI do run N com lista do run N+1, e nada no wire permite notar — lê em volta da imutabilidade que a [[ADR-347]] existe para garantir · o discriminador (`_report_lineage.pipeline_run_id`) já está no payload · P1: exige run novo entre publicação e leitura, mas o dano é silencioso e atinge relatório já entregue · exige ADR `Proposto` antes do PR (contrato de leitura de rota de relatório) |
| [[A40.l110]] | O baseline grava `date.today()` no artefato e o §F da [[ADR-409]] nomeia o produtor errado | P1 | ✅ **#1914 · #1929 · #1933** | aberta 2026-08-31 no tratamento dos achados da [[A42.l19]]; co-design `data-engineer` · **medido**: mesmo input em dois dias civis produz `sha256` diferente do balde `patrimonio` — quebra de idempotência em artefato **persistido**, e envenena toda métrica de massa por conteúdo · os 2 `required` fósseis (`pipeline_stage`, `data_processamento`) têm **zero leitor** de produção, e o `const` deles nomeia um stage que a [[ADR-093]] não reconhece · o §F manda re-derivar *"do produtor E1.5c"*, mas a `description` do schema declara o **E4 pós-normalização** — executá-lo ao pé da letra escolhe o shape errado · normalizer sintetiza + fixture hardcoda: produtor e teste concordando na crença errada, 2ª instância da patologia da [[ADR-427]] na mesma sprint · PR-A atômico (as duas pontas juntas) → PR-B re-deriva · **PR-A entregue 2026-09-01 em #1914** (`f8cbf94d`): drift do schema 101/169 → 6/169 no corpus, os 2 `required` fósseis mortos nas duas pontas, 226 linhas de dead code de disco removidas, e `_CONTRATO_NAO_DERIVADO` impede o flip sair `GO` sobre contrato não re-derivado · **PR-B pendente** — sobreposição declarada×emitida é **5 de 11**, medida · **PR-B entregue 2026-09-01** ([[ADR-432]]): contrato re-derivado do produtor, **5 de 11 → 14 de 14**, `oneOf` de raiz colapsado e `additionalProperties: false`; `_CONTRATO_NAO_DERIVADO` levantado. O flip segue bloqueado **pelo número** (74/169: 3 de valor da [[A40.l111]] + 71 de artefato histórico com os fósseis), não mais pela prosa |
| [[A40.l111]] | Imóvel de valor negativo entra na soma do patrimônio, e a guarda de sinal é cega ao item | P1 | ✅ **#1917** | aberta 2026-08-31 no tratamento dos achados da [[A42.l19]]; co-design `financial-planner` com regra RFB confirmada em fonte externa · **medido**: o agregado é positivo (o item é ~14% do total), logo o D6 da [[ADR-394]] **nunca dispara** — é cego ao grão do item, não tolerante · o PGD **não aceita** negativo em Bens e Direitos ⇒ o sinal é nosso, defeito de medição com eixo correto; e `instituicao` na linha do bem é o que a Receita **manda** preencher, não evidência de dívida · decisão: valor vira `null` fora da soma (não zero, não passivo, não `abs()`), com ressalva que diz **a direção do erro** · prescrições que dependem de cat_2 suprimidas com motivo; descritivos publicam · **entregue 2026-08-31** ([[ADR-431]] `Decidido` + emenda à [[ADR-394]]): o saneamento roda em **dois** pontos com a mesma função, porque sanear só no boundary deixava o negativo em `patrimonio_por_ano` e o E5 o **reinjetava** pelo resíduo `resumo − sintético` creditado ao titular · o leitor consulta a **declaração** do produtor e não o `null` cru: a cascata de chaves alternativas ressuscitaria um valor de **outra data** · supressão herda o canal do D6 (a guarda ganha `itens_sem_valor`, `motivo_supressao` compõe os dois eixos) ⇒ concentração, desvio e próximo aporte já saem suprimidos pelos consumidores existentes · `S4` fora do cap rate, do peso e do pro-rata · **a emenda à [[ADR-394]] corrige a consequência 3 da §Taxa de disparo**, que lia "0 disparos" como "0 ocorrências": a [[A40.l66]] fechou o roteamento pelo **rótulo**, não a chegada de negativo cujo eixo é decidido por **fato** · **contrafactual medido** em worktree destacada: 10 asserções falham sem o fix e **6 passam** — e são essas 6 que são controle, não gate · **defeito do próprio PR** achado em auto-revisão: a 2ª passagem do saneamento **sobrescrevia** os anos em vez de uni-los, e o teste de idempotência não pegava · ADR renumerada 430 → 431 por colisão com a [[A40.l96]] em paralelo · **entra na cláusula de reinício do contador** (muta E5) |
| [[A40.l112]] | Imóvel sem classificação nenhuma entra no numerador da concentração pelo `else` | P2 | open | aberta 2026-09-01 no fecho da [[A40.l95]], que carregava o item sem lane id desde 08-29 e o **roteou** para cá · classe **distinta** da l95: captura, não metodologia · o `else` dos dois splitters não separa "classificado como algo que fica" de "nunca foi classificado", e o não-classificado entra no numerador — lado **conservador** e correto como default ([[ADR-420]] §D2), mas conservador **sem dizer que é** · medido no golden 2026-09-01, nos 4 regimes: sem override nenhum dá **82,19%** (o numerador inteiro é fatia não-classificada) e reclassificar **um** para residência leva a **75,93%** — **6,3 pp** de um clique de UI, sem nada na tela dizer isso · ⚠️ o "→ 0,00" que o §Follow-up da l95 citava era do golden **anterior ao #1904**, de imóvel único; re-medido aqui · escopo: estado **ternário** no molde da [[ADR-412]] §D2 + cobertura publicada + fixture que discrimina os três estados · ⚠️ **prevalência em produção NÃO-MEDIDA** (evidência é fixture sintética; o dogfood tem os 6 imóveis classificados) — exposição estrutural, não incidência  · ⚠️ **2026-09-01 (#1962):** a prevalência deixou de ser não-medida — no dogfood é **8 de 9 itens (89%) e 57,4% do valor** em `desconhecido` ([[ADR-433]] §D3). Os 6 overrides existem, mas são chaveados por `property_id` e 8 de 9 itens não o têm neste run |
| [[A40.l113]] | A identidade de imóvel churna entre runs e os dois classificadores falham **fechados**: residência e imóvel gerador publicados como zero | P0 | in_progress | aberta 2026-09-01 do `RR9-01` da **U5** · **medido A/B sobre corpus documental idêntico**: 95 de 400 escalares do payload se moveram, `bruto` **−48,1%**, `residencia` e `imoveis_geradores` **→ zero**, `if_gap` **+27,3%**, e o run saiu `completed` sem sinal bloqueante — nas 3 rodadas unificadas anteriores o mesmo corpus dava os números anteriores, logo é **regressão de 2 dias** · cadeia: descrição ganha caractere duplicado (churn LLM) ⇒ `endereco_canonical` e `property_id` colapsam **juntos** (5 de 7 → **1 de 9**; os 9 campos da consolidação seguem 9/9, o que prova que o enricher **rodou e falhou**) ⇒ `patrimonio_imovel_classifier.py:58-62` e `:76-78` mandam `pid` nulo para o `else` ⇒ residência e gerador **não recebem nada** ⇒ `investimentos_classes_analyzer.py:266` **falha aberto** e `nao_classificado_pct` sobe 3,93 → 5,61 pp **sem o numerador crescer** ⇒ dedup cross-IRPF ([[ADR-246]] passo 3b) **vira no-op** e 3 pares duplicados chegam ao publicado · o produto afirma que a família **não tem casa própria nem imóvel de renda**, sem aviso: os classificadores não distinguem "não é" de "não sei" · a [[A42.l15]] resolveu o eixo análogo para investimento ancorando em campo **estruturado** (efeito medido no mesmo run: `proprietario` 55 de 58, D2=0, D3=0) — **a população de imóveis não recebeu tratamento equivalente** · **entra na cláusula de reinício** (muta E5 a montante) · ⚠️ **a cadeia acima foi parcialmente REFUTADA por medição (PR #1962 `c551e832`, [[ADR-433]]):** o caractere duplicado não é a causa (8 de 9 descrições não têm via+número), o eixo do churn é a grafia do `codigo_rfb` entre anos, e `residencia` saía zero **com** `property_id` e override gravados — porque `anos_base_por_membro` elegia o ano por `max()` sobre a **união** das classes de ativo e aplicava 2026 (investimento) a imóveis/veículos/dívidas, que param em 2025 · a mesma linha zerava `veiculos` e `total_dividas`, logo a [[A40.l114]] **compartilha a raiz** · entregues o eixo por classe (`residencia` → 996.821,46, `total_dividas` → 230.459,13) e o estado ternário; seguem abertos a supressão do agregado e a âncora estruturada |
| [[A40.l114]] | O ano-base afirma um 31/12 que ainda não fechou, e o eixo zera imóveis, veículos e a dívida do titular | P0 | ✅ **#1961** | aberta 2026-09-01 do `RR9-02` da **U5** · `total_dividas` publicado **zero** com 4 financiamentos listados (Σ = 230.459,13 **na mesma página**); `score.componentes[taxa_endividamento]` `nota=10,0` · peso 18,8% · `status="emitted"`; "Endividamento Mínimo" como **Ponto Forte**; `liquido` idêntico ao bruto · ⚠️ **duas premissas do enunciado caíram na medição**: o LLM converteu exercício→ano-calendário **certo em 3 de 3** declarações, e o `2026` vem de um documento que **É de 2026** (tela de posição do Itaú de **29/03/2026**, descrita como tal no próprio `_meta.notes`) — logo o critério 1 ("reconciliar contra os anos observados") seria **INERTE**, porque 2026 *é* o máximo observado; o invariante que pega a classe é **temporal** · ⚠️ **crédito da manchete re-atribuído**: o `bruto −48,1%` sai inteiro do eixo de ano (2.012.174,02 ÷ 3.879.177,72 = 0,519, ao centésimo), mas a [[ADR-433]] da [[A40.l113]] mergeou antes e **resolve o corpus sozinha** — medido pós-rebase, os números ficam **idênticos** com qualquer um dos dois consertos desligado · entregue: **produtor único de saldo** (`saldo_divida_resolver`, Rota C do `financial-planner` — C1/C2 carry-forward declarado com direção do erro/C3 zero **declarado** como quitação), matando a contradição entre os dois leitores do mesmo `saldo_31_12`; `_total_dividas_for` (o defeito **dormente** que a lane pediu, errado nas **duas** direções); **tripwire** de contradição interna, que pega a classe **independentemente da causa**; **supressão no score** (`dividas_nao_apuradas`, `nota: null`, `status: suprimido`, peso fora do denominador, `piso` publicado e classificação de `min(valor, piso)`) + os 2 analyzers que reintroduziam o zero pela porta dos fundos; `review_reason` `domain.ano_referencia_nao_fechado` (WARN-first) · **3 rotas recusadas por medição**: rebaixar o ano no produtor (publicaria março como 31/12), filtrar o doc do E1.5 (perderia **R$ 322.865,96** — o stage próprio pega só a fatia de previdência e mislabela igual), e filtrar dentro da eleição por classe (quebrava o **controle de não-inércia** da [[ADR-433]]; virou residual nomeado) · contrafactual **reprovou** e revelou 2 consertos independentemente suficientes ⇒ alvo próprio para cada um; teste do piso validado **por mutação** · ⚠️ **o closeout achou um defeito da própria lane**: a supressão re-normaliza o denominador, o que a [[ADR-217]] §D2 (`Decidido`) proíbe, e **nenhuma ADR foi aberta antes do PR** — corrigido com [[ADR-437]] (decide, publica `score.piso`) + emenda datada de retratação parcial na [[ADR-217]] (aponta, e ratifica `absent_normalized`/`absent_penalized` intactos) |
| [[A40.l115]] | O sanitizer de PII mede o contexto de **entrada** e nunca o output: CPF parcialmente mascarado e conta bancária completa no publicado | P1 | shipped | aberta 2026-09-01 do `RR9-16` da **U5** · CPF com dígitos finais em claro em prosa **e** em `evidencia`; agência e conta **completas** na mesma página ⇒ a política protege 1 identificador e publica 2 · o gate mede o contexto que vai ao LLM, **nada mede o output**, e a docstring afirma "CPF/CNPJ redigidos" enquanto o conjunto coberto tem 1 elemento que não é nenhum dos dois — a asserção falsa **vale como justificativa de ausência de gate** · **SHIPPED** PR #1964 ([[ADR-435]]): eram **3** egressos, não 1 — o relatório já redigia (0 hits: o defeito era **vocabulário**), o parecer não redigia nada, e `suggestions` copia prosa do LLM verbatim. 7 hits medidos, 7→0, zero FP em 5.562 strings. 3 afirmações do enunciado caem (ver a lane) e a severidade do CPF parcial baixa para Médio |
| [[A40.l116]] | O guard de autocontradição erra a seção pela **terceira** vez, e o teste que o cobre importa a própria constante | P1 | ✅ **#1969** | aberta 2026-09-01 do `RR9-09`+`RR9-22` da **U5** · reincidência medida da [[A40.l80]]: o parecer **elogia** "Reserva Robusta" e **alerta** "Reserva Excessiva" sobre o mesmo objeto com `autocontradicao_removidos: 0` · `parecer_guardrails_divida.py:165` fixa a seção num **literal**; o #1800 trocou o literal em vez de derivar do layout, então o guard voltou a errar quando o rótulo do modelo mudou · **o achado novo**: `tests/test_parecer_guardrails_divida.py` **importa a constante** e monta a fixture com ela ⇒ o teste é **invariante ao valor** e não pode falhar — mesma classe de [[A42.l24]] · o contador não é falso: o **detector** não alcança par elogio × alerta |
| [[A40.l117]] | O parecer publica dois números para a mesma grandeza, cita a seção errada em 4 de 11 riscos, e o prompt se contradiz sobre ter ferramentas | P1 | ✅ **#1996** | aberta 2026-09-01 do `RR9-08`+`RR9-11`+`RR9-14` da **U5** · dois valores de "renda fixa na carteira" na mesma seção com **4,15 pp** de spread (um autoral, um carimbado) e nada reconcilia porque `parecer_prose_money.py:16` deixa percentual **fora** do invariante — medido: **27 percentuais em 47 campos de prosa, 0 sob invariante** · 4 de 11 riscos citam seção errada (2 de proteção citam imóveis; sucessão cita renda do IRPF) — ⚠️ **REFUTADA** na mesma medição a alegação de "âncora morta": as **27 citações resolvem, 27 de 27** · o YAML convida `get_e5_section` na linha 441 e nega ferramentas na **179**, e o convite morto é injetado sob budget — causa proximal dos 3 `campos_faltantes` · **desmembrada 2026-09-01** por arbitragem do `senior-cto`: o sintoma dos dois números sai para a [[A40.l120]] (é o único que move número publicado e exige rebaseline de golden). Ficam aqui o destino de citação e a contradição de `tools`. ⚠️ Duas correções de enunciado medidas: `campos_faltantes` são **8**, não 3 (o 3 é o subconjunto em seção evictada), e a linha 441 é o convite que **NÃO** está sob budget — o que está é o `_eviction_marker` (`parecer_distiller.py:273`). São **8** superfícies (6 prometem, 2 pressupõem; o "7" que publiquei era repetição não-conferida), e a persona é o original · ✅ **sintoma 3 MERGEADO** [#1966](https://github.com/davidrobert/mathoms/pull/1966) (`24a375eb`): as 7 superfícies param de prometer `tools`, [[ADR-341]] §D5 revogada e [[ADR-203]] emendada · **lane segue `open`** pelo sintoma 2 (destino de citação), bloqueado por 3 precondições nomeadas na lane — a 3ª é a ordem da cascata, cujo item decisivo foi contaminado por um título falso meu · sintoma 1 → [[A40.l120]] · ✅ **SHIPPED 2026-09-02 em 4 PRs**: sintoma 3 no [#1966](https://github.com/davidrobert/mathoms/pull/1966) `24a375eb` (8 sítios que prometiam `tools`, [[ADR-341]] §D5 revogada) e sintoma 2 no [#1996](https://github.com/davidrobert/mathoms/pull/1996) `1583cf97` ([[ADR-438]] — destino de leitura derivado pela máquina; no corpus real os 2 riscos de proteção migram S4→S9, a S9 sai de **0 para 4** riscos e o terminal fica em **0**) · sintoma 1 → [[A40.l120]] · a fusão da [[A40.l119]] foi avaliada e **recusada** com motivo, e o item 3 dela mudou por esta entrega · ⚠️ dos 4 critérios de aceite do enunciado **2 caíram por medição**, e 5 afirmações minhas foram refutadas — a pior, medir cobertura global do discriminador em vez de cobertura onde o default erra |
| [[A40.l118]] | Campo emitido **sem consumidor** pode carregar valor errado, e o gate de classe mede existência do leitor — nunca a corretude do número | P2 | open | aberta 2026-09-01 ao triar o `RR8-04` sob a §6.1 do runbook da rodada unificada; **lane de classe, não da instância** · o caso: `fluxo_caixa_enricher.py:592` tem `if v > 0:` e descarta o mês negativo da janela de 12m, reproduzindo **ao centavo** em 3 rodadas consecutivas em 1 de 12 fontes · ficou em `P3` por *"magnitude pequena"* e a régua mostra que `P3` e `P1` estão **os dois errados**: o campo **embarca** no payload publicado (12 chaves) e **nada o renderiza** — o único hit no frontend é a declaração de tipo em `report-fluxo.ts:155` ⇒ **`P2` latente**, porque o tipo declarado é exatamente **como** alguém liga o campo a um card, e aí o número errado publica **sem sinal** · a [[A40.l88]] entregou gate de **existência** de consumidor; ele mede o leitor, **nunca** o número, e schema valida forma, não invariante de valor · escopo: (1) agregador de janela não descarta valor por sinal sem declarar a exclusão, no molde da [[ADR-431]]; (2) inventário dos campos sem leitor **com o valor conferido**, publicando `n_conferidos` (zero ⇒ `INAPLICÁVEL`); (3) decisão por campo — ligar o leitor ou remover · ⚠️ **prevalência NÃO-MEDIDA**: a evidência é **um** campo; se o inventário achar zero outros, a lane fecha com o gate de pé e isso é resultado |
| [[A40.l119]] | O elogio à reserva tem **dois produtores** e o guard alcança um; e a ressalva reescreve a descrição sem tocar o **título** que o leitor vê | P2 | open | aberta 2026-09-01 no closeout da [[A40.l116]] (#1969) — os três follow-ups que a medição do conserto abriu e ele não fechou · **(1)** `parecer_planejador.yaml:699` projeta `$.pontos_fortes` **cru** e o modelo **ecoa** o título determinístico `"Reserva de Emergência Robusta"` descartando a ressalva da `descricao` ⇒ elogio (S10) × alerta (parecer) sobrevive ao guard consertado — **resíduo do `RR9-09`** · **(2)** `_com_ressalva` reescreve só `descricao`: no regime em que o piso de 3 amarra (**1 de 14 runs**) o título fica ao lado do risco · **(3)** `tema_canonico` é **opcional** em `PontoForte` e é a única âncora que restou — medido **0 em 64**, fica contado com gatilho mecânico (`autocontradicao_tema_ausente > 0`) · mesmo eixo da [[A40.l117]], fora dela porque o `#1966` estava em voo sobre o arquivo |
| [[A40.l120]] | O parecer chama de renda fixa uma soma que inclui previdência, e o número com que ele deveria concordar não chega até ele | P1 | ✅ **#2003** | desmembrada 2026-09-01 da [[A40.l117]] (`RR9-08`/`PV13-08` da **U5**) · o discriminador **refuta o denominador**: `Caixa = 0`, e `90,25 + 4,14 = 94,39` é o carimbo ao centésimo — **o spread de 4,15 pp é a linha Previdência** · duas causas: `alocacao_alvo_deviation.py:17` dobra **toda** previdência em renda fixa sem declarar no rótulo, e `$.goals` só é projetado na seção que **foi evictada** neste run, então o modelo nunca recebeu o número · ordem obrigatória: projetar → corrigir o hint 357 → só então gatear (gate antes reprova o insolúvel, [[ADR-292]]) · ✅ **SHIPPED 2026-09-02** — não era defeito de prosa: `$.goals` só existia na seção **evictada**, então o modelo nunca recebeu o número com que devia concordar. Entra a projeção do comparável, o rótulo passa a declarar que previdência está inclusa (veredito de domínio: permanece agregada pelo critério da [[ADR-141]], não por consenso — classificar pelo subjacente foi rejeitado por **falta de insumo medida**) e nasce o gate de coerência, com tolerância = meio passo da precisão escrita e **nenhum limiar novo** · ⚠️ roteado e NÃO entregue: o guarda de flip do `next_aporte_classe`, que muda cálculo e exige ADR própria |
