...
| **Race no readiness do backend no `frontend-e2e`** — o step "Start backend" faz `sleep 5` fixo + UMA tentativa de `curl -sf /health`; quando a importação da app passa desse orçamento o job morre com "backend não subiu" e o Playwright **não roda nenhum teste** (medido 2× consecutivas no #1277; o artefato `backend-logs` mostra uvicorn e celery subindo com sucesso). Mecanismo distinto da linha "não gateia" acima: aqui o job é vermelho **sem carregar informação de teste** | — | **PR em voo** — #1283 (*"probe de readiness do backend ganha orçamento"*). O fix pedido era loop com timeout em vez de `sleep`, e mensagem que distinga "não subiu" de "subiu depois do orçamento" |
