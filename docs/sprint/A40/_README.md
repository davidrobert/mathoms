...
| [[A40.l114]] | O ano de referência é saída **crua do LLM** e o total de dívida vira zero quando esse ano não existe em documento nenhum | P0 | open | aberta 2026-09-01 do `RR9-02` da **U5** · `total_dividas` publicado **zero** com 4 financiamentos listados; `percentual_patrimonio` zero; `score.componentes[taxa_endividamento]` `nota=10,0` com peso **18,8%** e `status="emitted"`; superfície publica "Endividamento Mínimo/Nulo" como **Ponto Forte** em 2 inventários; `liquido` **idêntico ao bruto** · **três totais de dívida coexistem** nos artefatos do run · cadeia: `pipeline/stages/extract_baseline.py:96` faz `"ano_referencia": output.reference_year` com `_meta.source: "E1.5-llm"` e **nada confronta com os anos dos documentos** — o derivado do documento é computável na **linha 173 do mesmo arquivo** ⇒ `consolidate_baseline` re-rotula `patrimonio_por_ano` com conteúdo **byte-idêntico** ⇒ `patrimonio_resolvers.py:561` `_split_dividas` lê `saldo.get(ano_ref, 0)` **sem fallback** · o irmão `_resolve_saldo` ([[ADR-401]]) **tem** fallback, e é por isso que a lista soma certo **na mesma página** — dois leitores do mesmo campo discordam sobre ano ausente · ⚠️ **correção de atribuição**: o comentário em `endividamento_analyzer.py:138` nomeia `_total_dividas_for`, que é defeito real mas **dormente** (o objeto por ano já existia quando o número saía certo) · a [[ADR-431]] da própria sprint dá o remédio — zero publicado é afirmação sobre o patrimônio — aqui na direção **inversa**, subdeclarando passivo · **entra na cláusula de reinício** (muta E5 a montante) |
