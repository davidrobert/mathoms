...
- **Achados novos, registrados (não "a registrar") em 2026-08-06.** Descrição sem
  ...
  |---|---|---|
  | `dedupeBySemanticKey` (`curadoriaDestaques.ts`) descarta item da lista **sem declarar** — colapso por regex sobre texto, *first-wins*, então a **ordem** decide quem sobrevive | **esta lane** | **Devolvido 2026-08-15** no closeout da [[A40.l22]]: a l22 shipou o parecer; o dedupe é de destaques S10 (`PontosFortesCard` / `PontosUrgentesCard`) e ainda vive. A linha de 2026-08-06 que apontava para a l22 fica como rastro |
