...
- **`ReportSection`** — `grid-cols-1` explícito. O track era `auto` e crescia
  ...
  abaixo de 768px, atingia o PDF inteiro ([[ADR-379]] D5). **Sem `min-w-0` nos
...
- **`VariacaoSection` (V0)** — tabela vira lista rótulo/valor abaixo de `sm:`
  ...
  e `aria-label` preservados juntos ([[ADR-379]] D4). O rótulo passa a quebrar e
...
- **`hidden md:block` como classe.** Hoje `alocacaoCardParts` e
  ...
  desenho. [[ADR-379]] D1 fixa a regra; a varredura dos call-sites existentes
