...
| [[A42.l15]] | `investment_id` é hash de campos que o extrator LLM reescreve — **23,5%** de estabilidade entre runs; o comparador dispara uma perna diferente a cada par consecutivo · **U2 `LC6-02`** · ✅ **#1937/#1939** — defeito procede. Estabilidade **37,68% → 61,78%** (pooled `|A∩B|/|A∪B|`, 836 artefatos / 28 grupos; mediana 29,19% → 68,12%), ancorando no CNPJ do documento — o `PR3` que a [[ADR-271]] §147 deferiu em 2026-05. **A âncora não custou token de LLM:** o CNPJ já estava no texto da `descricao` em metade dos casos, e ler o campo ⊳ o texto dá a mesma raiz ⇒ o bump 1.4.0 não deixa 91,6% do corpus órfão e a §Armadilha (B) morre. **O bloqueio humano do PR1 era pergunta mal-posta** — a variável operante é o consumidor (indexado por grupo), não a forma que o extrator emite; #1937 recuperou 96 fundos que caíam no balde genérico. **Duas correções ao plano por medição:** `instituicao` FICA na perna fraca (tirá-la paga +7,4pp e funde o mesmo título em duas corretoras — some patrimônio, [[ADR-271]] §139), e a âncora **não** se compõe com `tipo` (63,49% < 69,20%) | **P0** | 1 | — |
...
| Resíduo da §Armadilha (C) da [[A42.l15]]: consumidor que compara código RFB de 2 dígitos estritamente e **falha aberto** — `codigo_rfb` composto no resolver de imóvel (`db_property_identity_resolver.py:134,168`), `wise_fiscal_flags.py:32,35,38` (`==` contra `"13"/"62"/"41"`), e `dividas_dedup` sem o gate de acoplamento do critério 4 | **esta seção** (sem lane) | **Latente, não vivo** — `property_id` é UUID resolvido contra o DB e mede 100% de estabilidade; o #1937 fechou a ponta viva (`normalize_grupo` + catálogo). Lane nova violaria o §Critério de admissão: não alcança o usuário hoje. Retomar se o composto passar a aparecer em `dividas_onus` (zero hoje, 836 artefatos) ou se a cobertura de `GG-CC` sair dos 1,82% |
