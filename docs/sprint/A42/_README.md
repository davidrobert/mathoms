...
| [[A42.l22]] | A **ETA exibida durante o run** mistura no-ops de milissegundos com execuções de minutos: a mediana filtra `status==completed` e o no-op auto-declarado grava `completed` · **U4 `PV12-01`** — ⚠️ os **72%** e a superfície de usuário **não se encontram**: o número reproduz só sem a janela de 20 da produção, e o stage medido não emite ETA (medido na entrega) · ✅ **#1959** — o corte é de **população**: a ETA só é lida no ramo em que o stage executa, então a amostra de no-op nunca pertenceu ao evento previsto. O `PV13-15` foi roteado aqui e **não é o mesmo defeito** — o extremo de 5.766× vem de `total_processed > 0`, a **2ª grafia de "zero trabalho"** que a [[A42.l21]] já havia nomeado; segue **aberto e sem dona** | P2 | 1 | — |
...
| [[A42.l24]] | Três checks da rodada publicam verde sobre população em que a falha é **impossível por construção**, e um exclui justamente o stage sob suspeita | P1 | ✅ **#1958** | aberta 2026-09-01 dos `LC9-04`/`LC9-05` (§r9) e `PV13-10` (§r13) da **U5** — a origem citava `LC9-10`, que é a sonda de imóveis da [[A40.l113]], achados do instrumento contra si mesmo · X4: dos 10 literais monetários do parecer, **9** vivem em campo que o backend preenche copiando `path → valor` do **mesmo payload** que o check relê ⇒ órfão impossível; superfície **autoral** é **n=1**, e eu publiquei `FECHA ✅ n=10/10` · X5: examina **17 de 18** stages e o excluído é o que constrói o payload com a regressão da rodada — o check que consertei para "poder sair verde" sai verde ignorando o suspeito · teto de iterações de ferramenta: **19 contra 6** sem alarme, porque emissor e teto contam populações distintas (as 19 são carimbos do backend, **zero** do modelo) · o anti-vácuo do runbook publica `n_comparado`/`n_esperado` e **funciona**; o que não pega é **denominador tautológico** — `n_esperado` alto **parece** cobertura · mesma classe do `LC8-01` e da [[A42.l21]] · **entregue #1958:** `veredito()` exige um **terceiro** denominador (`n_falsificavel`, keyword-only — opcional é como a guarda ficaria inerte) e publica a fração dele sobre o examinado; X4 `10/10` → **n_falsificavel=1 (10%)**, separando carimbado de autoral pelo resolvedor do **próprio estampador** e por **ocorrência** (o único literal autoral repete o mesmo número de uma âncora, e subtrair por valor o apagaria); X5 `17/17` → **`18/18`**, com a exclusão nomeada no veredito e não no denominador; **X7** novo mede o teto contra a população que o emissor conta, por assinatura (`LLMService.call` não aceita `tools` ⇒ round-trip estruturalmente 0 ⇒ `INAPLICÁVEL`, e isso é resultado) · ⚠️ **o `INAPLICÁVEL` com `n=1` que o critério 2 prescrevia não procede** — `riscos[].evidencia` não é sobrescrito, logo *pode* reprovar; o rótulo apagaria medição verdadeira e a proteção real é publicar os 10% |
| [[A42.l25]] | A mesma linha do razão publica o delta com **dois sinais opostos**, e o sinal observado é o único que nenhuma perda de pipeline produz | P1 | shipped | aberta 2026-09-01 dos `LC9-06`/`LC9-07` da **U5**; sucessora direta da [[A42.l18]], que **ligou** a perna de valor — o número destravado é o objeto desta lane · `dev/ledger_conservation.py:220` calcula `value_in - value_out` só para o texto ⇒ campo negativo e detalhe positivo na **mesma linha**, e a direção do viés é **irresolvível a partir da saída** (dois leitores honestos do painel chegaram a conclusões opostas) · destino > origem é o sinal que **perda não produz**; dois mecanismos de instrumento nomeados: **dois parsers do mesmo campo** (`"1.234"` lido como inteiro num lado e decimal no outro, fator 1000×) e **quatro termos com duas convenções de sinal** (o delta é líquido de opostos) · enquanto ambíguo, a perna de valor **não sustenta veredito** · ✅ **[#1965](https://github.com/davidrobert/mathoms/pull/1965)** ([[ADR-434]] + emenda datada na [[ADR-426]]) — o eixo **fecha ao centavo** no corpus real (`Δvalor(destino−origem)` **−1.998.772 → 0**, veredito `coberto-sem-verificação-de-valor` → **`conservado`**). ⚠️ **A segunda metade do título é FALSA e foi refutada por medição:** o campo publicava destino **MENOR** (`out − in = −1.998.772`), que é justamente a assinatura de perda — o `LC9-07` leu o **detalhe** (`+1.998.772`) e concluiu "destino maior". Ele é vítima direta do `LC9-06`, o achado irmão. A conclusão dele (*não é perda*) sobrevive, mas por medição de mecanismo, não pelo sinal citado. O mecanismo **1** (dois parsers, 1000×) cai na magnitude e procede na **classe** (`cents_int` vs `Decimal(str(v))`, meio-centavo); o mecanismo **2** (quatro termos, duas convenções) é 100% do gap: 48 receitas negativas, `2 × 999.386 = 1.998.772` exato. **Acrescentado pela medição:** o Δ era **offset constante**, não resíduo — uma perda real do mesmo tamanho publicaria `Δ = 0` e o veredito diria `conservado` (canal de mascaramento). As 48 receitas negativas viram **evidência de produção** para a [[ADR-429]] / [[A40.l102]], que hoje só tem fixture |
| [[A42.l26]] | O guard de escrita do E4 passa com **zero erros** e não mede profundidade: item vazio, campo lixo e número-como-string atravessam | P1 | shipped | aberta 2026-09-01 do `PV13-13` da **U5**; sucessora da [[A42.l19]], cujo conserto **segura** (o balde de patrimônio resolve e valida 0 erros) — a lane é o **limite** que a medição revelou · o contrato re-derivado tem `required`/`additionalProperties:false` **só na raiz**; o item de `imoveis_consolidados` sai com `required` nulo · **a regressão de identidade da [[A40.l113]] passou por este guard**, e não por acidente de configuração: ele não olha o grão do item, que é onde a identidade mora · "0 erros" sem grão declarado não é veredito  · **entregue** [#1968](https://github.com/davidrobert/mathoms/pull/1968) (`e3f595a7`), [[ADR-436]] · **a medição corrigiu o enunciado:** num→str em chave que **casa** o pattern do ano **já reprovava** por tipo — o vetor vivo era a chave que **não** casa, porque `valores_31_12` não tinha `additionalProperties`; e não eram 3 casos, eram **8** (item vazio nas três coleções + três em `patrimonio_por_ano`) |
...
| [[A42.l20]] · [[A42.l21]] · [[A42.l22]] | 2026-08-30 | os P2 da `U4`, alocados a pedido do dono | #1867 (lanes) · #1907 (l20) · #1906 (l21) · **#1959** (l22 — `shipped` 2026-09-01) |
...
| `LC7-02` do [[LEDGER-CERTIFY-active]] §r7 — a contagem publicada de `investimentos_consolidados` oscila com o corpus documental FIXO. Re-medido 2026-09-01: amplitude **10 (~17%)** sobre 62 runs, não os 38 do enunciado (a maior parte era descoberta, não churn); a [[A42.l15]] moveu **zero** nesta métrica (σ 5,72 → 5,67) | **era 1.4.x com K≥5** — o driver que sobra é `descricao` e o remédio (regra de formato) entrou no bump da l15 sem medição; o corpus tem zero artefatos nessa era | `dev/measure_e15_identity_stability.py` |
