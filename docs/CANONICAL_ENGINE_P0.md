...
1. Golden **por estágio** até E6 (E3→E4→E5→E5.N→HTML), incluindo **despesa categorizada**, **baseline patrimonial mínimo**, **`test_e5n_golden_execution`** (narrativas + `validate_narrativas`, incl. tenant com cônjuge / chart `ana_cenarios`), **`test_e6_golden_execution`** e assert de **`logs/qa_log.md`** (`pipeline_golden_asserts`); próximo incremento: **mais layouts PDF por banco** (além de BTG, Rico, Wise, PicPay, Bank of America, Santander, Itaú e Caixa — §4 item 2) ou fixtures LLM determinísticas (§4 item 3).
2. PDFs sintéticos + registry E2: `tests/test_e2_synthetic_pdf_parsers.py` (filename canônico por banco → `route_to_parser` → parse); `caixa` no `pdf_generator`. Layouts dedicados no gerador + testes com **≥1 transação** e **`saldo_final`**: **BTG**, **Rico**, **Wise**, **PicPay**, **Bank of America**, **Santander**, **Itaú**, **Caixa** (`test_btgpactual_*`, `test_rico_*`, `test_wise_*`, `test_picpay_*`, `test_bankofamerica_*`, `test_santander_*`, `test_itau_*`, `test_caixa_*`). Smoke texto: `backend/tests/test_golden_pipeline.py::TestSyntheticPDFsAreParseable`. **Gap restante:** layouts dedicados para os **demais** bancos do registry (incremental) — **prioridade:** fechar esta abordagem **só sintética** primeiro.
3. LLM: respostas gravadas para integração determinística (alinhar [ADR-070](DECISIONS.md#adr-070--premium-llm-e2e-mock-default--nightly-real-opt-in)).
4. **Depois** do sintético completo por banco-alvo: segunda linha de defesa com **PDFs reais anonimizados** no repositório (processo de redação, revisão de PR, lint PII/metadados, testes de regressão de layout opcionais) — ver [PIPELINE_ARTIFACTS.md](PIPELINE_ARTIFACTS.md) § *E2 — sintético e real anonimizado* e [BACKLOG.md](BACKLOG.md).
