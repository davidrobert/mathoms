...
>
> **Balanço da onda R2 (2026-07-15):** dos 12 itens curados — **11 shipados** (PD-02, CTO-02, CTO-06 [R2.1]; FP-03, CTO-04, PD-01, CTO-05 [R2.2]; DE-02, **FP-02** (score_version 2.0 plateau), **FP-04** (label morto), DE-01-Fase1 [R2.3]); **DE-03** design travado (lane write-path, P3, não é code-only). Deferidos com gate explícito: **FIN-05** → [[ADR-340]] `Proposto` (`score_version 2.1`, co-design feito — gated pelo C11-Fase2; não confundir com FIN-01, já resolvido via ADR-333); **DE-01 Fase 2** (drop pós soak ≥1 mês / owner); **DE-03** ([[ADR-339]], write-path). FIN-01 do plano original era input fix, não parte do bump de fórmula.
