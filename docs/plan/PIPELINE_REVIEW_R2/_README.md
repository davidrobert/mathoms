...
| RV2-01 | parecer FABRICA métrica em `metricas[]` (valor não derivável de nenhum campo E5); itens sem âncora escapam do verify (`parecer_evidencia.py::_iter_items` só itera riscos+sugestões) | ADR nova, ID a alocar na escrita (extende [[ADR-296]]; o "ADR-354" antes reservado aqui é da [[A40.l2]] desde #1114) | ADR novo | 🔬 co-design prompt-engineer **feito** · **BIFURCA (2026-08-21)**: a perna do `target` fechou por [[ADR-399]] — o bloqueio declarado ("dep. catálogo KPI") **não procedia** para esse eixo: `metricas[]` não tem campo de âncora e o catálogo é *input-side*; 9/10 dos paths de `target` resolvem em `get_e5_jsonpath` e **0/10** estão no catálogo. Segue aberta só a perna "item sem âncora escapa do `_iter_items`", que é o **PE-1** do §r7 — e que **deixa de ser bloqueante** |
