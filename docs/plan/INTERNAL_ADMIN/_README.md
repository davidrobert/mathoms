...
| **Documentos** | Apagar documentos de um **usuário** ou **workspace** deletando registros no BD **e** objetos no **storage** (`stored_path` / camadas em [storage](../../../backend/app/services/storage/__init__.py)); modo **"prévia"** lista o que seria removido antes de confirmar. |
