...
| **1 — Contrato aditivo + substrato de golden** | **`dl-f1-golden-substrate`** (P0 — `dev/golden_diff.py` valor-a-valor cents int + snapshot do view-model de `/reports/[id]/data` + invariantes de conservação; **fecha DE-005**; ANTES de F2 tocar golden) · `dl-f1-natural-key` (K4+moeda+direction+hash_version, B3/B4 · [[A23.l3]] ✅; **passo 2 gated por [[ADR-282]]** — unificar identidade de `TransactionOverride` no v2 antes do flip de consumo E4, senão orfaniza override em massa) · `dl-f1-data-source` (tabela+coluna+SourceRef) · `dl-f1-amount-decimal` (B5) · `dl-f1-extract-check` · `dl-f1-migration-runbook` (G-e) | P0/P1 | **G1:** goldens E3/E4/E5 **+ snapshot do view-model + invariantes de conservação** verdes **sem rebaseline** (aditividade) |
