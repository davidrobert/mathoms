...
(`failed=0` + 2º dry-run com `rotated=0`), aprovação da janela de FREEZE (W3→W8). **[[ADR-319]] e [[ADR-320]]
...
- **G0:** 8 ADRs mergeadas com decisão textual; backup mirror clonável de teste; tag
  ...
  `failed=0` **e** 2º dry-run com `rotated=0` (ver [[A34.l3]] §Critério — por
  que as duas condições são necessárias, por que `skipped` é ambíguo e por que
  `old_key_decryptable` não existe).
