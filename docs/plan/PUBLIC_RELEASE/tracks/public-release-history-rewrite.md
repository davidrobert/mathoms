...
| 0.2 | Rotação Fernet confirmada em prod ([[A34.l3]] · [[ADR-171]]) — `failed=0` + 2º dry-run com `rotated=0` | Saída da task `rotate_fernet_secrets` (`celery -A backend.app.worker call …`, runbook `fernet_rotation.md`): `failed` zerado em todos os targets **e** um segundo dry-run sem nada a rotacionar (o 1º sozinho não prova que um lote interrompido terminou; `skipped` é ambíguo — [[A34.l3]] §Critério). O rewrite remove o **blob** do `.env`, mas a key só é inócua se rotacionou em prod (não verificável do repo) | Owner |
