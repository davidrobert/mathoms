...
5. **O Redis da suíte é o mesmo do dev.** Não há fake global: chave **global**
   lida num teste devolve o que o dogfood gravou, e o que o teste gravar fica
   lá até o TTL (o catálogo de instituições vive 30 dias). O `conftest` já
   desliga o cache do catálogo (`_institution_catalog_no_redis`) e o
   rate limit; ao tocar outra chave global — `categories:latest_template_version`,
   por exemplo — isole com
