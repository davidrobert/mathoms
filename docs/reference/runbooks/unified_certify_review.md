...
| **X4** | Todo literal monetário **autoral do modelo** — não o carimbado pelo backend — pertence, em cents, ao conjunto de cents do E5 do mesmo run, ou deriva de fórmula declarada | Número ancorado pelo modelo, não pelo dado. ⚠️ **O denominador é a metade difícil:** medido no `U5`, **9 de 10** literais vivem em `riscos[*].ancoras[].valor_renderizado`, que `stamp_ancora_values` preenche copiando `path → valor` do **mesmo** payload que o check relê ⇒ órfão **impossível por construção**, e o `FECHA ✅ n=10/10` publicado era vazio. A superfície autoral é **n=1**. **Corrigido na [[A42.l24]]:** o check separa carimbado de autoral pelo resolvedor do **próprio estampador** e por **ocorrência** (subtrair por valor apagaria a cópia autoral do mesmo número), e publica `n_falsificavel`. Medido no run do `U5`: `FECHA ✅ n_comparado=10 n_esperado=10 n_falsificavel=1 (10% do examinado)`. ⚠️ **O `INAPLICÁVEL` que esta linha prescrevia não procede** — `n=1` *pode* reprovar (o literal vive em `riscos[].evidencia`, que nada sobrescreve) e não reprovou; `INAPLICÁVEL` aí seria apagar medição verdadeira. O que protegia o leitor era publicar os **10%**, não trocar o rótulo. **Buraco de cobertura declarado:** só `R$` explícito — prosa monetária por extenso ("350 mil") fica fora e **não** está contada. Instrumento: `dev/unified_xchecks.py <ws> <run> x4 <parecer.json> <vm.json>` |
| **X7** | O teto de iterações de tool compara a **população que o emissor conta**. `max_tool_iterations: 6` ([[ADR-203]] §D2) governa round-trips `tool→LLM→tool`; `_meta.tool_iterations` conta **toda** invocação da instância `PlannerDrillDown` — cache hit e estampagem pós-LLM de âncoras ([[ADR-296]]) e métricas ([[ADR-399]]) — logo cresce com o tamanho do parecer | `PV13-10`: o `U5` leu `19` contra teto `6`, estourado 3,2× **sem defeito no run**. Emissor e teto contavam coisas diferentes, e a semântica canônica já estava decidida (OBS-1 · A37.l1 · [[ADR-341]]) — quem não a acompanhou foi o `maximum: 6` do schema do artefato, anotado no campo do **emissor** (corrigido na [[A42.l24]]). O oráculo é por **assinatura**: `LLMService.call` não aceita `tools` ⇒ round-trip iniciado pelo modelo é estruturalmente **0** ⇒ `INAPLICÁVEL`, e isso é resultado. Ligar tools inverte o oráculo sozinho, e aí o check exige `fase` no trace antes de voltar a medir. Instrumento: `dev/unified_xchecks.py <ws> <run> x7 <parecer.json>` |
...
| ☐ | Todo cross-check publica `n_comparado`, `n_esperado` **e** `n_falsificavel`; zero em qualquer um ⇒ `INAPLICÁVEL`, nunca ✅. O par responde **cobertura** e não responde **poder**: no `U5` o `X4` fechou `10/10` sobre população em que **9 de 10** não podiam falhar ([[A42.l24]]) |
...
- **2026-08-29 (fecho do `U2`) — seis furos, e quatro deles são o MESMO furo.** A rodada
  ...
  2. **Meça o denominador antes de imprimir o veredito.** Duas das três correções da F3.a
     ...
     **Regra:** todo cross-check publica `n_comparado` **e** `n_esperado` na mesma linha do
     ...
     > **Emenda 2026-09-01 ([[A42.l24]]) — o par não bastava.** Ele mede *cobertura* e é
     > cego a **denominador tautológico**: população grande, verdadeira e **incapaz de
     > exibir a falha**. Os três casos do `U5` publicaram os dois números e passaram — o
     > `X4` fechou `10/10` com 9 literais carimbados pelo backend a partir do mesmo payload
     > que o check relê; o `X5` publicou `17/17` num run de **18**, e o excluído era o
     > stage em `needs_review` sob suspeita naquela rodada; o teto de `tool_iterations`
     > lia `19 > 6` contra uma população vazia. A guarda passa a exigir um **terceiro**
     > número — `n_falsificavel`, quantos elementos **podiam reprovar** — e a linha publica
     > a fração dele sobre o examinado. Zero ⇒ `INAPLICÁVEL`. `veredito()` o recebe
     > **keyword-only e obrigatório**: parâmetro opcional é exatamente como esta guarda
     > ficaria inerte.
