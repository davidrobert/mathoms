...
```
/audit-vault --full — escopos plan, sprint, claude, prompt e root
(rode o coletor uma vez por escopo; consolide numa única seção rN)
```
...
**Decida antes de rodar**, com o dado das fases 1-2:
...
```
/audit-vault --scope adr --full — julgue apenas o sub-lote N/5
(ordene por status Proposto/Roadmap primeiro, depois Decidido por id)
```
