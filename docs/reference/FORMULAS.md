...
| `concentracao_alerta_pct` | 50% | 45-60% | Base **carteira** ([[ADR-340]]): leituras ~1,5-2× as da base líquido antiga (a carteira exclui residência/veículos), então o limiar subiu de 40→50. ⚠️ **Emendado em 2026-08-31 ([[ADR-420]] §D4):** com o numerador estreitado para `imoveis_alocacao` esse "1,5-2×" deixa de descrever a leitura corrente — `uso_pessoal` e `nu_proprietario` saíram dos DOIS lados. **O limiar 50 não se move**: a procedência dele é doutrinária (a banda 40–60 em que as três referências divergem legitimamente, encodada em `risk_trigger_registry`), não função do numerador — e numerador menor sobre o mesmo limiar é MAIS conservador, não menos. Provisório (N=1, âncora real 60%), recalibrar com N famílias. Co-threshold `spread_critico` sobe 30→45. RL-7 (hard-block) sobe 60→75. |
