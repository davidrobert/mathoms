...
| Meta TRS (yield-alvo) | Referência consagrada de **5% a.a.** (yield diversificado de carteira de renda). `RentabilidadeConfig.meta_pct` é configurável **no código**, mas hoje `RatiosCalculator()` roda com o default e o `trs_pct` do wizard nunca chega ao calculator — o número exibido não é o da família (residual declarado em A40.l4). | `ratios.rentabilidade.meta_pct` |
| Yield-alvo × taxa de retirada segura ([[ADR-191]] emenda 2026-07-15 · FP-03) | **Dois conceitos distintos, nunca colapsar.** *Yield-alvo/TRS = 5%* (`trs_pct`) — retorno-alvo/observado da carteira geradora (acumulação); rege o card de Rentabilidade e o rendimento da meta patrimonial. *Taxa de retirada segura/SWR = 4%* (`taxa_retirada_segura_pct`, regra dos 300 / Trinity ×25) — taxa prudente de decumulação; rege a **estimativa** de renda passiva sobre o investível atual (`renda_passiva_estimada_4pct`) e o dimensionamento conservador da meta de IF. | **Yield-alvo:** `ratios.rentabilidade.meta_pct` (a casa que o card S3 e o card de TRS da S7 leem). **SWR:** `goals.taxa_retirada_segura_pct`. `goals.trs_pct` **não existe** no payload — o E5 emite a meta configurada como `goals.if_trs` (`if_projector.py`), e ler `goals.trs_pct` produziu o `?? 5.0` do S7 corrigido em A40.l4 ([[ADR-356]] §D8). |
