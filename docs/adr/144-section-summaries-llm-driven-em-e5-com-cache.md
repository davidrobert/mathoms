...
- **Storage:** Redis (preferido — já usado em ADR-111 cache layer, ADR-117 invitation rate limit). Adapter pequeno em `backend/app/services/storage/llm_cache.py` com interface mínima (`get(key) -> Optional[str]`, `set(key, value, ttl_s)`).
...
2. `LLMCacheBackend` protocol + `RedisLLMCache` / `PostgresLLMCache` em `backend/app/services/storage/llm_cache.py`.
