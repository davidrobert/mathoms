...
1. **Caminho 1 é o default proposto** quando algum dos 4 gatilhos disparar. Razões:
   ...
   - Mantém domínio Python intacto — goldens, ADR-090/097/090 e regras de domínio em [docs/reference/ARCHITECTURE.md §4.1 Domain glossary](../reference/ARCHITECTURE.md) continuam autoridade única.
...
4. **Pré-requisitos do Caminho 1, ordem obrigatória:**
   ...
   - **A3.cli** — ✅ **entregue** (2026-07-02, PR #737 — Fase 1 do track [a3cli-orchestrator-cli](../plan/GO_SHELL/tracks/a3cli-orchestrator-cli.md)): entry-point `python -m pipeline.orchestrator run-stage <stage> --workspace <path> --run-id <id> --workspace-id <id> [--config-dir <path>] [--incremental] [--incremental-doc <path>...] [--base-run-id <id>] [--base-run-fallback-stages <csv>]` em `pipeline/cli_run_stage.py`. JSON do `StageResult` puro em stdout, erros estruturados em stderr, exit codes 0/1/2. Injeção de `DBArtifactStore` por-stage via `backend/app/services/artifact_session_factory.py` (a sessão vive no backend — ADR-256), com **`MATHOMS_DATABASE_URL`** obrigatório no env (prefixo canônico do backend; a emenda item 6 dizia `DATABASE_URL` genérico). `--workspace-id` é adição à assinatura original (tenancy do store, [[ADR-303]] D3); `--base-run-id`/`--base-run-fallback-stages` dão paridade com o delta aditivo D2.
   - **A3.cli.otel** — ✅ **entregue** (2026-07-02, PR #738 — Fase 2 do track): CLI restaura o contexto W3C de `TRACEPARENT` (span `pipeline.<stage>` nasce filho do trace do chamador) e inicializa o provider do backend com `OTEL_EXPORTER_OTLP_ENDPOINT` (ADR-110), ambos best-effort. `_run_stage` emite os 6 attributes canônicos (`pipeline.stage/workspace_root/run_id/is_llm/success/exit_code`) em todo caminho — antes `success`/`exit_code` só apareciam nas falhas.
   - **A3.cli.benchmark** — ✅ **medido, gate PASSA** (2026-07-02, track [a3cli-benchmark](../plan/GO_SHELL/tracks/a3cli-benchmark.md)): cold start (overhead fork+exec+imports = parede do subprocess − `duration_ms` do stage) **mediana 413ms ≤ 500ms → Caminho 1 segue**; p95 830ms (contenção pontual), acumulado projetado 4,1s/run determinístico (10 stages) e 7,4s/run full (18) — dentro da estimativa 6-13s abaixo, abaixo do limite de risco de 10s. Import dominante: cadeia backend/SQLAlchemy do `DBArtifactStore` (~440ms), inerente ao boundary DB-only ([[ADR-212]]/[[ADR-303]]). Método + números completos: [PERFORMANCE_BASELINE §11](../reference/PERFORMANCE_BASELINE.md); script reproduzível `dev/benchmark_run_stage_cold_start.py` (refalsificar na revisita 2027-Q2 / 100 workspaces).
...
- [docs/reference/GO_PORT_DEPS.md](../reference/GO_PORT_DEPS.md) — A1, inventário de dependências.
