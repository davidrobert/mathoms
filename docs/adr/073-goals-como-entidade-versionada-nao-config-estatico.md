...
6. **Migração do `goals.json` de exemplo** — one-shot script em `backend/app/scripts/seed_if_goal_example.py` cria registro inicial para a workspace existente com `renda_passiva_mensal_brl=10000, trs_pct=5.0, retorno_real_anual_pct=6.0` → `derived.if_meta_brl=3000000` (paridade bit-a-bit com valor legado).
