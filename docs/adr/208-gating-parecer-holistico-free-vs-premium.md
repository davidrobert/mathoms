...
- ~~Endpoint `GET /workspaces/{id}/reports/{report_id}/planner-review` lê `workspace.tier` (existing column).~~ *(Correção audit r6, 2026-07-03: não existe coluna `tier` em `workspaces` — o tier é **derivado** da config LLM/BYOK via `resolve_llm_tier_async`/`_classify_llm_config` em `backend/app/services/pipeline/pipeline_service.py`.)*
