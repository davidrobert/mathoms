...
- **E2 PDF Caixa layout (2026-04-17):** `pdf_generator` — `_draw_caixa_extrato` (`Conta`/`Período dos lançamentos`/`SALDO ANTERIOR` + tabela 7 colunas C/D + linha `SALDO DIA` para `parse_caixa`); `test_caixa_synthetic_extracts_transactions`. Próximo: demais bancos do registry no mesmo padrão incremental ou fixtures LLM (CANONICAL_ENGINE_P0 §4 item 3).
