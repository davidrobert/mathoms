...
- **feat(report): A8.4 PR3 — APP_C "Cenários de Estresse" (hide-when-empty + visualização comparativa) (2026-05-06):**
  Apêndice C renomeado de "Cenários Alternativos"/"Cenários de Sensibilidade" para **"Cenários de Estresse"** (terminologia CVM/Susep, decisão D2 do plano A8.4 com product-designer). Comportamento `hide-when-empty` (D4): quando `data.cenarios_conjuge` ausente (gate ADR-167 retorna `False`), o componente retorna `null` — APP_D continua rotulado "D" porque a numeração no YAML é literal, não recomputada. Subtítulo CVM/Susep ("Não são previsões — testes de resiliência para validar a margem de segurança do plano atual"). Visualização (D3) comparativa lado-a-lado base vs estresse com delta explícito (sinal+cor para a11y AA) + parágrafo "Leitura:" justificando o stress test em tom não-alarmista.
  Novo componente `frontend/src/components/report/cards/StressScenarioCard.tsx` (extraído para preservar limite ≤500 linhas em `ApendicesSections.tsx`). Frontend agora lê **só** `data.cenarios_conjuge` — fallback dual-key removido em `S3InvestimentosSection`, `ApendicesSections`, `UsaSections`, `frontend/src/lib/api/reports.ts`.
  YAML: `config/report_layout.yaml` — chart S3 `mariana_cenarios` renomeado para `cenarios_conjuge`; APP_C title atualizado + flag `optional: true`; canvas map + section_charts atualizados. Schema `config/schemas/report_layout.schema.json::appendixSpec` ganha `optional: boolean` (fixup `f259e94`). Codegen `dev/codegen_report_layout.py` regenera `frontend/src/generated/report-layout.ts` + `backend/app/generated/report_layout.py`. Prompt `config/prompts/chart_conclusions.yaml` atualizado.
  Tests TS: `frontend/tests/components/report/apendices.test.tsx` reescrito com 3 casos (hide-when-empty retorna null, visualização comparativa renderiza com `cenarios_conjuge`, copy não-alarmista CVM/Susep no subtítulo).
...
- **chore(docs,config): A8.4 PR5 — limpeza editorial final de copy USA-related (2026-05-06):**
  ...
  Iniciativa A8.4 (Cenários de Estresse) **encerrada** após 6 PRs sequenciais (#78 docs · #80 rename schema · #81 gate + analyzer · #82 APP_C UX · #83 delete Modo USA · #85 limpeza editorial). 3 ADRs registradas: ADR-166 (schema estável `cenarios_conjuge`), ADR-167 (eligibility gate domain service), ADR-168 (remoção do Modo USA).
