...
| **Onda 10 — coerência cross-rota** (~3-5d · P1) — 6 fixes UI puros que destravam ritual mensal: MonetaryValue em /plano + CTA "Abrir relatório" + backward link Suggestion→relatório + SuggestionCallout migra para tokens semânticos + drafts Suggestion regras 2/3 enriquecidos + /acao workspace zero redireciona. Independente de Onda 8 (lógica vs UI distinta); coordenar hotspot SuggestionCard.tsx. | [track_onda_10_cross_route_coherence.md](track_onda_10_cross_route_coherence.md) | Direção E pós-revisão polish | `agent/onda-10-cross-route-coherence/*` |
| **A8 TRS real — renda passiva observada + TRS efetiva** (~5-6d · P1) — `PassiveIncomeCalculator` novo + fix `_bucket_capital` (aluguel: trabalho→capital, com paridade IRPFAnalyzer) + ratios sai de placeholder `"N/D"` + popula `goals.taxa_retirada_efetiva_pct` (destrava `rule_trs_desalinhada` dormente, com filtro `if_pct ≥ 50`) + S7 ganha 4 KPIs (renda R$/mês + TRS efetiva + Carteira de renda + acumuladores %) + tooltip antierro-iniciante + 2 banners (defasagem 6m/15m, acumuladores >40%) + 2 empty states (sem_irpf, gerador_zero) + ADR canônica. **G0 (financial-planner) feito;** copy review com product-designer recomendado. | [track_a8_trs_real.md](track_a8_trs_real.md) | independente (paralelo a Onda 8/9) | `agent/a8-trs-real/*` |
| **Platform Review — orquestração multi-agent** (revisão completa pipeline + backend + docs com 6 subagents em paralelo, consolidação CTO, plano resumível em `docs/PLATFORM_REVIEW_PLAN.md`). Não toca código — só produz findings + plano. | [track_platform_review.md](track_platform_review.md) | independente | `agent/platform-review/*` |
| **W5-T01 A11y onda — `scope=col` + `role=progressbar` + aria-label charts + reduced-motion** (Platform Review · Wave 5; PD-007/014/015/021; ~6h ativos) | [track_w5t01_a11y.md](track_w5t01_a11y.md) | Platform Review · Wave 5 | `agent/w5t01-a11y/*` |
| **W5-T03 MonetaryValue migration — `<MonetaryValue size="kpi"/>` cutover** (Platform Review · Wave 5; PD-006/010/011/012/013; 18 call-sites) | [track_w5t03_monetary_value.md](track_w5t03_monetary_value.md) | Platform Review · Wave 5 | `agent/w5t03-monetaryvalue/*` |
| **W5-T04 ADR-161 enrichment — 5 sub-PRs paralelos** (5 regras Suggestion dormentes: taxa_poupanca_caindo, seguros_insuficientes, concentracao_instituicao, lifestyle_creep, renda_passiva_real_baixa) | [track_w5t04_adr161_enrichment.md](track_w5t04_adr161_enrichment.md) | Platform Review · Wave 5 | `agent/w5t04-adr161-enrich/*/*` |
| **W5-T05 Goal IF v2 cutover — 3 PRs sequenciais** (FP-010/011/012/017 — additive → frontend reads → drop v1; ADR-140/142) | [track_w5t05_goal_if_v2.md](track_w5t05_goal_if_v2.md) | Platform Review · Wave 5 | `agent/w5t05-goal-if-v2/*` |
| **W6-T01 Schema hardening — E5 strict + 7 sub-schemas E4 + ADR-090 wire compliance** (3 sub-PRs sequenciais; codegen Pydantic→JSON Schema; backfill `string-decimal`) | [track_w6t01_schema_hardening.md](track_w6t01_schema_hardening.md) | Platform Review · Wave 6 | `agent/w6t01-schema-hardening/*/*` |
| **W6-T05 Pipeline artifacts retention + cascade-on-delete** (5 PRs sequenciais; `retention_until` + prune diário + FK CASCADE; DE-010/011/017/022) | [track_w6t05_artifacts_retention.md](track_w6t05_artifacts_retention.md) | Platform Review · Wave 6 | `agent/w6t05-artifacts-retention/*/*` |
