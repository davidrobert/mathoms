...
| Data Lineage fim-a-fim + Fonte plugável (Sprint A23 `current`, origem 2026-06-02) — lineage forward+reverso legível por LLM + `SourceAdapter`/`SourceRef` (fonte plugável acoplada) + extração limpa (de-leak). 8 fases (F0 gate → F7 debug substrate); F0 abre 4 ADR Proposto (ADR-278 a ADR-281) + emenda ADR-146 (B1); supersede ADR-045. Nenhuma lane abre antes de B1–B8 travados | [docs/plan/DATA_LINEAGE/_README.md](docs/plan/DATA_LINEAGE/_README.md) |
