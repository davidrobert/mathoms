#!/usr/bin/env python3
"""Isolamento da superfície de diagnóstico (ADR-404; uso:
...
def violations_in_source(src: str, path: str) -> list[str]:
    ...
    for lineno, scope in _session_scopes(tree, funcs):
        ...
        out.append(
            ...
            "e proteja o diagnóstico com try/except (ADR-404)"
        )
    return out
...
def _public_session_params(tree: ast.Module, path: str) -> list[str]:
    ...
    return [
        ...
        "compartilhar a transação (ADR-404)"
        for n in ast.walk(tree)
...
def _constructions_outside_sink(tree: ast.Module, path: str) -> list[str]:
    ...
    return [
        ...
        "escrita de diagnóstico tem um dono só (ADR-404)"
        for n in ast.walk(tree)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.verbose and not errors:
        print("OK: diagnóstico isolado da transição de run (ADR-404)")
