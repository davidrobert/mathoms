    ...
    for ann, label in checks:
        if _ann_is_dict_str_any(ann):
            out.append(
                Offender(
                    id="",
                    category="P3_dict_str_any_boundary",
                    severity="high",
                    file=rel,
                    line_start=ann.lineno,
                    line_end=ann.end_lineno or ann.lineno,
                    length=1,
                    identifier=f"{node.name}.{label}",
...
def _check_optional_in_fn(
    node: ast.FunctionDef | ast.AsyncFunctionDef, rel: str, lines: list[str]
) -> list[Offender]:
    out: list[Offender] = []
    ...
    for i, arg in enumerate(positional):
        ...
        out.append(
            Offender(
                id="",
                category="P4_optional_no_default",
                severity="low",
                file=rel,
    ...
    for node in ast.walk(tree):
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            if _ann_is_float(node.annotation) and MONEY_NAME_PATTERN.search(node.target.id):
                out.append(
                    _float_offender(
                        rel, node.lineno, node.end_lineno or node.lineno, node.target.id, "variable"
                    )
                )
    return out
...
def _float_money_in_fn(node: ast.FunctionDef | ast.AsyncFunctionDef, rel: str) -> list[Offender]:
    ...
    if _ann_is_float(node.returns) and MONEY_NAME_PATTERN.search(node.name):
        out.append(
            _float_offender(
                rel, node.lineno, node.returns.end_lineno or node.lineno, node.name, "return"
            )
        )
    return out
...
def _docstring_offender(node: ast.AST, rel: str) -> Offender | None:
    ...
    return Offender(
        id="",
        category="P7_multiparagraph_docstring",
        severity="low",
        file=rel,
        line_start=line,
        line_end=line,
...
def _boundary_line_to_offender(line: str) -> Offender | None:
    ...
    return Offender(
        id="",
        category="P10_pipeline_boundary",
        severity="critical",
        file=rel,
        line_start=ln,
        line_end=ln,
