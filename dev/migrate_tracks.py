...
_SPRINT_PATTERNS = (
    (re.compile(r"^a(\d+)"), "A"),
    (re.compile(r"^f(\d+)"), "F"),
    (re.compile(r"^w(\d+)t\d+"), "W"),
)


def infer_sprint(stem_after_track: str) -> str:
    """Sprint inferida: 'a6e3_x' → 'A6'; 'w5t01_x' → 'W5'; demais → DEFAULT_SPRINT."""
    for pattern, prefix in _SPRINT_PATTERNS:
        m = pattern.match(stem_after_track)
        if m:
...
def extract_h1(md_path: Path) -> str | None:
    ...
    matches = (
        line.strip()[2:].strip() for line in _read_lines(md_path) if line.strip().startswith("# ")
    )
...
def _build_mapping(src: Path) -> TrackMapping:
    stem = src.stem
    ...
    return TrackMapping(
        ...
        title=derive_title(extract_h1(src), slug),
        destination=SPRINT_BASE / sprint / "tracks" / f"{slug}.md",
    )


def plan_migration(sources: list[Path]) -> list[TrackMapping]:
    """Constrói TrackMapping para cada arquivo source."""
    return [_build_mapping(src) for src in sources]
