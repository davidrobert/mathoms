#!/usr/bin/env python3
"""
STAGE E2-extratos-llm: Extract investment positions and CDB data from PDFs/XLSX
"""
...
# ============================================================================
# INVESTMENT POSITIONS PDF EXTRACTORS
# ============================================================================
...
def extract_btgpactual_investimentosposicao(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            ...
            # Extract date from text (look for date patterns)
                        ...
                        for row in table[1:] if len(table) > 1 else table:
                            if row and len(row) >= 3:
                                posicao = {
                                    ...
                                    "applied_value": normalize_decimal(str(row[1]) if row[1] else "0"),
                                    "current_value": normalize_decimal(str(row[2]) if row[2] else "0"),
                                    "quantity": None,
                                    "unit_price": None,
                                    "issuer": None,
                                    "rate": None,
                                ...
                                if posicao["name"]:
                                    ...
                                    result["total"] += posicao["current_value"]
    ...
    try:
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages:
                ...
                if date_match and not result["data_posicao"]:
                    result["data_posicao"] = extract_date(date_match.group(1))
                        ...
                        for row in table[1:] if len(table) > 1 else table:
                            if row and len(row) >= 2:
                                try:
                                    ...
                                    value = normalize_decimal(str(row[-1]) if row[-1] else "0")
...
def extract_rico_investimentosposicao(pdf_path: str) -> Dict[str, Any]:
    ...
    try:
                        ...
                        for row in table[1:] if len(table) > 1 else table:
                            if row and len(row) >= 2:
                                try:
                                    ...
                                    values = [col for col in row[1:] if col]
                                    if values:
                                        value = normalize_decimal(str(values[-1]))
                                        if name and value > 0:
                                            posicao = {
                                                "product_type": "FundoInvestimento",
                                                "name": name,
        ...
        print(f"Error extracting Rico: {e}")
...
def extract_santander_cdbdetalhes_pdf(pdf_path: str, filename: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Error extracting Santander CDB details from {filename}: {e}")
...
def extract_xlsx_santander_cdbresumo(xlsx_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Error extracting XLSX Santander CDB: {e}")

    return result

def extract_html_xls_santander(xls_path: str) -> Dict[str, Any]:
    """Extract HTML-disguised XLS files for Santander CDB"""
    result = {
        "tipo": "cdbdetalhes",
        "banco": "santander",
        "data_posicao": None,
    ...
    try:
            ...
            for row in rows[1:] if len(rows) > 1 else rows:
                ...
                if len(cells) >= 2:
                    try:
                        ...
                        if produto["nome"] and produto["nome"].upper() not in ["NOME", "PRODUTO"]:
                            result["produtos"].append(produto)
                            result["total_bruto"] += produto["saldo_bruto"]
                            result["total_liquido"] += produto["saldo_liquido"]
                    except:
        ...
        print(f"Error extracting HTML XLS {xls_path}: {e}")

    return result

# ============================================================================
# BANK STATEMENT IMAGE EXTRACTORS
# ============================================================================

def extract_bank_statement_image(jpg_path: str, banco: str) -> Dict[str, Any]:
    """Extract bank statement from JPG image using vision"""
    result = {
        "tipo": "extratoconta",
        "banco": banco,
        "conta": None,
    ...
    # For images, we'd need OCR/vision API which isn't available here
    # Return basic structure with note
    result["_note"] = "Requires OCR/vision processing - manual review needed"
