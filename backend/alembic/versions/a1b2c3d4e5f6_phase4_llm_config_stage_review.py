...
def upgrade() -> None:
    ...
    op.create_table('stage_reviews',
        ...
        sa.Column('status', sa.Enum('pending', 'approved', 'edited', name='stagereviewstatus'), nullable=False),
    ...
    with op.batch_alter_table('pipeline_runs') as batch_op:
        batch_op.add_column(sa.Column('tier_at_run', sa.String(length=20), nullable=False, server_default='free'))
