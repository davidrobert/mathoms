...
def _personal_documents(tenant_root: Path) -> list[Path]:
    ...
    for sub in _PERSONAL_DIRS:
        ...
        if d.exists():
            docs.extend(
                f for f in sorted(d.rglob("*")) if f.is_file() and f.suffix.lower() in _TEXT_EXTENSIONS
            )
    return docs
...
def backfill_member_cpfs(
    ...
    """Preenche ``cpf_encrypted`` NULL a partir dos documentos originais.

    Idempotente: nunca sobrescreve CPF existente. ``dry_run=True`` só conta.
    """
    ...
    if not members:
        return counts
    docs = _personal_documents(Path(tenant_root))
    if not docs:
...
def purge_cpf_from_e1_artifacts(db: Session, *, workspace_id: str, dry_run: bool = True) -> dict:
    ...
    Substitui por ``cpf_present: true`` — preserva o sinal, elimina o valor.
    """
