"""Sink de `review_reasons` (ADR-404) — sessão própria, fail-open, sem `Session`
...
# Não aceita `Session` de propósito (ADR-404): quem chama não consegue
...
def record_review_reasons(*, run_id: str, workspace_id: str, stage_name: str, reasons: Any) -> int:
    ...
    try:
        with SyncSessionLocal() as db:
            ...
            db.commit()
    except Exception as exc:  # noqa: BLE001 — fail-open é a decisão da ADR-404
        _log_sink_failure(
