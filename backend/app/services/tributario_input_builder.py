...
def _load_irpf_renda_tributavel(workspace_id: str, *, db: SyncSession) -> RendaTributavelPF:
    ...
    cpf = _cpf_do_titular(workspace_id, db=db)
    return extract_renda_tributavel_pf(_irpf_do_ano_base(workspace_id, declaracoes, cpf))


def _cpf_do_titular(workspace_id: str, *, db: SyncSession) -> Optional[str]:
    """CPF do membro `titular`; ``None`` quando não há um só, ou não tem CPF."""
    from backend.app.models.family_member import FamilyMember
    from backend.app.services.security.vault import get_vault

    titulares = (
        db.query(FamilyMember)
        .filter(FamilyMember.workspace_id == workspace_id, FamilyMember.role == "titular")
        .all()
    )
    # Dois titulares cadastrados é cadastro ambíguo — mesma política de
    # `protection_snapshot_compute._primary_subject`: sem um só, não elege.
    if len(titulares) != 1 or not titulares[0].cpf_encrypted:
        return None
    return get_vault().decrypt(titulares[0].cpf_encrypted)
