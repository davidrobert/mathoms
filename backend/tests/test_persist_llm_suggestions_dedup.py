"""ADR-266 regressão de _persist_llm_suggestions: soft-supersede + dedup_key + dismiss window + source isolation + normalização léxica."""
...
async def _create_ws_and_run(session, user_id: str) -> tuple[str, str]:
    ws = Workspace(id=str(uuid.uuid4()), owner_id=user_id, name="ADR-266 WS")
    session.add(ws)
    await session.flush()
    run = PipelineRun(
        id=str(uuid.uuid4()),
