...
@pytest.mark.asyncio
async def test_create_task_inside_horizon_creates_notification(
    db, enable_event_driven
):
    ...
    rows = (
        await db.execute(
            select(Notification).where(Notification.workspace_id == ws.id)
        )
    ).scalars().all()
...
@pytest.mark.asyncio
async def test_create_task_overdue_creates_critical_notification(
    db, enable_event_driven
):
    ws = await make_workspace(db)
    await db.commit()
...
@pytest.mark.asyncio
async def test_create_task_outside_horizon_does_not_notify(
    db, enable_event_driven
):
    ws = await make_workspace(db)
    await db.commit()
...
@pytest.mark.asyncio
async def test_update_task_to_urgent_deadline_creates_notification(
    db, enable_event_driven
):
    ws = await make_workspace(db)
