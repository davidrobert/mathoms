#!/usr/bin/env python3
"""
STAGE E2-extratos-llm V2: Enhanced extraction with better PDF parsing
"""
...
def normalize_decimal(value: str) -> float:
    ...
    value = re.sub(r'[^\d,.-]', '', value)  # Remove non-numeric chars except comma and dot
    if value.count(',') > 0 and value.count('.') > 0:
        # Both exist - Brazilian format
        last_comma = value.rfind(',')
        last_dot = value.rfind('.')
        if last_comma > last_dot:
            # Comma is last - it's the decimal separator
            value = value.replace('.', '').replace(',', '.')
        else:
            # Dot is last - remove all commas
            value = value.replace(',', '')
    elif value.count(',') > 0:
        ...
        if value.count(',') == 1 and len(value.split(',')[1]) == 2:
            ...
            # Multiple commas or not 2 digits = thousands separator
            value = value.replace(',', '')
...
def extract_pdf_text_and_tables(pdf_path: str) -> tuple:
        ...
        for table in tables:
            for row in table:
                if row and len(row) >= 2:
                    # Skip header rows
                    if any(h in str(row[0]).upper() for h in ["PRODUTO", "NOME", "ATIVO"]):
                        continue
        ...
        for table in tables:
            for row in table:
                if row and len(row) >= 1:
                    name = str(row[0]).strip() if row[0] else ""
                    if not name or len(name) < 2 or name.upper() in ["PRODUTO", "NOME"]:
                        continue
            ...
            for row in rows[1:] if len(rows) > 1 else rows:
                ...
                if len(cells) >= 2:
                    try:
                        ...
                        if produto["nome"].upper() not in ["NOME", "PRODUTO"]:
                            result["produtos"].append(produto)
                            result["total_bruto"] += produto["saldo_bruto"]
                            result["total_liquido"] += produto["saldo_liquido"]
                    except:
...
def extract_bank_statement_image(jpg_path: str, banco: str) -> Dict[str, Any]:
    ...
    result = {
        ...
        "saldo_final": 0.0,
        "transacoes": [],
        "_note": "Image requires OCR processing - requires vision model"
    }
...
def process_all_files():
    ...
    # Summary
    print("\n" + "="*70)
    print(f"STAGE E2 PROCESSING COMPLETE (V2)")
