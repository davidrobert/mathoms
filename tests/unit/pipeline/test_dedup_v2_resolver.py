...
from scripts import e4_categorize  # noqa: E402
...
class TestHasDbStore:
    def test_workspace_none_is_false(self):
        assert e4_categorize._e4_has_db_store(_ctx(workspace_id=None), object()) is False

    def test_non_db_store_is_false(self):
        assert e4_categorize._e4_has_db_store(_ctx(), object()) is False
...
class TestNoDbStoreEnvIsSoleOverride:
    ...
    def test_env_1_enables(self, monkeypatch):
        monkeypatch.setattr(e4_categorize, "_e4_has_db_store", lambda c, s: False)
        monkeypatch.setenv(_ENV, "1")
        assert e4_categorize._e4_dedup_v2_enabled(_ctx(), object()) is True

    def test_env_unset_disabled(self, monkeypatch):
        monkeypatch.setattr(e4_categorize, "_e4_has_db_store", lambda c, s: False)
        assert e4_categorize._e4_dedup_v2_enabled(_ctx(), object()) is False
...
class TestDbStoreIsSovereign:
    ...
    def _patch(self, monkeypatch, db_flag):
        monkeypatch.setattr(e4_categorize, "_e4_has_db_store", lambda c, s: True)
    ...
    def test_db_true_holds_even_with_env_unset(self, monkeypatch):
        ...
        assert e4_categorize._e4_dedup_v2_enabled(_ctx(), _db_store()) is True
    ...
    def test_db_false_ignores_env_on(self, monkeypatch):
        ...
        assert e4_categorize._e4_dedup_v2_enabled(_ctx(), _db_store()) is False
...
class TestResolutionPathsAreExactlyTwo:
    ...
    def test_matrix_fully_determined_by_two_branches(self, monkeypatch):
        ...
        for has_db, db_flag, env in itertools.product(
            ...
            monkeypatch.setattr(e4_categorize, "_e4_has_db_store", lambda c, s, _h=has_db: _h)
            monkeypatch.setattr(ff, "is_enabled_sync", lambda ws, k, db=None, _v=db_flag: _v)
            monkeypatch.delenv(_ENV, raising=False)
            if env is not None:
                monkeypatch.setenv(_ENV, env)
            got = e4_categorize._e4_dedup_v2_enabled(_ctx(), _db_store())
