...
| **ADR-280** | Campos em de-leak F2 (A24.l1→A24.l3) | Extratos carregam `numero_conta`/`numero_conta_norm` (norm consistente com `normalize_account_number` — invariante de produção via `finalize_e2_result`) para o blast radius de strip ter sinal (F2-DB8). `tipo_lancamento` foi **removido do contrato E2** em A24.l3 (de-leak entregue; campo proibido por construção — `additionalProperties:false` + `test_e2_contract_no_methodological_fields`). | ✅ prova one-shot (l1): strip → E3 byte-idêntico, zero `value_delta` monetário; de-leak efetivado em A24.l3 |
