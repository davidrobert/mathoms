...
# -*- coding: utf-8 -*-
"""
[DEPRECATED] Use scripts/e2_extract.py instead.
...
# Bank display names — from config/institutions.json banco_canonical
_BANCO_CANONICAL = _INST_CONFIG.get("banco_canonical", {})


def _banco_display(key: str) -> str:
    """Get display name for a bank from config, with title-case fallback."""
    name = _BANCO_CANONICAL.get(key, key)
    return name.title() if name == name.lower() else name


_BANCO_C6 = _banco_display("c6bank")
_BANCO_ITAU = _banco_display("itau")
_BANCO_PICPAY = _banco_display("picpay")
_BANCO_BRADESCO = _banco_display("bradesco")
_BANCO_SANTANDER = _banco_display("santander")
_BANCO_RICO = _banco_display("rico")
_BANCO_WISE = _banco_display("wise")
...
def parse_c6bank_csv(csv_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_C6, tipo, moeda)
...
def parse_itau_xls(xls_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_ITAU, tipo, "BRL")
...
def parse_picpay(pdf_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_PICPAY, "extratoconta", "BRL")
...
def parse_bradesco(pdf_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_BRADESCO, tipo, "BRL")
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            while i < len(lines):
                ...
                if dm:
                    ...
                    if line and not line.startswith("Data ") and not line.startswith(_BANCO_BRADESCO) and not re.match(r'^Total\s', line):
...
def parse_santander_xls(xls_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_SANTANDER, "extratoconta", "BRL")
...
def parse_rico(pdf_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_RICO, "extratoconta", "BRL")
...
def parse_wise(pdf_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = make_result_template(_BANCO_WISE, tipo, moeda)
