...
_BANCO_QUINTOANDAR = _banco_display("quintoandar")

# Card display names — from config/institutions.json cartoes
_CARTAO_CARBON = _CARTOES.get("faturacarbon", {}).get("nome_cartao", "Carbon")
_CARTAO_UNIQUE = _CARTOES.get("faturaunique", {}).get("nome_cartao", "Unique")
_CARTAO_PDA = _CARTOES.get("faturapaoacucar", {}).get("nome_cartao", "Pão de Açúcar")
...
def parse_santander_fatura_csv(csv_path: Path, filename: str) -> Dict[str, Any]:
    ...
    result = {
        "banco": _BANCO_SANTANDER,
        "tipo": "faturaunique",
        "cartao": _CARTAO_UNIQUE,
        "titular": None,
        "moeda": "BRL",
        "data_vencimento": None,
        "saldo_anterior": None,
