 ...
 *
 * `@critical` é o que faz este gate GATEAR. Sem a tag ele não rodaria em PR
 * nenhum: o step default (`Report render gate`) filtra `--grep @critical
 * --project=chromium`, e o job "Frontend E2E" é opt-in por label `e2e`. Um spec
 * sem tag cai no vão entre os dois — que é exatamente o modo de falha desta
 * lane, um gate que existe e não mede.
 *
 * Custo cross-browser: com a tag, Firefox e WebKit também o pegam, mas só no
 * job opt-in. No gate de todo PR roda chromium apenas.
 */
...
test.describe("Faixa sticky · scroll-spy @critical", () => {
  ...
  async function lerAtivos(page: Page) {
    return page.evaluate(() => {
      const chip = document.querySelector(
        "[data-report-topnav] a[data-active='true']",
      );
      const rail = document.querySelector("[data-report-nav-rail]");
      const toc = document.querySelector(
        "aside.sidebar-toc [data-toc-id][class*='brand-primary']",
      );
  ...
  test("a faixa marca chips ativos ao longo do scroll", async ({ page }) => {
    ...
    expect(
      vistos.size,
      `ativos vistos: ${[...vistos].join(",")}`,
    ).toBeGreaterThanOrEqual(3);
  });

  test("o índice montado desde o load também marca — é o usuário que volta", async ({
    page,
  }) => {
  ...
  test("faixa e índice concordam sobre a seção corrente", async ({ page }) => {
    ...
    for (const top of PONTOS) {
      ...
      if (faixa && toc)
        expect(faixa, `divergiram em scrollTop=${top}`).toBe(toc);
    }
  });
  ...
  test("todo chip declarado tem tinta", async ({ page }) => {
    ...
    const semTinta = await page.evaluate(() => {
      const chips = [
        ...document.querySelectorAll("[data-report-nav-rail] a[data-nav-id]"),
      ];
      return chips
        .filter((a) => {
  ...
  test("abaixo de md a faixa não é focável", async ({ page }) => {
    ...
    expect(
      estado.fab,
      "sem faixa E sem FAB o relatório fica sem navegação",
    ).toBe(true);
  });
});
