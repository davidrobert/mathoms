...
export function ReportDataQualityBanner({
  ...
  if (signals.count === 0) return <CleanBar />;
  return <SignalsAlert signals={signals} />;
}
  ...
  return (
    ...
    >
      <ShieldCheck
        ...
        aria-hidden="true"
      />
      <span>Qualidade dos dados: sem pendências que afetem a leitura deste relatório.</span>
    </div>
  );
}
    ...
    <div className="mb-6" data-testid="data-quality-banner">
      ...
      >
        <p className="font-medium">
          Qualidade dos dados — {n} {n === 1 ? "pendência afeta" : "pendências afetam"} a
          precisão deste relatório.
        </p>
        <ul
        ...
        >
          ...
          {signals.needsReviewDocs > 0 && <NeedsReviewRow count={signals.needsReviewDocs} />}
...
function NaoIdentificadoRow({ share }: { share: NaoIdentificadoShare }) {
  return (
    ...
    >
      ...
      {share.pct.toFixed(1).replace(".", ",")}% do total) — reclassificar devolve
      precisão ao fluxo de caixa.
    </SignalRow>
  );
}
...
function NeedsReviewRow({ count }: { count: number }) {
  return (
    <SignalRow cta={{ href: "/documents?filter=needs_review", label: "Revisar documentos" }}>
      {count}{" "}
      {count === 1 ? "documento aguarda revisão" : "documentos aguardam revisão"} de
      classificação — dados deles podem estar fora desta análise.
    </SignalRow>
  );
}
  ...
  return (
    <SignalRow cta={{ href: "#APP_B", label: "Ver premissas adotadas" }}>
      ...
      {degrade.classesTotal > 0 && (
        <>
          {" "}
          ({degrade.classesIndisponiveis}/{degrade.classesTotal} classes sem premissa
          vigente)
        </>
      )}{" "}
...
function ImoveisPendentesRow({ count }: { count: number }) {
  return (
    <SignalRow
      cta={{ href: "/config?tab=members", label: "Classificar em Configurações" }}
    >
      {count}{" "}
      {count === 1
...
function SignalRow({
  ...
  return (
    <li className="flex flex-wrap items-baseline gap-x-2">
      <span>{children}</span>
      {isAnchor ? (
        <a href={cta.href} style={ctaStyle} className="whitespace-nowrap font-medium">
          {cta.label}
        </a>
      ) : (
        <Link href={cta.href} style={ctaStyle} className="whitespace-nowrap font-medium">
          {cta.label}
        </Link>
      )}
    </li>
