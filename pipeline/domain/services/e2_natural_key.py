"""Estampa K4 ``natural_key`` + ``direction`` no write-path comum de E2 (ADR-278 B4
passo 1) — NÃO em ``to_e2_dict`` (só roda no round-trip E3). Cobre vocabulário
determinístico (banco/titular/tipo_conta) e LLM (instituicao/membro/tipo_documento)
por fallback; emite ``natural_key`` só com discriminantes presentes (senão ``null``
classe-c, nunca hash degenerado). F1 **emite + mede**, não consome: E4 segue v1 (D4)."""
...
def stamp_natural_key(result: dict) -> NaturalKeyStats:
    """Popula ``transacoes[].natural_key`` + ``direction`` in-place; retorna cobertura."""
