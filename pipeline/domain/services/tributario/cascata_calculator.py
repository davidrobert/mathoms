...
def _compute_layers(inp: CascataInput) -> _ComputedLayers:
    ...
    # A base do teto de 12% é o TOTAL dos rendimentos tributáveis, e só o IRPF o
    # tem. Somar `cargas.bruto_anual` contava o pró-labore duas vezes: ele já
    # está no IRPF como `rendimentos_pj` — a própria empresa do titular é fonte
    # pagadora PJ. Medido: +82,8% na base e teto de R$ 38.160 no lugar de
    # R$ 20.880 ([[A40.l36]]).
    #
    # Não se subtrai nada do lado do IRPF: a ficha da própria PJ também carrega
    # lucro excedente tributável e gratificações, e trocá-la pelo valor do fluxo
    # produziria base de ano híbrido — o híbrido que a [[ADR-375]] já recusou.
