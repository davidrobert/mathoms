...
from pipeline.domain.services.e5_output_converters import (
    _janela_irpf,
    _passive_income_to_dict,
    _proventos_summary_to_dict,
)
...
class E5OutputInputs:
    ...
    # ``faixa_ref``). None quando o workspace não declara membros ou o caller é
    # legado; a chave é omitida do output (on_null:skip no manifest do parecer).
...
def build_e5_output(inputs: E5OutputInputs) -> dict[str, Any]:
    ...
    # PE-3 (r7): lado CIVIL da família. Projetado junto do lado fiscal
    # ($.irpf_kpis.dependentes) para que o parecer possa reconciliar em vez de
    # afirmar os dois com confianças incompatíveis.
    if inputs.composicao_familiar is not None:
        output["composicao_familiar"] = inputs.composicao_familiar
