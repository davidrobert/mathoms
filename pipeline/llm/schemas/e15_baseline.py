...
class PatrimonialItem(BaseModel):
    ...
    category: str = Field(..., description="Category: imovel, veiculo, investimento, conta_corrente, poupanca, previdencia, outros")
...
class BaselinePatrimonialOutput(BaseModel):
    ...
    items: list[PatrimonialItem] = Field(default_factory=list, description="All patrimonial items extracted")
    ...
    members_found: list[str] = Field(default_factory=list, description="Member keys found in declarations")
