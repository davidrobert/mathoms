...
- [../../docs/reference/PRODUCT.md](../../docs/reference/PRODUCT.md) — **público real**: PJ/CLT alta renda + famílias com patrimônio diversificado (imóveis + investimentos) + futuro B2B2C (planejadores). Isto define qual metodologia tende a dominar: **AUVP** em patrimônio diversificado, **Cerbasi** em ciclo familiar/casal, **Perini** em meta de independência. Modelo Free vs. Premium (BYOK) define se análise LLM-augmented está disponível.
...
- [../../docs/reference/ARCHITECTURE.md](../../docs/reference/ARCHITECTURE.md) §4.1 Domain glossary — **fonte de verdade de domínio** (rules-as-code, ADR-143): índice que aponta, por conceito, o módulo enforcer + ADR canônica. Membros/instituições/categorias vivem no **DB** via `ConfigStore` (`family_members`/`institution_catalog`/`category_template`, ADR-134/137) — os JSONs legados de `config/` foram migrados e seus paths são proibidos (`docs/methodology/` idem). Não invente regra de categorização.
...
- [../../docs/_MOC/SPRINTS-active.md](../../docs/_MOC/SPRINTS-active.md) — sprint atual + lanes ativas. Não recomende mudança que choca com lane em voo.
