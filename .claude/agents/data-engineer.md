...
## Schema e modelagem
- **Dinheiro nunca é `float`** ([ADR-090](../../docs/DECISIONS.md#adr-090)): `Numeric(18,2)` em DB, `Money.brl` em Python, decimal string no wire. Quebrar essa regra produz bug silencioso de arredondamento.
...
- **Fallback explícito**: LLM opcional. Padrão do repo (`classify_document` [ADR-081](../../docs/DECISIONS.md#adr-081)): regex/det. primeiro → LLM se confidence < 0.8 → `needs_review` se < 0.7. Stage LLM novo segue esse contrato.
...
- **Custo é feature**: meça tokens por chamada, cacheie ([ADR-144](../../docs/DECISIONS.md#adr-144) é o padrão para E5), use modelo menor quando viável (Haiku para classificação simples; Sonnet para análise).
...
- **Schema drift detection**: snapshot de OpenAPI ([ADR-109](../../docs/DECISIONS.md#adr-109)) é o padrão para API; aplique a mesma disciplina em contratos de pipeline (JSON Schema versionado).
