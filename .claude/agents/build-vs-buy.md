...
- [../../docs/_MOC/_generated/SPRINT_CURRENT.md](../../docs/_MOC/_generated/SPRINT_CURRENT.md) — sprint atual + lanes ativas + tamanho do time (BACKLOG.md é shim). "Vamos construir X" precisa caber no time real, não em time hipotético.
...
- [../../docs/_MOC/_generated/ADR_INDEX.md](../../docs/_MOC/_generated/ADR_INDEX.md) — ADRs vigentes, notas atômicas em `docs/adr/` (DECISIONS.md é shim), podem **já ter decidido** build vs. buy de algo adjacente. Antes de propor, `rg -i` em `docs/adr/` por nome do vendor/lib e por categoria (auth, queue, storage, monitoring, LLM provider).
...
- [../../docs/reference/STATELESS_AUDIT.md](../../docs/reference/STATELESS_AUDIT.md) + [ADR-111](../../docs/adr/111-stateless-rigoroso-padrao-e-gate-empirico-a6f6.md) — invariantes que dependência nova **deve** respeitar (cache em Redis, rate limit em DB/Redis SET NX, sem estado mutável local).
