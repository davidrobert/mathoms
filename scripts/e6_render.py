...
def _load_config_rates(base: Path) -> dict:
    ...
    if config_path.exists():
        ...
        print("  [WARN] config/taxas.json não encontrado — usando defaults")
    return defaults
...
def _init_config(base_dir: Path) -> None:
    ...
    FISCAL_CONFIG = _load_json_config(
        BASE_DIR / "config" / "parametros_fiscais.json", "parametros_fiscais.json"
    )
    CENARIOS_CONFIG = _load_json_config(BASE_DIR / "config" / "cenarios.json", "cenarios.json")
    INSTITUTIONS_CONFIG = _load_json_config(
        BASE_DIR / "config" / "institutions.json", "institutions.json"
    )
    ...
    _CONJUGE_KEY = next(
        (
            k
            for k, v in _MEMBROS_DATA.items()
            if isinstance(v, dict) and v.get("papel") == "conjuge"
        ),
        None,
    )
    CONJUGE_NOME = (
        _MEMBROS_DATA.get(_CONJUGE_KEY, {}).get("nome_curto", "Cônjuge")
        if _CONJUGE_KEY
        else "Cônjuge"
    )
...
SECTION_CHARTS = (
    {int(k): v for k, v in _raw_section_charts.items()}
    if _raw_section_charts
    else {
...
def chart_html(chart_key: str, title: str, narrativas_charts: dict, extra_attrs: str = "") -> str:
    ...
    if chart_key == "score_gauge":
        ...
        parts.append(
            '    <button id="rdm-prev" class="chart-nav-btn" title="Meses anteriores">&#8249;</button>'
        )
        parts.append('    <span id="rdm-period" class="chart-nav-period"></span>')
        parts.append(
            '    <button id="rdm-next" class="chart-nav-btn" title="Meses seguintes">&#8250;</button>'
        )
        parts.append("  </div>")
        ...
        parts.append(f'  <canvas id="{canvas_id}"></canvas>')
        parts.append(
            f'  <div id="legend-{chart_key.replace("_", "-")}" class="chart-legend-grouped"></div>'
        )
    else:
        parts.append(f'  <canvas id="{canvas_id}"></canvas>')
    if conclusion:
...
def build_kpi_section(e4: dict) -> dict:
    ...
    _nav_icon = (
        "".join(w[0] for w in FAMILY_SOBRENOME.split()[:2]).upper() if FAMILY_SOBRENOME else "FR"
    )
    ...
    replacements = {
        ...
        "{{KPI_PATRIMONIO_INVESTIVEL_SUB}}": (
            f"{(p['investivel']/p['bruto']*100):.1f}% do bruto".replace(".", ",")
            if p["bruto"] > 0
            else "N/D"
        ),
        "{{KPI_RENDA_MENSAL}}": fmt_brl(
            f.get("janela_12m", f).get("receita_recorrente_mensal", f["receita_recorrente_mensal"])
        ),
...
def build_receita_despesa_mensal(f: dict, e4: dict) -> dict:
    ...
    """
    detalhado = f.get("receita_despesa_mensal_detalhado") or e4.get("fluxo_caixa", {}).get(
    ...
    if detalhado and "labels" in detalhado and "receita_datasets" in detalhado:
        ...
        receita_sorted = sorted(
            detalhado["receita_datasets"], key=lambda ds: sum(ds["data"]), reverse=True
        )
        despesa_sorted = sorted(
            detalhado["despesa_datasets"], key=lambda ds: sum(ds["data"]), reverse=True
        )

        receita_color_map = {
            ds["label"]: RECEITA_PALETTE[i % len(RECEITA_PALETTE)]
            for i, ds in enumerate(receita_sorted)
        }
        despesa_color_map = {
            ds["label"]: DESPESA_PALETTE[i % len(DESPESA_PALETTE)]
        ...
        def build_sorted_slots(cat_datasets, color_map, stack_name):
            ...
            for m in range(num_months):
                entries = [
                    (
                        ds["data"][m] if m < len(ds["data"]) else 0,
                        ds["label"],
                        color_map[ds["label"]],
                    )
        ...
        legend_receita = [
            {"label": ds["label"], "color": receita_color_map[ds["label"]]} for ds in receita_sorted
        ]
        legend_despesa = [
            {"label": ds["label"], "color": despesa_color_map[ds["label"]]} for ds in despesa_sorted
        ]
        ...
        print(
            "[E6.3] receita_despesa_mensal: FALLBACK para médias planas (dados detalhados ausentes)"
        )
        return {
            "labels": [
                "jan",
...
def build_report_data_json(e4: dict) -> tuple:
    ...
    kpis = {
        ...
        "if_pct": g["if_pct"],
        "score": s["valor"],
        "renda_passiva_mensal": g.get("renda_passiva", {}).get(
            "atual_mensal", g.get("renda_passiva_estimada_4pct", 0)
        ),
        "renda_passiva_meta": g.get("renda_passiva", {}).get(
            "meta_mensal",
            g.get(
                "if_trs_monthly_value",
                GOALS_CONFIG.get("independencia_financeira", {}).get(
                    "renda_passiva_meta_mensal", 0
                ),
            ),
        ),
    }
...
def _build_top5_decisoes_fallback() -> list:
    ...
    if cfg and all(isinstance(d, dict) and "label" in d for d in cfg):
        return cfg
    print(
        "  ⚠️  WARNING: top5_decisoes não encontrado em goals.json — configure 'top5_decisoes' para evitar dados genéricos"
    )
    aporte = GOALS_CONFIG.get("aportes", {}).get("meta_aporte_mensal", 0)
    if aporte <= 0:
        print(
            "  ⚠️  WARNING: top5_decisoes e meta_aporte_mensal ausentes em goals.json — usando placeholder genérico"
        )
        return [
    ...
    return [
        {
            "label": f"Aportes R$ {aporte/1000:.0f}k/mês",
            "impacto_1a": aporte * 12,
            "impacto_10a": aporte * 12 * 10,
        },
...
def build_charts(e4: dict) -> dict:
    ...
    yield_anual = (
        round((aluguel_mensal * 12 / imoveis_inv) * 100, 1)
        if p.get("imoveis_investimento", 0) > 0
        else 0
    )
    ...
    renda_dividendos = round(
        safe_float(f.get("por_fonte", {}).get("receita_investimento", 0)) / num_months_chart, 2
    )
    renda_juros = round(
        safe_float(f.get("por_fonte", {}).get("rendimentos_financeiros", 0)) / num_months_chart, 2
    )
    ...
    charts = {
        ...
        "receita_bar": (
            lambda pfd: {
                ...
                "datasets": [{"data": list(pfd.values()), "backgroundColor": PALETTE[: len(pfd)]}],
            }
                ...
                "datasets": [
                    {
                        ...
                        "backgroundColor": [
                            "#15803D",
                            "#B91C1C",
                            "#2E86AB" if rec >= desp else "#F4A261",
                        ],
                        "borderRadius": 4,
                    }
                ],
        ...
        "despesas_doughnut": (
            lambda dc: {
                ...
                "datasets": [{"data": [v for _, v in dc], "backgroundColor": PALETTE[: len(dc)]}],
            }
        ...
        "alocacao_alvo": (
            lambda alvo: {
                ...
                "datasets": [
                    {"data": list(alvo.values()), "backgroundColor": PALETTE[: len(alvo)]}
                ],
            }
        )(
            {
        ...
        "yield_imoveis": (
            lambda im_list: {
                ...
                "datasets": [
                    {
                        "data": [round(im.get("yield_anual", 0), 1) for im in im_list]
                        + [CONFIG_RATES["cdi_anual"]],
        ...
        "custos_f1f2": (
            lambda cambio, tuition_usd, rb_usd: {
        ...
        "cenarios_cambiais": (
            lambda cb, renda, cb_pess, cb_otim, f1f2_total_usd, mariana_usd: {
                "labels": [
                ...
                "datasets": [
                    ...
                    {
                        "label": f"Com {CONJUGE_NOME} (NCLEX)",
                        "data": [
            ...
            GOALS_CONFIG.get("fase_f1f2", {}).get("tuition_usd_anual", 27500)
            + GOALS_CONFIG.get("fase_f1f2", {}).get("room_board_usd_anual", 16500),
            GOALS_CONFIG.get("cenarios_conjuge", GOALS_CONFIG.get("mariana_eua", {})).get(
        ...
        "projecao_3cenarios": {
            ...
            "aporte_mensal": g.get(
                "aporte_mensal", GOALS_CONFIG.get("aportes", {}).get("meta_aporte_mensal", 0)
            ),
            "anos": CENARIOS_CONFIG.get("horizonte_projecao_anos", 20),
            "trs_pct": GOALS_CONFIG.get("independencia_financeira", {}).get("trs_pct", 5.0),
            "renda_passiva_meta": GOALS_CONFIG.get("independencia_financeira", {}).get(
                "renda_passiva_meta_mensal", 30000
            ),
            "taxa_imoveis_pessimista": CENARIOS_CONFIG.get("valorizacao_imoveis", {}).get(
                "pessimista_pct", 2.0
            )
            / 100,
            "taxa_imoveis_realista": CENARIOS_CONFIG.get("valorizacao_imoveis", {}).get(
                "realista_pct", 5.0
            )
            / 100,
            "taxa_imoveis_otimista": CENARIOS_CONFIG.get("valorizacao_imoveis", {}).get(
                "otimista_pct", 8.0
            )
            / 100,
            "taxa_pessimista": CENARIOS_CONFIG.get("retorno_real", {}).get("pessimista_pct", 4.0)
            / 100,
        ...
        "renda_passiva": (
            lambda rp_atual, rp_meta: {
                ...
                "datasets": [
                    {
                        "label": "Aluguéis",
                        "data": [round(renda_aluguel)],
                        "backgroundColor": "#2E86AB",
                    },
                    {
                        "label": "Dividendos",
                        "data": [round(renda_dividendos)],
                        "backgroundColor": "#15803D",
                    },
                    {
                        "label": "RF/Cupons",
                        "data": [round(renda_juros)],
                        "backgroundColor": "#F4A261",
                    },
                    {
                        "label": "Gap até Meta",
                        "data": [max(0, round(rp_meta - rp_atual))],
            ...
            "datasets": [
                {
                    "label": "Valores (R$)",
                    "data": [
                        round(receita_pj_anual),
                        round(renda_tributavel),
                        round(receita_pj_anual * _das_pct),
                        round(limite_pgbl),
                        round(economia_ir),
                    ],
        ...
        "top5_decisoes": (
            lambda decs: {
                ...
                "datasets": [
                    {
                        "label": "Impacto 1 ano",
                        "data": [d.get("impacto_1a", 0) for d in decs],
                        "backgroundColor": "#2E86AB",
                    },
                    {
                        "label": "Impacto 10 anos",
                        "data": [d.get("impacto_10a", 0) for d in decs],
                        "backgroundColor": "#2DC653",
                    },
                ],
            }
        ...
        f"{_CONJUGE_KEY}_cenarios": (
            lambda cm, m_min, m_max: {
                ...
                "datasets": [
                    {
                        "label": "Aporte mensal",
                        "data": cm.get("aportes", [0, m_min, m_max]),
                        "backgroundColor": "#2E86AB",
                    },
                    {
                        "label": "Prazo IF (anos)",
                        "data": cm.get("prazos_if", [0, 0, 0]),
                        "backgroundColor": "#2DC653",
                        "yAxisID": "y1",
                    },
                ],
        ...
        "viagens": (
            lambda vg12, teto: (
                ...
                )([pm.get(m, 0) for m in meses])
            )(vg12.get("meses", []), vg12.get("por_mes", {}))
        )(
...
def build_investimentos(e4: dict) -> dict:
    ...
    return {
        ...
        "total": inv_data.get(
            "total",
            e4["patrimonio"].get(_KEY_INV_TITULAR, 0) + e4["patrimonio"].get(_KEY_INV_CONJUGE, 0),
        ),
        "kpis": {
            "yield_medio_pct": "N/D",
        ...
        "blocos": [
            {"nome": c["categoria"], "valor": c["valor"], "pct": c.get("pct", 0)} for c in tabela
        ],
...
def build_estrategia_aporte(e4: dict) -> dict:
    ...
    if total > 0:
        print(
            "  ⚠️  WARNING: aportes_destinos_detalhados não encontrado em goals.json — usando meta_aporte_mensal genérica"
        )
        return {
            "total_aporte": total,
            "dia_aporte": aportes_cfg.get("dia_aporte", 5),
            ...
            "destinos": [
                {
                    ...
                    "moeda": "BRL",
                    "veiculo": "—",
                    "notas": "Configure aportes_destinos_detalhados em goals.json para detalhamento",
                }
            ],
            "pct_brl": 100,
            "pct_usd": 0,
...
def build_tactical_dashboard(e4: dict) -> dict:
    ...
    if not _TETOS:
        print(
            f"  [WARN] tetos_orcamentarios não encontrado em goals.json — usando {_teto_fallback_mult*100:.0f}% da média como fallback"
        )
    ...
    for i, d in enumerate(destinos_cfg):
        ...
        for kw_group, keywords in aporte_keywords.items():
            if any(kw in dest_lower for kw in keywords):
                ...
                break
        if valor_feito >= d.get("valor", 0) * (
            _DASH_CFG.get("aporte_feito_threshold_pct", 50) / 100
        ):
            feito = True
        aportes_dash[key] = {
    ...
    if prev:
        ...
        _inv_label_pairs = [
            (f"investimentos_{k}", v.get("label_curto", k))
            for k, v in _INV_BLOCOS.items()
            if isinstance(v, dict)
        ]
        if not _inv_label_pairs:
            _inv_label_pairs = [
                (_KEY_INV_TITULAR, f"Inv. {TITULAR_NOME}"),
                (_KEY_INV_CONJUGE, f"Inv. {CONJUGE_NOME}"),
            ]
        ...
        novas_feitas = sum(
            1
            for k, v in tarefas_status.items()
            if str(v) == "feito" and str(prev_status.get(k, "")) != "feito"
        )
        if novas_feitas:
...
def build_patrimonio_categorias_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">Composição do patrimônio bruto de {fmt_brl(bruto)} em {len(categorias)} categorias</div>'
    )
    ...
    for item in categorias:
        ...
        pct = item.get("pct", 0)
        html_parts.append(
            f"      <tr><td>{cat}</td><td>{fmt_brl(valor)}</td><td>{fmt_pct(pct)}</td></tr>"
        )

    html_parts.append(
        f'    <tr class="total-row"><td><strong>PATRIMÔNIO BRUTO</strong></td><td><strong>{fmt_brl(bruto)}</strong></td><td><strong>100%</strong></td></tr>'
    )
    html_parts.append(f"    <tr><td>(-) Dívidas</td><td>{fmt_brl(dividas)}</td><td>—</td></tr>")
    html_parts.append(
        f'    <tr class="total-row"><td><strong>PATRIMÔNIO INVESTÍVEL (excl. residência + veículos)</strong></td><td><strong>{fmt_brl(investivel)}</strong></td><td>—</td></tr>'
    )
    html_parts.append("    </tbody>")
    html_parts.append("  </table>")
...
def _aggregate_receitas_by_period(txns_compact: list) -> dict:
    ...
    for pkey, cutoff in periods.items():
        ...
        result[pkey] = {
            "rows": [{"c": c, "v": round(v, 2)} for c, v in arr],
            "total": round(total, 2),
        }
    return result
...
def build_receitas_fonte_card(e4: dict) -> str:
    ...
    if not txns_compact:
        ...
        for it in fallback:
            html_parts.append(
                f'    <tr><td>{it.get("categoria","")}</td><td>{fmt_brl(it.get("valor",0))}</td><td>{fmt_pct(it.get("pct",0))}</td></tr>'
            )
        html_parts.append(
            f'    <tr class="total-row"><td><strong>Total</strong></td><td><strong>{fmt_brl(total)}</strong></td><td><strong>100,0%</strong></td></tr>'
        )
    ...
    html_parts.append(
        '    <button class="period-btn" data-period="3m" onclick="filterRF(\'3m\')">3M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="6m" onclick="filterRF(\'6m\')">6M</button>'
    )
    html_parts.append(
        '    <button class="period-btn active" data-period="12m" onclick="filterRF(\'12m\')">12M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="ytd" onclick="filterRF(\'ytd\')">Ano</button>'
    )
    html_parts.append("  </div>")
    ...
    html_parts.append(
        '      document.querySelectorAll("#rf-period-toggle .period-btn").forEach(function(b){b.classList.remove("active")});'
    )
    html_parts.append(
        '      document.querySelector("#rf-period-toggle .period-btn[data-period=\\""+p+"\\"]").classList.add("active");'
    )
    ...
    html_parts.append(
        '        tb.innerHTML="<tr><td colspan=\\"3\\" class=\\"empty-state\\">Nenhuma receita no período</td></tr>";'
    )
    ...
    html_parts.append(
        '          return"<tr><td>"+i.c+"</td><td>"+fB(i.v)+"</td><td>"+fP(pct)+"</td></tr>";'
    )
    html_parts.append('        }).join("");')
    html_parts.append(
        '        rows+="<tr class=\\"total-row\\"><td><strong>Total</strong></td><td><strong>"+fB(d.total)+"</strong></td><td><strong>100,0%</strong></td></tr>";'
    )
...
def _build_classes_from_e4_positions() -> list:
    ...
    if total <= 0:
        return []
    result = [
        {"categoria": k, "valor": v, "pct": round(v / total * 100, 2)} for k, v in agg.items()
    ]
...
def build_investimentos_classe_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">Distribuição do patrimônio investível de {fmt_brl(total_inv)} por classe</div>'
    )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
    html_parts.append("      <tr><th>Categoria</th><th>Valor (R$)</th><th>% do Total</th></tr>")
...
def build_reserva_emergencia_card(e4: dict) -> str:
    ...
    if isinstance(niveis_raw, dict):
        ...
        niveis = {
            "minimo_6m": {
                "valor": despesa_mensal * 6,
                "status": "✅" if total_liquido >= despesa_mensal * 6 else "⚠️",
            },
            "conforto_9m": {
                "valor": despesa_mensal * 9,
                "status": "✅" if total_liquido >= despesa_mensal * 9 else "⚠️",
            },
            "conservador_12m": {
                "valor": despesa_mensal * 12,
                "status": "✅" if total_liquido >= despesa_mensal * 12 else "⚠️",
            },
        }
    ...
    html_parts.append(
        f'  <div class="card-subtitle">Baseado na despesa mensal média de {fmt_brl(despesa_mensal)}</div>'
    )
    ...
    html_parts.append("    <thead>")
    html_parts.append(
        "      <tr><th>Critério</th><th>Meses</th><th>Valor Necessário</th><th>Liquidez Atual</th><th>Status</th></tr>"
    )
    html_parts.append("    </thead>")
    html_parts.append("    <tbody>")
    ...
    for nivel_key in ["minimo_6m", "conforto_9m", "conservador_12m"]:
        ...
        liquidez_cell = (
            f'<td rowspan="3">{fmt_brl(total_liquido)}<br><small>({cobertura_meses:.1f} meses)</small></td>'
            if first_row
            else ""
        )
        html_parts.append(
            f"      <tr><td>{label}</td><td>{meses}</td><td>{fmt_brl(valor)}</td>{liquidez_cell}<td>{status}</td></tr>"
        )
        first_row = False
    ...
    for key, nome in comp_items:
        ...
        if val > 0:
            ...
            html_parts.append(
                f"      <tr><td>{nome}</td><td>{fmt_brl(val)} ({pct:.0f}%)</td><td>{fmt_pct(pct)}</td></tr>"
            )

    html_parts.append(
        f'      <tr class="fw-bold"><td>Total</td><td>{fmt_brl(total_liquido)}</td><td>—</td></tr>'
    )
    html_parts.append("    </tbody>")
    html_parts.append("  </table>")
...
def build_endividamento_card(e4: dict) -> str:
    ...
    if dividas:
        html_parts.append(
            f'  <div class="card-subtitle">{len(dividas)} dívida(s) ativa(s) totalizando {fmt_brl(total_saldo)} em saldo devedor</div>'
        )
    else:
        html_parts.append(
            '  <div class="card-subtitle">Nenhuma dívida identificada — situação saudável</div>'
        )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
    html_parts.append(
        "      <tr><th>Descrição</th><th>Saldo Devedor</th><th>Parcela Mensal</th><th>Taxa</th></tr>"
    )
    html_parts.append("    </thead>")
    html_parts.append("    <tbody>")
    ...
    for divida in dividas:
        ...
        html_parts.append(
            f"      <tr><td>{desc}</td><td>{fmt_brl(saldo)}</td><td>{parcela_display}</td><td>{taxa_display}</td></tr>"
        )

    total = end.get("total_dividas", 0)
    html_parts.append(
        f'    <tr class="total-row"><td><strong>Total</strong></td><td><strong>{fmt_brl(total)}</strong></td><td></td><td></td></tr>'
    )
    html_parts.append("    </tbody>")
    html_parts.append("  </table>")
    html_parts.append("</div>")
...
def _aggregate_orcamento_by_period(labels: list, despesa_datasets: list) -> dict:
    ...
    for pkey, cutoff in periods.items():
                ...
                cats.append(
                    {
                        "key": key,
                        "label": ds.get("label", ""),
                        "avg": round(total_sum / n_months, 2),
        ...
        result[pkey] = {
            "cats": cats,
            "totalM": round(total_mensal, 2),
            "nM": n_months,
            "first": first_label,
            "last": last_label,
            "pLabel": p_labels[pkey],
        }
    return result
...
def build_orcamento_prospectivo_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-title">Orçamento Prospectivo ({num_cats} Categorias)</div>'
    )

    html_parts.append('  <div class="period-toggle" id="op-period-toggle">')
    html_parts.append(
        '    <button class="period-btn" data-period="3m" onclick="filterOP(\'3m\')">3M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="6m" onclick="filterOP(\'6m\')">6M</button>'
    )
    html_parts.append(
        '    <button class="period-btn active" data-period="12m" onclick="filterOP(\'12m\')">12M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="ytd" onclick="filterOP(\'ytd\')">Ano</button>'
    )
    html_parts.append("  </div>")
    ...
    html_parts.append("    <thead>")
    html_parts.append(
        "      <tr><th>Categoria</th><th>Média Mensal</th><th>% do Total</th><th>Acum. %</th></tr>"
    )
    html_parts.append("    </thead>")
    html_parts.append("    <tbody></tbody>")
    html_parts.append("  </table>")
    ...
    html_parts.append(
        '      document.querySelectorAll("#op-period-toggle .period-btn").forEach(function(b){b.classList.remove("active")});'
    )
    html_parts.append(
        '      document.querySelector("#op-period-toggle .period-btn[data-period=\\""+p+"\\"]").classList.add("active");'
    )
    html_parts.append("      var d=A[p];if(!d)return;")
    html_parts.append('      var ctx=document.getElementById("op-context");')
    html_parts.append(
        '      ctx.innerHTML="Projeção mensal baseada na média de <strong>"+d.nM+" meses</strong>"'
    )
    ...
    html_parts.append(
        '        var cls=c.key==="nao_identificado"?" class=\\"row-highlight-warn\\"":"";'
    )
    html_parts.append(
        '        rows+="<tr"+cls+"><td>"+c.label+"</td><td>"+fB(c.avg)+"</td><td>"+fP(pct)+"</td><td>"+fP(acum)+"</td></tr>";'
    )
    html_parts.append("      });")
    html_parts.append(
        '      rows+="<tr class=\\"total-row\\"><td><strong>Total Mensal</strong></td><td><strong>"+fB(d.totalM)+"</strong></td><td><strong>100,0%</strong></td><td></td></tr>";'
    )
    html_parts.append("      tb.innerHTML=rows;")
    html_parts.append('      var tm=document.getElementById("op-treemap");if(tm){var th="";')
    html_parts.append(
        "        d.cats.forEach(function(c,idx){var pct=d.totalM>0?c.avg/d.totalM*100:0;if(pct<1)return;"
    )
    html_parts.append("          var bg=PAL[idx%PAL.length];var w=Math.max(pct,4);")
    html_parts.append(
        '          th+="<div class=\\"treemap-cell\\" style=\\"flex:"+w+" 1 0;background:"+bg+"\\" title=\\""+c.label+": "+fB(c.avg)+"/mês ("+fP(pct)+")\\"><span>"+c.label.split("_").pop()+"</span></div>";'
    )
    ...
    html_parts.append(
        '        if(nip>10)h+="<p class=\\"insight-note\\"><strong>Atenção:</strong> "+fP(nip)+" das despesas estão como \\"Não Identificado\\" ("+fB(ni.avg)+"/mês). Classificar essas transações melhora a precisão do orçamento.</p>";}'
    )
    html_parts.append(
        "      if(d.cats.length>=3){var t3=d.cats.slice(0,3);var t3p=d.totalM>0?t3.reduce(function(s,c){return s+c.avg;},0)/d.totalM*100:0;"
    )
    html_parts.append(
        '        h+="<p class=\\"insight-note\\">As 3 maiores categorias ("+t3.map(function(c){return c.label}).join(", ")+") representam <strong>"+fP(t3p)+"</strong> do orçamento mensal.</p>";}'
    )
    html_parts.append("      ins.innerHTML=h;")
    html_parts.append('      var me=document.getElementById("op-method");')
    html_parts.append(
        '      me.innerHTML="<strong>Metodologia:</strong> Valores calculados como média aritmética das despesas por categoria nos "+d.pLabel+" ("+d.nM+" meses). Categorias ordenadas por impacto decrescente. Coluna \\"Acum. %\\" mostra concentração acumulada (análise Pareto).";'
    )
    html_parts.append("    };")
...
def _aggregate_consumo_consciente_by_period(itens: list, aporte_mensal: float) -> dict:
    ...
    for pkey, cutoff in periods.items():
        filtered = sorted(
            [i for i in itens if i.get("mes", "") >= cutoff],
...
def build_consumo_consciente_card(e4: dict) -> str:
    ...
    html_parts.append(
        '    <button class="period-btn" data-period="3m" onclick="filterCC(\'3m\')">3M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="6m" onclick="filterCC(\'6m\')">6M</button>'
    )
    html_parts.append(
        '    <button class="period-btn active" data-period="12m" onclick="filterCC(\'12m\')">12M</button>'
    )
    html_parts.append(
        '    <button class="period-btn" data-period="ytd" onclick="filterCC(\'ytd\')">Ano</button>'
    )
    html_parts.append("  </div>")
    ...
    html_parts.append(
        '      document.querySelectorAll("#cc-period-toggle .period-btn").forEach(function(b){b.classList.remove("active")});'
    )
    html_parts.append(
        '      document.querySelector("#cc-period-toggle .period-btn[data-period=\\""+p+"\\"]").classList.add("active");'
    )
    ...
    html_parts.append(
        '        tb.innerHTML="<tr><td colspan=\\"4\\" class=\\"empty-state\\">Nenhum gasto pontual no período</td></tr>";'
    )
    html_parts.append("      }else{")
    html_parts.append("        tb.innerHTML=d.top.map(function(i){")
    html_parts.append(
        '          return"<tr><td>"+i.descricao+"</td><td>"+fB(i.valor)+"</td><td>"+i.data+"</td><td>"+i.det+"</td></tr>";'
    )
    html_parts.append('        }).join("");')
    html_parts.append("      }")
    ...
    html_parts.append(
        '        me.textContent=d.count+" gastos  \\u2022  Total: "+fB(d.total)+"  \\u2022  Equivale a "+d.equiv+" meses de aporte";'
    )
    html_parts.append("      }else{")
    html_parts.append(
        '        me.textContent="Nenhum gasto pontual relevante no per\\u00edodo selecionado.";'
    )
    html_parts.append("      }")
    html_parts.append("    };")
...
def build_diagnostico_comportamental_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">{len(diag)} padrão(ões) identificado(s) com base nas transações do período</div>'
    )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
...
def build_milhas_card(e4: dict) -> str:
    ...
    if not programas:
        if registrados:
            ...
            html_parts.append(
                "  <p>Atualize <code>config/milhas.md</code> com os saldos atuais de pontos/milhas.</p>"
            )
        else:
            html_parts.append(
                "  <p>Nenhum programa de milhas cadastrado. Atualize <code>config/milhas.md</code> com seus programas e saldos.</p>"
            )
        html_parts.append("</div>")
        return "\n".join(html_parts)
    ...
    html_parts.append("    <thead>")
    html_parts.append(
        "      <tr><th>Programa</th><th>Titular</th><th>Saldo (pts)</th><th>Valor Est. (R$)</th><th>Economia Período (R$)</th></tr>"
    )
    html_parts.append("    </thead>")
    html_parts.append("    <tbody>")
...
def build_previdencia_pgbl_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">Benefício fiscal de até 12% da renda bruta tributável — alíquota marginal de {pgbl.get("aliquota_marginal", 27.5)}%</div>'
    )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
    ...
    html_parts.append(
        f'    <tr><td>Renda Tributável Anual</td><td>{fmt_brl(pgbl.get("renda_tributavel_anual", 0))}</td></tr>'
    )
    html_parts.append(
        f'    <tr><td>Limite PGBL Anual</td><td>{fmt_brl(pgbl.get("limite_pgbl_anual", 0))}</td></tr>'
    )
    html_parts.append(
        f'    <tr><td>Aporte Mensal</td><td>{fmt_brl(pgbl.get("aporte_mensal", 0))}</td></tr>'
    )
    html_parts.append(
        f'    <tr><td>Alíquota Marginal</td><td>{fmt_pct(pgbl.get("aliquota_marginal", 0))}</td></tr>'
    )
    html_parts.append(
        f'    <tr><td>Economia IR Anual</td><td>{fmt_brl(pgbl.get("economia_ir_anual", 0))}</td></tr>'
    )
...
def build_pontos_fortes_card(e4: dict) -> str:
    ...
    h.append(
        f'  <div class="card-subtitle">{n} destaque(s) positivo(s) identificado(s) na análise financeira</div>'
    )
...
def build_pontos_urgentes_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">{len(urgentes)} ação(ões) prioritária(s) identificada(s) para atenção imediata</div>'
    )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
    html_parts.append(
        "      <tr><th>Prioridade</th><th>Ação</th><th>Impacto</th><th>Prazo</th></tr>"
    )
    html_parts.append("    </thead>")
    html_parts.append("    <tbody>")
    ...
    for item in urgentes:
        ...
        html_parts.append(
            f"      <tr><td>{prioridade}</td><td>{acao}</td><td>{impacto}</td><td>{prazo}</td></tr>"
        )
...
def build_equilibrio_cerbasi_card(e4: dict) -> str:
    ...
    html_parts.append(
        f'  <div class="card-subtitle">Metodologia de Gustavo Cerbasi para equilíbrio entre presente e futuro — {eq.get("classificacao", "N/D")}</div>'
    )
    html_parts.append("  <table>")
    html_parts.append("    <thead>")
    html_parts.append("      <tr><th>Métrica</th><th>Valor</th></tr>")
    ...
    html_parts.append(
        f'    <tr><td>% Presente (Despesas)</td><td>{fmt_pct(eq.get("pct_presente", 0))}</td></tr>'
    )
    html_parts.append(
        f'    <tr><td>% Futuro (Aportes)</td><td>{fmt_pct(eq.get("pct_futuro", 0))}</td></tr>'
    )
...
def build_estrategia_aporte_card(e4: dict) -> str:
    ...
    h.append(
        f'  <div class="card-subtitle">💰 Aporte mensal de {fmt_brl(total)} no dia {dia} de cada mês, distribuído em {len(destinos)} destinos. A partir de {periodo}.</div>'
    )
    ...
    h.append("        <td><strong>TOTAL</strong></td>")
    h.append(f'        <td class="text-right"><strong>{fmt_brl(total)}</strong></td>')
    h.append('        <td class="text-right"><strong>100%</strong></td>')
    h.append("        <td></td>")
    h.append("        <td></td>")
    h.append("      </tr>")
    h.append("    </tbody>")
...
def build_appendix_a() -> str:
    ...
    terms = [
        (
            "IF",
            "Independência Financeira — patrimônio suficiente para gerar renda passiva que cubra todas as despesas da família sem depender de trabalho ativo.",
        ),
        (
            "TRS",
            "Taxa de Retirada Segura — percentual anual que pode ser sacado do patrimônio investido sem exauri-lo ao longo do tempo. Referência: 4-5% a.a. para carteira diversificada.",
        ),
        (
            "CDI",
            "Certificado de Depósito Interbancário — taxa de referência para investimentos de renda fixa no Brasil. Acompanha de perto a taxa Selic.",
        ),
        (
            "Selic",
            "Taxa básica de juros da economia brasileira, definida pelo COPOM (Comitê de Política Monetária do Banco Central).",
        ),
        (
            "IPCA",
            "Índice Nacional de Preços ao Consumidor Amplo — indicador oficial de inflação no Brasil, medido pelo IBGE.",
        ),
        (
            "IPCA+",
            "Título público (Tesouro Direto) ou indexador que paga inflação IPCA mais um juro real prefixado.",
        ),
        (
            "IGPM",
            "Índice Geral de Preços — Mercado. Índice de inflação medido pela FGV, frequentemente usado em contratos de aluguel.",
        ),
        (
            "DAS",
            "Documento de Arrecadação do Simples Nacional — guia mensal de impostos para empresas optantes pelo Simples Nacional.",
        ),
        (
            "Simples Nacional",
            "Regime tributário simplificado para micro e pequenas empresas com faturamento até R$4,8M/ano. Unifica diversos impostos em uma guia (DAS).",
        ),
        (
            "Lucro Presumido",
            "Regime tributário alternativo ao Simples onde a base de cálculo do IR é um percentual presumido do faturamento (32% para serviços).",
        ),
        (
            "PGBL",
            "Plano Gerador de Benefício Livre — modalidade de previdência privada que permite deduzir até 12% da renda bruta tributável no IRPF.",
        ),
        (
            "VGBL",
            "Vida Gerador de Benefício Livre — previdência privada sem dedução fiscal, tributada apenas sobre os rendimentos no resgate.",
        ),
        (
            "CDB",
            "Certificado de Depósito Bancário — título de renda fixa emitido por bancos. Pode ser prefixado, pós-fixado (CDI) ou indexado (IPCA+).",
        ),
        (
            "LCI / LCA",
            "Letras de Crédito Imobiliário / Agrícola — títulos de renda fixa isentos de IR para pessoa física.",
        ),
        (
            "FII",
            "Fundo de Investimento Imobiliário — fundo negociado em bolsa que investe em imóveis ou ativos imobiliários, distribuindo rendimentos mensais.",
        ),
        (
            "ETF",
            "Exchange Traded Fund — fundo de índice negociado em bolsa. Ex: IVVB11 (replica S&P 500 em BRL).",
        ),
        (
            "IVVB11",
            "ETF listado na B3 que replica o índice S&P 500, proporcionando exposição ao mercado americano em reais.",
        ),
        (
            "DY",
            "Dividend Yield — rendimento de dividendos/rendimentos de um ativo, expresso como percentual anual sobre o preço.",
        ),
        (
            "P/L",
            "Preço/Lucro — múltiplo que relaciona o preço de uma ação ao lucro por ação. Indica quantos anos de lucro o mercado está pagando.",
        ),
        (
            "PM",
            "Preço Médio — custo médio de aquisição de um ativo, usado para cálculo de IR sobre ganho de capital.",
        ),
        (
            "Contrafluxo",
            "Estratégia de investimento da metodologia AUVP: comprar ativos atrelados ao indexador que está em baixa (ex: IPCA+ quando Selic está alta).",
        ),
        (
            "FBAR",
            "Foreign Bank Account Report — declaração obrigatória nos EUA para contas estrangeiras com saldo agregado acima de US$10.000.",
        ),
        (
            "FATCA",
            "Foreign Account Tax Compliance Act — lei americana que exige que instituições financeiras estrangeiras reportem contas de cidadãos/residentes dos EUA.",
        ),
        (
            "PFIC",
            "Passive Foreign Investment Company — classificação tributária americana que torna fundos brasileiros sujeitos a tributação punitiva nos EUA.",
        ),
        (
            "EB2-NIW",
            "Employment-Based Second Preference, National Interest Waiver — categoria de Green Card que dispensa oferta de emprego se o candidato demonstrar interesse nacional.",
        ),
        (
            "NCLEX-RN",
            "National Council Licensure Examination for Registered Nurses — exame de licenciamento para enfermeiros nos EUA.",
        ),
        (
            "IRPF",
            "Imposto de Renda Pessoa Física — imposto federal sobre a renda de pessoas físicas no Brasil.",
        ),
        ("IR", "Imposto de Renda — termo genérico para IRPF ou IRPJ."),
        (
            "IOF",
            "Imposto sobre Operações Financeiras — imposto federal sobre câmbio, crédito, seguros e títulos.",
        ),
        (
            "Carnê-Leão",
            "Recolhimento mensal obrigatório de IR sobre rendimentos recebidos de pessoas físicas ou do exterior, via DARF.",
        ),
    ]
    ...
    cats = [
        (
            "Patrimônio Bruto",
            "Soma de todos os ativos: imóveis, veículos, investimentos, criptos, contas bancárias, empresas.",
        ),
        (
            "Patrimônio Investível",
            "Patrimônio Bruto − imóvel de residência − veículos. São os ativos que geram ou podem gerar renda passiva.",
        ),
        ...
        (
            ...
            "Parcela líquida (resgate D+0 a D+2) destinada a cobrir 6-12 meses de despesas.",
        ),
    ]
...
def build_appendix_b(e4: dict = None) -> str:
    ...
    h.append(
        f"  <p>Os cenários abaixo fundamentam todas as projeções deste relatório. Fonte: BCB, IBGE, consenso de mercado ({_periodo_ref}).</p>"
    )
    h.append("  <table>")
    h.append(
        f"    <thead><tr><th>Variável</th><th>Pessimista</th><th>Realista (base)</th><th>Otimista</th><th>Atual ({_periodo_ref})</th></tr></thead>"
    )
    h.append("    <tbody>")
    ...
    premissas = [
        ...
        (
            ...
            f"{_cen_selic_p.get('selic', 8.0):.1f}%".replace(".", ",")
            if isinstance(_cen_selic_p, dict)
            else "8,0%",
            f"{_selic:.2f}%".replace(".", ","),
            f"{_cen_selic_o.get('selic', 15.0):.1f}%".replace(".", ",")
            if isinstance(_cen_selic_o, dict)
            else "15,0%",
    ...
    for var, pess, real, ot, at in premissas:
        h.append(
            f"      <tr><td>{var}</td><td>{pess}</td><td><strong>{real}</strong></td><td>{ot}</td><td>{at}</td></tr>"
        )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
    ...
    _rp_meta = GOALS_CONFIG.get("independencia_financeira", {}).get(
        "renda_passiva_meta_mensal", 30000
    )
    ...
    h.append(
        '  <p>Cálculo do "Número da Independência Financeira": patrimônio necessário = despesa anual desejada ÷ TRS. '
    )
    h.append(
        f"  Exemplo: {fmt_brl(_rp_meta)}/mês × 12 ÷ {_trs_ref}% = {fmt_brl(_if_meta)}. A projeção de prazo usa taxa de retorno real (acima da inflação) "
    )
    h.append("  e aporte mensal constante com juros compostos.</p>")

    h.append("  <h3>Gustavo Cerbasi — Equilíbrio Presente × Futuro</h3>")
    h.append(
        '  <p>Análise comportamental que classifica os gastos em "presente" (consumo, moradia, lazer) e "futuro" '
    )
    h.append(
        "  (investimentos, previdência, aportes). A proporção ideal é ~70% presente / 30% futuro. "
    )
    h.append(
        "  Classificações: <strong>Gastador</strong> (&lt;10% futuro), <strong>Equilibrado</strong> (20-40%), "
    )
    h.append(
        "  <strong>Poupador</strong> (&gt;40%). Referência: <em>Casais Inteligentes Enriquecem Juntos</em>.</p>"
    )

    h.append("  <h3>Raul Sena / AUVP — Contrafluxo e Análise Fundamentalista</h3>")
    h.append(
        '  <p>Estratégia de contrafluxo: investir no indexador que está "fora de moda". Quando a Selic está alta, '
    )
    h.append(
        "  prefixados e IPCA+ oferecem melhor relação risco/retorno. Quando a Selic está baixa, pós-fixados CDI protegem. "
    )
    h.append(
        "  A análise fundamentalista avalia ações por P/L, ROE, dívida/patrimônio e dividend yield histórico. "
    )
    ...
    h.append(
        "  <p>Média ponderada de 5 componentes (0-10), com interpolação linear entre extremos:</p>"
    )
    h.append("  <table>")
    h.append(
        "    <thead><tr><th>Componente</th><th>Peso</th><th>Nota 10</th><th>Nota 0</th></tr></thead>"
    )
    h.append("    <tbody>")
    ...
    if isinstance(_sc, dict) and any(k for k in _sc if not k.startswith("_")):
        _units = {
            ...
            "cobertura_despesas": " meses",
            "taxa_endividamento": "%",
            "progresso_if": "%",
            "diversificacao": " categorias",
        }
        score_comp = []
    ...
    h.append("  </table>")
    h.append(
        "  <p><em>Fórmula: Score = Σ(nota × peso) / Σ(peso), arredondado a 1 decimal.</em></p>"
    )
    ...
    h.append(
        f"  <p><strong>Nota metodológica:</strong> As taxas de poupança e a despesa mensal média "
    ...
    fontes = [
        (
            "Receitas e despesas",
            "Extratos bancários e faturas de cartão (PDFs originais)",
            _per_dados,
        ),
        (
            "Patrimônio (imóveis, veículos)",
            f"Declaração IRPF {_irpf_ano} + planilhas XLSX atualizadas",
            f"Posição {_pos_ref}",
        ),
        (
            "Investimentos",
            f"Posições de corretoras ({_build_broker_list()})",
            f"Posição {_pos_ref}",
        ),
    ...
    h.append(
        f"    <li>Projeções de renda passiva usam premissas de retorno real {_rr_real:.0f}% a.a. conforme cenários deste relatório. Revisar anualmente.</li>"
    )
    h.append(
        "    <li>Tabela fundamentalista de ações — valores estimados, confirmar antes de agir.</li>"
    )
    h.append("    <li>DY de FIIs de referência — DY passado não garante DY futuro.</li>")
    h.append(
        "    <li>Taxa PGBL — confirmar taxa real de administração antes de portabilidade.</li>"
    )
    h.append(
        '    <li>Benchmark de fundos — períodos variam por fundo (retorno acumulado desde aporte, não "últimos 12 meses").</li>'
    )
    h.append(
        "    <li>Para questões tributárias EUA, consultar CPA especializado em expatriados.</li>"
    )
    h.append("  </ul>")
    h.append("</div>")
...
def _compute_nper(
    investivel: float, aporte_mensal: float, taxa_anual_pct: float, meta: float
) -> float:
...
def build_appendix_c(e4: dict) -> str:
    ...
    meta_if = safe_float(
        goals.get("if_meta", GOALS_CONFIG.get("independencia_financeira", {}).get("if_meta", 0))
    )
    aporte = safe_float(
        goals.get("aporte_mensal", GOALS_CONFIG.get("aportes", {}).get("meta_aporte_mensal", 0))
    )
    ...
    h.append(
        f"  <p>Projeção de prazo para atingir a meta de {fmt_brl(meta_if)} com aporte de {fmt_brl(aporte)}/mês, variando a taxa de retorno real.</p>"
    )
    h.append("  <table>")
    h.append(
        f"    <thead><tr><th>Cenário</th><th>Retorno real a.a.</th><th>Aporte/mês</th><th>Prazo</th><th>{TITULAR_NOME} com</th></tr></thead>"
    )
    h.append("    <tbody>")
    h.append(
        f"      <tr><td>Pessimista</td><td>{fmt_dec(taxa_pess)}%</td><td>{fmt_brl(aporte)}</td><td>~{fmt_dec(prazo_pess)} anos</td><td>{idade_atual + round(prazo_pess)} ({ano_atual + round(prazo_pess)})</td></tr>"
    )
    h.append(
        f'      <tr class="total-row"><td><strong>Realista</strong></td><td><strong>{fmt_dec(taxa_real)}%</strong></td><td><strong>{fmt_brl(aporte)}</strong></td><td><strong>~{fmt_dec(prazo_real)} anos</strong></td><td><strong>{idade_atual + round(prazo_real)} ({ano_atual + round(prazo_real)})</strong></td></tr>'
    )
    h.append(
        f"      <tr><td>Otimista</td><td>{fmt_dec(taxa_otim)}%</td><td>{fmt_brl(aporte)}</td><td>~{fmt_dec(prazo_otim)} anos</td><td>{idade_atual + round(prazo_otim)} ({ano_atual + round(prazo_otim)})</td></tr>"
    )
    h.append("    </tbody>")
    h.append("  </table>")
    pct_atingido = (pat_investivel / meta_if * 100) if meta_if > 0 else 0
    h.append(
        f"  <p><strong>Progresso atual:</strong> {fmt_brl(pat_investivel)} de {fmt_brl(meta_if)} ({fmt_dec(pct_atingido)}% atingido).</p>"
    )
    h.append("</div>")
    ...
    f1f2_usd_anual = _f1f2.get("tuition_usd_anual", 27500) + _f1f2.get(
        "room_board_usd_anual", 16500
    )
    ...
    h.append("  <table>")
    h.append(
        f"    <thead><tr><th>Cenário</th><th>Câmbio</th><th>Custo F1/F2 mensal (BRL)</th><th>Meta US$ {fmt_num(meta_usd)} em BRL</th><th>Impacto</th></tr></thead>"
    )
    h.append("    <tbody>")
    h.append(
        f"      <tr><td>Pessimista (desvalorização)</td><td>R$ {fmt_dec(cb_pess, 2)}</td><td>{fmt_brl(custo_pess)}</td><td>{fmt_brl(meta_brl_pess)}</td><td>Custos +{var_pess}%, aporte USD rende menos</td></tr>"
    )
    h.append(
        f'      <tr class="total-row"><td><strong>Realista</strong></td><td><strong>R$ {fmt_dec(cb_real, 2)}</strong></td><td><strong>{fmt_brl(custo_real)}</strong></td><td><strong>{fmt_brl(meta_brl_real)}</strong></td><td><strong>Base do planejamento</strong></td></tr>'
    )
    h.append(
        f"      <tr><td>Otimista (valorização)</td><td>R$ {fmt_dec(cb_otim, 2)}</td><td>{fmt_brl(custo_otim)}</td><td>{fmt_brl(meta_brl_otim)}</td><td>Custos -{var_otim}%, folga para aportes maiores</td></tr>"
    )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
    ...
    selic_pess = (
        _cs.get("pessimista", {})
        if isinstance(_cs.get("pessimista"), dict)
        else {"selic": _selic_pess_val, "cdi": _selic_pess_val - 0.1}
    )
    selic_otim = (
        _cs.get("otimista", {})
        if isinstance(_cs.get("otimista"), dict)
        else {"selic": _selic_otim_val, "cdi": _selic_otim_val - 0.1}
    )
    ...
    h.append("  <table>")
    h.append(
        "    <thead><tr><th>Cenário</th><th>Selic</th><th>CDI estimado</th><th>Impacto na carteira RF</th><th>Ação recomendada</th></tr></thead>"
    )
    h.append("    <tbody>")
    h.append(
        f'      <tr><td>Queda acentuada</td><td>{fmt_dec(selic_pess["selic"])}%</td><td>~{fmt_dec(selic_pess["cdi"])}%</td><td>CDBs pós-fixados rendem menos; IPCA+ valoriza (marcação a mercado)</td><td>Manter IPCA+ até vencimento; aumentar prefixados longos</td></tr>'
    )
    h.append(
        f'      <tr class="total-row"><td><strong>Estabilidade</strong></td><td><strong>{fmt_dec(selic_atual)}%</strong></td><td><strong>~{fmt_dec(cdi_atual)}%</strong></td><td><strong>CDBs pós rendem bem; IPCA+ em carrego</strong></td><td><strong>Manter estratégia atual (contrafluxo IPCA+)</strong></td></tr>'
    )
    h.append(
        f'      <tr><td>Alta adicional</td><td>{fmt_dec(selic_otim["selic"])}%</td><td>~{fmt_dec(selic_otim["cdi"])}%</td><td>CDBs pós rendem mais; IPCA+ desvaloriza na marcação</td><td>Aumentar CDBs pós-fixados curtos; evitar IPCA+ longo novo</td></tr>'
    )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
    ...
    h.append(
        "  <p>Impacto da valorização (ou desvalorização) dos imóveis no patrimônio e no yield.</p>"
    )
    h.append("  <table>")
    h.append(
        "    <thead><tr><th>Cenário</th><th>Valorização anual</th><th>Impacto em 5 anos</th><th>Efeito no yield</th></tr></thead>"
    )
    h.append("    <tbody>")
    h.append(
        f"      <tr><td>Pessimista (estagnação)</td><td>0-{im_pess:.0f}%</td><td>Patrimônio imobiliário estável, perda real</td><td>Yield se mantém ou sobe (valor do imóvel cai)</td></tr>"
    )
    h.append(
        f'      <tr class="total-row"><td><strong>Realista</strong></td><td><strong>{im_real:.0f}%</strong></td><td><strong>Valorização ~{im_real_5a}% em 5 anos</strong></td><td><strong>Yield estável (aluguel acompanha valorização)</strong></td></tr>'
    )
    h.append(
        f"      <tr><td>Otimista (boom)</td><td>{im_otim:.0f}-{im_otim+2:.0f}%</td><td>Valorização ~{im_otim_5a_lo}-{im_otim_5a_hi}% em 5 anos</td><td>Yield pode comprimir (valor sobe mais que aluguel)</td></tr>"
    )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
    ...
    _reserva_meses = safe_float(
        e4.get("reserva_emergencia", {}).get(
            "cobertura_meses",
    ...
    stress = [
        (
            ...
            "CDBs pós rendem menos. Ação: contrafluxo — já ter IPCA+ longos na carteira captura a valorização. Manter Tesouro IPCA+ 2035/2040.",
        ),
        (
...
def build_appendix_d() -> str:
    ...
    h.append("  <table>")
    h.append(
        "    <thead><tr><th>Profissional</th><th>Área</th><th>Quando acionar</th></tr></thead>"
    )
    h.append("    <tbody>")
    ...
    contatos = [
        (
            f"Contador ({_contador})",
            "Contabilidade PJ, DAS, Simples Nacional",
            "Mensal (DAS) + IRPF anual + mudança de regime",
        )
    ]
...
def build_nclex_roadmap_card(e4: dict) -> str:
    ...
    _mariana_usd_min = GOALS_CONFIG.get(
        "cenarios_conjuge", GOALS_CONFIG.get("mariana_eua", {})
    ).get("renda_rn_minima_usd", 4000)
    _mariana_usd_max = GOALS_CONFIG.get(
        "cenarios_conjuge", GOALS_CONFIG.get("mariana_eua", {})
    ).get("renda_rn_maxima_usd", 7000)

    cost_strings = []

    h.append('<div class="card">')
    h.append(
        f'  <div class="card-title">NCLEX Roadmap{f" — {CONJUGE_NOME}" if CONJUGE_NOME else ""}</div>'
    )
    h.append(
        '  <div class="card-subtitle">Caminho para licenciamento como Registered Nurse nos EUA (estimativa 8-18 meses)</div>'
    )
    h.append("  <table>")
    h.append(
        "    <thead><tr><th>Etapa</th><th>Descrição</th><th>Custo</th><th>Duração</th></tr></thead>"
    )
    h.append("    <tbody>")
    if _nclex_steps:
        for step in _nclex_steps:
            ...
            h.append(
                f'      <tr><td>{step.get("etapa", "")}</td><td>{step.get("descricao", "")}</td><td>{custo}</td><td>{step.get("duracao", "—")}</td></tr>'
            )
    else:
        nclex = [
        ...
        for etapa, desc, custo, dur in nclex:
            cost_strings.append(custo)
            h.append(
                f"      <tr><td>{etapa}</td><td>{desc}</td><td>{custo}</td><td>{dur}</td></tr>"
            )
    ...
    h.append(
        f'      <tr class="total-row"><td colspan="2"><strong>Total estimado</strong></td><td><strong>{total_str}</strong></td><td></td></tr>'
    )
    h.append("    </tbody>")
    h.append("  </table>")
    ...
    _perfil_parts = [
        ...
        for p in [
            _conjuge_esp,
            f"Mestrado {_conjuge_mestrado}" if _conjuge_mestrado else "",
            _conjuge_profissao,
        ]
        if p
    ]
    _perfil_str = " + ".join(_perfil_parts) if _perfil_parts else ""
    _hourly_min = GOALS_CONFIG.get("cenarios_conjuge", GOALS_CONFIG.get("mariana_eua", {})).get(
        "renda_rn_hourly_min_usd", 45
    )
    _hourly_max = GOALS_CONFIG.get("cenarios_conjuge", GOALS_CONFIG.get("mariana_eua", {})).get(
        "renda_rn_hourly_max_usd", 80
    )
    _nclex_est = GOALS_CONFIG.get("nclex_estimativa_meses", "8-18")
    h.append(
        f'  <p><strong>Custo total estimado: {total_str}</strong>{f" | <strong>Perfil competitivo {_conjuge_nome}:</strong> {_perfil_str}" if _perfil_str else ""}.</p>'
    )
    h.append(
        f"  <p><strong>Projeção EUA:</strong> RN US${_hourly_min}–{_hourly_max}/hora → US${fmt_num(_mariana_usd_min)}–{fmt_num(_mariana_usd_max)}/mês líquido.</p>"
    )
    h.append("</div>")
    return "\n".join(h)
...
def build_simulacao_conjuge_card(e4: dict) -> str:
    ...
    meta_if = safe_float(
        goals.get(
            "if_meta", GOALS_CONFIG.get("independencia_financeira", {}).get("if_meta", 7200000)
        )
    )
    taxa_real = CENARIOS_CONFIG.get("retorno_real", {}).get("realista_pct", 6.0)

    _sim_mariana = e4.get(
        f"simulacao_{_CONJUGE_KEY}_sem_trabalhar", e4.get("simulacao_mariana_sem_trabalhar", {})
    )
    ...
    h.append(
        f'  <div class="card-subtitle">Cenário conservador com fator de redução de {_sim_fator*100:.0f}% no aporte mensal</div>'
    )
    h.append("  <table>")
    ...
    if _sim_mariana:
        for row in _sim_mariana.get("linhas", []):
            h.append(
                f'      <tr><td>{row.get("metrica", "")}</td><td>{row.get("valor", "")}</td></tr>'
            )
    else:
        ...
        h.append(
            f"      <tr><td>IF com aporte R$ {_aporte_cfg/1000:.0f}k mantido</td><td><strong>{fmt_dec(_prazo_full)} anos</strong> (folga absorve a perda)</td></tr>"
        )
        _aporte_red_fmt = fmt_dec(_aporte_red / 1000)
        h.append(
            f"      <tr><td>IF com aporte reduzido R$ {_aporte_red_fmt}k</td><td>{fmt_dec(_prazo_red)} anos (+{fmt_dec(_prazo_red - _prazo_full)} anos)</td></tr>"
        )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
...
def build_appendix_e(e4: dict) -> str:
    ...
    if tarefas:
        h.append("  <table>")
        h.append(
            "    <thead><tr><th>#</th><th>TAREFA</th><th>CATEGORIA</th><th>PRAZO</th><th>ESS.</th></tr></thead>"
        )
        h.append("    <tbody>")
        for t in tarefas:
            if isinstance(t, str):
            ...
            h.append(
                f'      <tr><td>{num}</td><td>{desc}</td><td>{categoria}</td><td>{prazo}</td><td><span class="priority-badge priority-{prio}">{prio_letter}</span></td></tr>'
            )
        h.append("    </tbody>")
        h.append("  </table>")
        ...
        h.append(
            f'  <p class="text-sm mt-sm"><span class="priority-badge priority-alta">S</span> = Essencial (S) — {essenciais} tarefas &nbsp; '
        )
        h.append(
            f'  <span class="priority-badge priority-media">R</span> = Recomendada (R) — {recomendadas} tarefas &nbsp; '
        )
        h.append(
            f'  <span class="priority-badge priority-baixa">O</span> = Opcional (O) — {opcionais} tarefas</p>'
        )
        if feitas > 0:
        ...
        if sugeridas:
            ...
            h.append(
                '  <p class="text-sm"><em>Sugeridas automaticamente com base nos dados financeiros. Pendente aprovação do titular para inclusão no backlog.</em></p>'
            )
            h.append("  <table>")
            h.append(
                "    <thead><tr><th>Sugestão</th><th>Motivo</th><th>Prioridade sugerida</th></tr></thead>"
            )
            h.append("    <tbody>")
            for s in sugeridas:
                ...
                h.append(
                    f'      <tr><td>{desc_s}</td><td>{motivo}</td><td><span class="priority-badge priority-{prio_s}">{prio_labels.get(prio_s, "R")}</span></td></tr>'
                )
            h.append("    </tbody>")
            h.append("  </table>")
    else:
    ...
    h.append("    <tbody>")
    h.append(
        f"      <tr><td>Teto anual de viagens</td><td><strong>{fmt_brl(_teto_viagens)}</strong></td></tr>"
    )
    h.append(
        f"      <tr><td>Gasto 12 meses (lazer/viagens)</td><td><strong>{fmt_brl(_gasto_viagens)}</strong></td></tr>"
    )

    if _meses_com_gasto:
        h.append(
            '      <tr><td colspan="2" class="pt-sm"><strong>Detalhamento mensal</strong></td></tr>'
        )
    ...
    h.append(
        f"      <tr><td>Saldo disponível</td><td{saldo_class}><strong>{fmt_brl(_saldo_viagens)}</strong></td></tr>"
    )
    h.append("    </tbody>")
    h.append("  </table>")
    ...
    if _txns:
        h.append(
            "  <details><summary>Ver transações individuais ({} itens)</summary>".format(len(_txns))
        )
        ...
        for t in _txns:
            h.append(
                f'      <tr><td>{t.get("data","")}</td><td>{t.get("descricao","")}</td><td>{fmt_brl(safe_float(t.get("valor",0)))}</td></tr>'
            )
        h.append("    </tbody></table></details>")

    h.append(
        "  <p><em>Nota: Custos da estadia EUA (F1/F2) NÃO entram no orçamento de viagens — são custo de vida.</em></p>"
    )
    h.append("</div>")
    ...
    h.append(
        "  <p>Detalhamento completo disponível no <strong>Modo USA</strong> (seções U3 e U4). Ative o modo USA no seletor superior para visualizar o roadmap NCLEX, custos estimados e a simulação de cenários.</p>"
    )
    h.append("</div>")
    ...
    if _calendario_e5:
        for ev in _calendario_e5:
            h.append(
                f'      <tr><td>{ev.get("data", "—")}</td><td>{ev.get("item", "")}</td><td>{ev.get("tipo", "")}</td></tr>'
            )
    else:
        ...
        for ev in GOALS_CONFIG.get("calendario_fallback", []):
            ...
            h.append(
                f'      <tr><td>{_data}</td><td>{_item}</td><td>{ev.get("tipo", "")}</td></tr>'
            )
    h.append("    </tbody>")
    h.append("  </table>")
    h.append("</div>")
...
def build_kpi_rentabilidade_card(e4: dict) -> str:
    ...
    renda_dividendos = round(
        safe_float(f.get("por_fonte", {}).get("receita_investimento", 0)) / num_months, 2
    )
    renda_juros = round(
        safe_float(f.get("por_fonte", {}).get("rendimentos_financeiros", 0)) / num_months, 2
    )
    ...
    h.append(
        '  <div class="card-subtitle">Indicadores derivados do fluxo de caixa e patrimônio — dados reais do período</div>'
    )
    ...
    h.append("    <tbody>")
    h.append(
        f'      <tr><td>Yield imóveis (anual)</td><td><span class="badge {yield_badge}">{fmt_pct(yield_imoveis)}</span></td><td>CDI {fmt_pct(cdi)}</td></tr>'
    )
    h.append(
        f"      <tr><td>Renda passiva mensal</td><td><strong>{fmt_brl(renda_passiva_total)}</strong></td><td>Aluguéis {fmt_brl(aluguel_mensal)} + Rendimentos {fmt_brl(renda_dividendos + renda_juros)}</td></tr>"
    )
    h.append(
        '      <tr><td>Rentabilidade carteira</td><td><span class="badge badge-neutral">N/D</span></td><td>Requer série temporal de cotas</td></tr>'
    )
    h.append(
        '      <tr><td>Volatilidade</td><td><span class="badge badge-neutral">N/D</span></td><td>Requer série temporal de cotas</td></tr>'
    )
    h.append("    </tbody>")
    h.append("  </table>")
    ...
    h.append("    <tbody>")
    h.append(
        f'      <tr><td>Instituições</td><td>{n_institutions}</td><td>{"✅ Bem distribuído" if n_institutions >= 3 else "⚠️ Concentrado"}</td></tr>'
    )
    h.append(
        f'      <tr><td>Tipos de ativo</td><td>{n_tipos}</td><td>{"✅ Diversificado" if n_tipos >= 5 else "⚠️ Poucas classes"}</td></tr>'
    )
    h.append(f"      <tr><td>Posições ativas</td><td>{n_positions}</td><td></td></tr>")
    h.append(
        f'      <tr><td>Concentração (maior posição)</td><td>{fmt_pct(top_pct)}</td><td>{"✅ OK" if top_pct <= 15 else "⚠️ Risco de concentração"}</td></tr>'
    )
    h.append(
        f'      <tr><td>HHI (Herfindahl por tipo)</td><td>{fmt_num(hhi)}</td><td><span class="badge {hhi_badge}">{hhi_label}</span></td></tr>'
    )
    h.append("    </tbody>")
    h.append("  </table>")

    h.append(
        '  <p class="note-muted">⚠️ Rentabilidade e volatilidade requerem dados históricos de cotas — não disponíveis nos extratos atuais.</p>'
    )
    h.append("</div>")
    return "\n".join(h)
...
def build_contrafluxo_card(e4: dict) -> str:
    ...
    h.append(
        f'  <div class="card-subtitle">Selic {fmt_dec(selic)}% a.a. · CDI {fmt_dec(cdi)}% · IPCA {fmt_dec(ipca)}% · Juro real ~{fmt_dec(selic - ipca)}%</div>'
    )
    ...
    if total_inv > 0:
        ...
        h.append("  <table>")
        h.append(
            "    <thead><tr><th>Indexador</th><th>Valor</th><th>%</th><th>Posições</th></tr></thead>"
        )
        h.append("    <tbody>")
        for idx_key, info in sorted_idx:
            ...
            h.append(
                f'      <tr><td>{label}</td><td>{fmt_brl(info["valor"])}</td><td>{fmt_dec(pct)}%</td><td>{info["count"]}</td></tr>'
            )
        h.append(
            f'      <tr class="total-row"><td><strong>Total</strong></td><td><strong>{fmt_brl(total_inv)}</strong></td><td><strong>100%</strong></td><td><strong>{sum(b["count"] for b in by_idx.values())}</strong></td></tr>'
        )
        h.append("    </tbody>")
        h.append("  </table>")
    ...
    if cenarios:
        h.append('  <div class="card-title mt-3">Cenários de Selic</div>')
        h.append("  <table>")
        h.append(
            "    <thead><tr><th>Cenário</th><th>Selic</th><th>Pós-CDI</th><th>IPCA+</th><th>Ação</th></tr></thead>"
        )
        h.append("    <tbody>")
        for cen in cenarios:
    ...
    if rec_text:
        pct_ipca = (
            by_idx.get("ipca_plus", {}).get("valor", 0) / total_inv * 100 if total_inv > 0 else 0
        )
        pct_cdi = (
            by_idx.get("pos_cdi", {}).get("valor", 0) / total_inv * 100 if total_inv > 0 else 0
        )
        h.append(
            f'  <p class="method-note"><strong>Sinal contrafluxo:</strong> {rec_text} '
...
def _apply_card_variant(
    card_html: str, variant: str = "", size: str = "", card_id: str = ""
) -> str:
    """Apply variant, size, and accessibility attrs from layout config to a card's HTML."""
    if variant:
        card_html = re.sub(
            r'class="card card-\w+"', f'class="card card-{variant}"', card_html, count=1
        )
    ...
    if title_match and "aria-label=" not in card_html:
        ...
        card_html = card_html.replace(
            '<div class="card"', f'<div class="card" role="article" aria-label="{label}"', 1
        )
        card_html = re.sub(
...
def build_sections(e4: dict) -> dict:
        ...
        for section_cfg in layout_sections:
            ...
            while i_ch < len(chart_cfgs):
                ...
                if row_group:
                    ...
                    html += (
                        chart_html(chart_key, chart_title, charts_narrativas, extra_attrs=extra)
                        + "\n"
                    )
                    i_ch += 1
            ...
            print(
                f"  [LAYOUT] {section_id} — OK ({len(section_cfg.get('charts', []))} charts, {len(section_cfg.get('cards', []))} cards)"
            )
    ...
    _usa_summaries = {
        "u1": summaries.get(
            "s5", summaries.get("u1", "Estrutura de custos para visto F1/F2 nos EUA.")
        ),
        "u2": summaries.get(
            "s6", summaries.get("u2", "Processo e compliance EB2-NIW para Green Card.")
        ),
        "u3": summaries.get(
            "u3", "Roadmap de licenciamento NCLEX-RN para atuação como enfermeira nos EUA."
        ),
        "u4": summaries.get(
            "u4", "Cenário financeiro caso a cônjuge não trabalhe nos primeiros meses nos EUA."
        ),
    }
    for i in range(1, 5):
        replacements[f"{{{{SUMMARY_U{i}}}}}"] = _usa_summaries.get(
            f"u{i}", f"USA seção {i} — dados pendentes"
        )
    ...
    if usa_sections:
        ...
        for section_cfg in usa_sections:
            ...
            print(
                f"  [LAYOUT] {section_id} — OK ({len(chart_cfgs)} charts, {len(section_cfg.get('cards', []))} cards)"
            )
    else:
